...
"A 能把手势/状态转手给 B，B 也应该能转手回 A"这类双向机制，很容易只接完整一边——转手用的选项通常是可选参数（如 `delegateXxx?: boolean`），另一边的组件不接对应的事件/回调完全不报错、不 typecheck 失败，代码看起来是完整的，只有真跑到那条交互路径才会发现转手落进没人接的地方。新增或复用这类机制时，两个方向的收发两端都要各自确认接了没有，不能默认"这机制是通用的，另一边大概率也接了"。
