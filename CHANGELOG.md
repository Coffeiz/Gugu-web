...
- **单容器部署迁移脚本**：镜像不再内置数据库后，v1.2.x 及更早的单容器部署（`docker run` / NAS 面板，数据在匿名卷里、含内嵌数据库）升级前必须先运行 `scripts/migrate-single-container-to-compose.sh`，自动完成数据库导出恢复、用户文件与凭据迁移；旧容器与旧卷保留不删除，支持 `--dry-run` 预演。详见部署文档「从单容器版本迁移」。
