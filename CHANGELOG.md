...
- **Admin Docker 多部署模式更新**：更新页识别一体化 Compose、分体 Compose 和纯 Docker 单容器部署；受支持的部署可按对应安全边界执行在线更新，不支持或无法识别的配置会显示原因与手动更新指引。
- **通用自动模式**：GuguChat 可切换自动模式，自动通过操作确认；权限授予、访问范围扩大和澄清问题仍需用户确认。
- **Worker 消息准入背压**：限制进程内尚未确认的消息数量，超出部分留在 Redis 队列；运行中的 Agent 并发上限调整后可即时生效。
- **Admin 调试日志检索**：支持按条件搜索并分页查看调试日志。
- **可配置默认问候语**：管理员可控制新会话是否显示默认问候。
- **画布便签搜索与聊天引用**：支持全局搜索便签、按标题引用，并可将便签拖入 GuguChat。
- **当前时间工具**：需要实时日期、星期和时间时，Gugu 可通过独立工具获取当前时间。

### 改进

- **统一多媒体读取**：`read_file` 支持批量读取文件库图片、历史附件、音频和视频；优先使用当前模型的多模态能力，网络读取仅限图片。
- **大附件流式接收**：IM 单条消息附件总量上限提升至 512 MiB，并以流式方式暂存，减少大文件接收时的内存占用。
- 文件全量校验时分块提示内核回收已读取的大文件缓存，降低同步扫描造成的可回收内存占用。
- **沙盒 Compose 与离线分发简化**：默认 Compose 提供沙盒运行环境；收敛重复服务配置，支持离线镜像包部署。一体化应用镜像不再内嵌沙盒执行镜像。
- **Agent 自动化边界统一**：普通交互不再受产品级工具调用次数限制；定时任务保留独立执行预算。工具循环与确认策略统一，保留 destructive 运行时保护。
- **上下文缓存与反思诊断增强**：统一反思触发阈值，空闲收束缩短至 3 分钟；改进压缩缓存复用、缓存命中变化诊断及反思失败关联，并修正重复注入与缓存率统计。
- **闲置压缩缓存复用**：压缩请求复用主会话快照前缀，减少重复上下文并提升缓存命中稳定性。
- **消息时间注入**：批量消息保留原始发送时间，实时当前时间改由独立工具提供，减少动态尾部变化。
- **文件与工作区工具优化**：统一个人、项目和工作区目录操作，限制过深目录遍历、降低 grep 读取开销，并按用户时区显示文件日期。
- **部署与运行状态更清晰**：改进多部署模式在线更新、镜像构建缓存命中和 Admin 沙盒状态识别。
- **界面细节统一**：统一弹窗关闭按钮样式与文件大小显示；优化 Markdown 任务列表、模型列表弹层和用量面板标识。
- **文件预览扩展**：支持 CSV 表格预览与原文切换，字幕和播放列表文件按文本只读预览；未知扩展名会先探测内容，避免把二进制文件误当正文。

### 修复

- 修复记忆反思在上下文未达到模型实际 90% 用量时提前压缩、压缩请求无法复用主对话缓存前缀的问题。
- 修复 Anthropic 兼容接口跨轮重复恢复旧工具调用，导致 MiniMax 等服务返回 400 的问题。
- 修复交互终端大量输出时 FitAddon 未计入终端内边距、导致底部内容被裁切的问题；面板尺寸变化时会重新适配终端行列。
...
- 修复生成流心跳短暂丢失时被误回收，刷新页面后才能看到工具调用和回复的问题。

## [1.3.2] - 2026-09-19

### 新功能

- **@ 引用与全局搜索支持技能 / MCP / 定时任务**：聊天 @ 补全、便签引用和顶栏全局搜索现在都能搜到技能、MCP 服务和定时任务；@ 技能咕咕可直接装载其内容，@ MCP 服务可调用其工具（凭据与端点不进上下文），@ 定时任务可查看或调整它；聊天里点击这三类引用 chip 直接跳转对应管理页。
- **定时任务投递目标与周期选择**：Web 侧定时任务可投递到 QQ 群或私聊（当前会话目标自动兼容 QQ 渠道）；周期表单把工作日/周末合并为「每周」并支持勾选周几。
- **文件重命名可同时修改后缀**：文件库、项目文件区和最近文件面板支持分段编辑名称与后缀；只修改文件名和存储路径，不转换文件内容。

### 改进

- **记忆反思闲置收束**：owner 记忆在固定轮数阈值外，连续 15 分钟无新对话也会自动反思；owner 与群/群友反思共用闲置策略和追加分支执行入口。
- **反思复用主会话分支**：知识反思、记忆反思和群反思统一追加到原会话分支执行，复用已有上下文前缀；反思任务只追加规则和处理边界，不再重复发送整段目标消息，进一步降低输入量并提升跨轮缓存命中率；清理旧的独立反思和实验性缓存实现。
- **反思触发统一按 admin 阈值**：记忆反思严格按管理后台设置的轮数阈值触发，工具回合不再提前立即反思，与普通回合同一口径计数。
- **长流程上下文滚动压缩**：工具调用持续过程中滚动摘要较早历史，保留当前请求和最近完整执行轮，降低长对话上下文溢出风险。
- **LLM 调用失败自动重试全链路统一**：各 provider 链路统一重试节奏（上游过载/超时自动重试），重试过程对用户可见，错误提示带脱敏后的上游标签；修复 OpenAI 兼容链路缺重试的问题。
- **文件空间语义统一**：文件夹工具统一支持个人、项目和工作区三类空间，创建、浏览、移动等操作返回明确的空间与路径信息。
- **循环诊断可观测性**：为工具轮次、provider 重试、重复调用熔断和轮次上限增加脱敏结构化诊断记录，便于定位偶发空转问题。
- **TextViewer 相对图片直接显示**：Markdown 预览里的相对路径图片自动定位到文件库同库文件显示；图片链接改索引化查找，多图文件打开不再卡顿。
- **QQ 群 @ 成员识别白名单**：多机器人同群场景下，@ 已登记群成员的消息按群记忆转换识别，不再漏认或全员误响应。

### 修复

- **取消工具调用后的强制自动复查**：工具执行结果直接交给模型继续收束，不再无条件追加读取或复查轮次；保留重复调用、工具预算和绝对轮次守卫，避免任务无限循环。
- **文件搜索与工具状态实时更新**：全局搜索排除已删除文件夹并显示所属位置；工具事件按稳定消息标识更新，修复完成状态、结果气泡需要刷新后才显示的问题。
- **web_download 事务清理**：网络下载前结束文件夹读取事务，连接失效时安全清理，避免后端连接关闭后再次 rollback 导致 `Transaction.rollback` 异常。
- **大文件分片传输**：移除文件发送、文件库上传和 `web_download` 的单文件大小门槛，统一按分块流式传输，并改用文件库/聊天暂存总容量限制；文件播放支持 HTTP Range，降低大音频传输压力。
- **文件流式下载边界**：下载接口补齐文件存在性、删除态和真实大小校验；空文件返回正确的零长度响应，大文件下载改走浏览器直下不占内存，失败给出提示。
- **Responses 工具链与历史兼容**：stale-chain 改走无状态历史时仍使用统一瞬时重试；跨轮工具调用及旧历史出站时补齐 Responses item ID，失败轮次的空 assistant 行不再污染后续上下文。
- **Responses 回退用量统计**：兼容回退成功后仍将本轮 usage/cache 记录到 LoopScope，避免请求成功但后台统计显示为 0。
- **定时任务与反思迁移边界**：只修改任务其他字段或渠道时保留已有 QQ 群目标；升级时将旧版 owner 私聊反思缓冲按 session 拆分迁移。
- **Sandbox 运行状态检查**：由 sandboxd 检查其实际管理的 Rootless runtime，不再误探测 Backend/Worker 的 Docker daemon；修正 sandboxd 和一次性容器继承的 HTTP 健康检查配置。
- **取消/失败的 run 保留已生成内容**：run 被取消或失败时，已产生的展示产物照常落库，不再整场消失。
- **MCP 动态工具确认循环**：动态工具确认门桥接统一确认按钮，修复确认后再次弹确认的死循环。
- **LLM 流式解析崩溃兜底**：anthropic 兼容端点流式解析越界时自动降级非流式重试，不再整次请求报错。
- **send_file 报 NameError**：分块流式改造遗留的变量引用修正，本地路径文件发送恢复正常。
- **文件气泡 hover 过渡**：亮暗两色的悬停阴影淡入淡出恢复正常（transition 清单补回 box-shadow）。
- **工具事件时间线错序**：失败或取消后按落库时间排序会造成工具事件乱序；改为按真实执行顺序持久化，IM/定时任务会话刷新后工具气泡不再整体跳到对话前面。
- **Markdown 与定时规则解析**：正文中的 Markdown 分隔线不再误删表格内容；修正标准 cron 周字段编号偏移。
- **工具参数约束与纠错提示**：工具目录补充可识别的模式互斥条件，并增强文件编辑、思维笔记结构错误时给模型的调用提示。
- **会话历史交互卡片堆积**：选择卡/确认卡现在随会话消息裁切一起清理（仅已完结的），不再永久累积挤占会话历史；引用分组标签的键名外露修正。
...
- **MCP 服务接入（PRD-MCP-1）**：新增独立的「MCP」页面（侧栏独立入口，`/skills/mcp` 旧链接自动重定向），可接入 Streamable HTTP 与 stdio 两类外部 MCP 服务，工具按用户动态加载进咕咕的工具集（stdio 经沙盒运行，容器内隔离）；凭据采用槽位制——在服务配置里声明 header/query 槽位，凭据值只在服务端加密保存、按当前用户注入请求，不进日志与模型上下文；支持每服务工具白名单、超时与调用确认门（全部确认/自动调用）；服务数与单服务工具数有上限，超限整服务拒绝加载并给出明确提示。
- **MCP 凭据编辑明文可见**：编辑 MCP 服务时已保存的凭据值直接回显在槽位里，可查看、修改后保存（只在传输与落库加密）；旧版 headers/query_params 配置在编辑视图自动转换成槽位，保存即迁移。
- **聊天 @ 引用带上文件 id 与目录**：引用文件时咕咕直接拿到文件 id、所在目录路径与目录 id，「把这些图片移到同目录的插画参考」类指令不再需要按文件名搜索定位。
- **@ 引用支持文件夹**：聊天与笔记的 @ 补全能搜到并选择文件夹；引用文件夹时咕咕拿到目录 id、完整路径与内含文件清单，「把这个目录里的图片归类」类指令直接以目录为操作范围。
- **拖文件进聊天直接变 @ 引用**：从文件库或项目编辑卡把文件/文件夹卡片拖到聊天窗口任意位置松手，即转成 @ 引用 chip（不产生附件拷贝）；悬停有「松开，添加为引用」提示，重复拖同一对象自动去重。

### 改进

- **恢复一体化镜像内置 PostgreSQL/Redis（单容器一键部署）**：`docker run` / NAS 面板单容器部署重新开箱即用（内置库 supervisord 托管、只监听 127.0.0.1）；持久化守卫要求 `/data` 绑定宿主机目录，匿名卷默认拒绝启动（`GUGU_ALLOW_ANONYMOUS_DATA=1` 可临时放行），完全未挂卷一律拒绝，并兼容 NAS 面板严格权限的映射目录；Compose 部署不受影响。
- **聊天链接按钮（PRD-LLM-24）**：新增 `send_link_buttons` 工具，咕咕可在回复下方渲染可点击的链接按钮组；QQ/飞书走原生卡片按钮，微信与不支持的场景自动降级为文本链接；网页端气泡下方渲染安全锚点按钮（新标签打开 + noopener）；出站层对 URL 做二次校验（https、凭据/内网/端口拒绝、部署级域名白名单），不信任上游原始链接。
- **知识清单截断如实提示**：知识条目超过清单容量时明确标注剩余条数并引导先列举拿 id 再直读，旧知识不再对模型「隐形」。
- **镜像与运行时瘦身**：罗马音词典换 sudachidict small 档（venv 约 -190MB）、清理未使用的前端字体资源（约 -124MB）。
- **MCP 凭据槽位输入框默认加高**，JSON 槽位一眼可见更多。
- **咕咕文件浏览工具统一为 list_dir**：单入口按路径逐级定位目录（如「素材/2024/插画」），文件夹带文件数，结果截断如实标注并支持确定性分页——大目录不再被当作全量、找不到目录时报错会列出同级可用目录。
- **文件信息弹窗暗色适配**：操作按钮与关闭按钮在暗色主题下使用暗色表面，不再白底突兀。

### 修复

- **登录态处理**：修复用户与管理员登录态失效时的处理；退出登录后重新登录会回到退出前的页面（如 MCP 页），不再跳回首页。
- **Admin 切换 MCP 开关后全局 500**：配置热更新缺 `mcp` 段合并分支，开关一次后 MCP 状态接口持续报「服务器内部错误」且无法恢复；现配置解析保持强类型，开关即关即用。
- **MCP 稳定性复审修复**：空闲会话回收与外呼的竞态、凭据「未修改」被误判为变更等问题修正。
- v1.2.3 单容器部署等待 `postgres:5432` 超时失败的问题随内置库回归一并解决。

## [1.2.3] - 2026-09-15

### 新功能

- **管理员一键重建 RAG 索引**：管理后台新增按用户重建全部 RAG 来源索引的入口，后台执行并显示进度；只更新派生索引，不修改原始文件、对话或业务数据。
- **文件库压缩与解压**：文件库支持把文件/文件夹压缩为压缩包、解压到指定目录；解压时展示进行中反馈，重复的顶层目录自动去除。

### 改进

- **RAG 索引目录统一**：Memory 与统一来源索引统一收纳到用户存储目录的 `.agent/rag/` 下；新版本直接重建，不读取或迁移旧索引目录。
- **文件来源索引收敛为文件名**：文件 RAG 不再读取和切分文件正文，索引体积更小，正文继续通过文件读取和预览链路按需获取。
- **Office 预览交互优化**：改进 DOCX 页面缩放、视图模式记忆、缩放锚点和预览窗口尺寸适配；工具栏比例显示支持小数缩放。
- **预览窗口交互优化**：标题栏支持双击最大化；宽表格和 Office 文档按内容尺寸打开，减少首次打开时窗口尺寸跳变。
- **主题与项目归档布局优化**：亮色模式下内容分割线更清晰，归档/已删除项目收起时不再保留多余底部空间。
- **QQ 键盘按钮按标签宽度自适应换行**：选项多、标签长时不再全部挤一行被截断；按钮按估算宽度贪心排列（行预算常量可按真机校准），长标签独占一行，超出键盘 5 行容量自动退回文本序号兜底。
- **RAG 检索支持按月查询**：数字日期、中文月名和英文月名统一映射为规范月份词项，「三月的记录」「September 的文件」类按月查询不再漏召回。
- **记忆注入更诚实也更宽裕**：明确告知模型「注入的记忆是有限快照，没出现不代表没发生过」，涉及过往事实先检索再回答；注入预算从 2000 提升到 3000 字符，对话压缩摘要目标约 2000 字。
- **Responses 推理续接自动适配**：对 OpenAI 兼容接口按需探测 Responses 能力，成功时启用续接，不支持时自动保留 Chat Completions；临时网络错误不会长期阻塞后续探测。
...
- **镜像签名与更新验签（OCI referrers）**：发布流程对四个镜像做 Cosign keyless 签名，签名以 OCI 1.1 referrers artifact 挂在镜像 digest 下，不再生成 `sha256-<digest>.sig` 普通镜像标签；一键更新前用固定 digest 的官方 Cosign verifier 校验发布签名（锚定 GitHub Actions 发布工作流身份），验签失败即阻断更新。主镜像不内置 Cosign，不增加体积与 CVE 面；灾难恢复可用宿主机 env `GUGU_UPDATER_SKIP_COSIGN=on` 显式跳过（会体现在预检与更新历史中）。

### 修复

- **RAG 冷启动与索引增量更新稳定性**：并发恢复时避免重复启动 worker，冷启动探活使用构建级超时；索引缓存可在统一目录下正确复用和增量更新。
- **XLSX 预览稳定性与安全边界**：限制超大压缩包、XML、图片和表格解析范围；图片按需加载并缓存，超限预览返回明确错误，不再以 500 失败。
- **文档与测试契约同步**：同步 Python 与 TypeScript 文件适配器行为、索引目录结构和构建产物，避免文件正文重新进入 RAG 索引。
- **RAG worker 响应契约**：修复 TypeScript worker 与 Python 调用方之间的响应类型不一致，避免索引任务结果解析异常。
- **上下文缓存稳定性**：清理工具名中的不安全字符，避免工具调用名称污染模型上下文缓存。
- **文件同步配额读取崩溃**：绑定目录对账时配额信息缺失不再抛 `TypeError` 中断同步。（#60）
- **LLM 上下文溢出识别与配置校验**：上下文超限错误能被正确识别并给出可读提示；管理后台保存模型配置时校验「最大输出不得超过上下文窗口」，避免误配导致每次调用必报错。（#60）
- **项目阶段进度卡片**：进度刷新不再滞后；阶段悬停展示增强。
- **笔记时间轴**：拖拽中断后惯性动画不再失控。
...
- **一键更新生命周期与 rootless 兼容修复**：更新 helper 绕过应用启动入口并继承数据库配置，避免等待数据库后超时；自更新重启后可恢复任务状态，并正确使用宿主机实际 Docker socket 挂载。
...
- **RAG 注入内容不再重复**：记忆类记录的摘要原是正文截断拷贝，导致每条注入内容出现两遍；现自动去重，存量数据无需迁移。
- **文件卡误点击系列**：重命名拖选松手到别的卡片上不再误开预览或进目录；在卡片内输入框中点击不再被当成卡片点击；按下的目标被移出 DOM 时不再误触发。
- **确认门授权原子消费**：一次性授权在并发下不会重复使用。
- **当前时间提醒边界明确**：避免把仅供时间参考的系统提醒原文带入回复。
- **egress 引导服务启动脚本权限兼容修复**；聊天输入框文字下沿不再被裁切。

## [1.2.1] - 2026-09-13

### 改进

- **敏感操作确认范围统一**：画布、项目、日历、客户和回收站等批量删除可一次确认完整目标集合；定时任务创建/修改也统一接入确认，沙盒权限支持批量授权或撤销，但仍逐任务隔离，不会扩展到未选任务。
- **句末冒号自动续写**：回复以冒号结束时会自动尝试补全，减少半句话结束，也避免把普通说明误判成工具操作承诺。

### 修复

- **批量操作确认边界修复**：回收站仅有文件或文件夹时也能正常发起清空确认；批量沙箱授权确认执行后即失效，撤销后重新授权需要再次确认；画布批量删除缺少便签 ID 时返回参数错误。
- **定时任务附件投递**：文件库文件与暂存附件统一走 IM 附件发送器，修复文件库产物因 `file_id` / `attach_id` 格式不同而漏发，并校验文件归属。
- **项目阶段模板显示与重命名**：已保存的内置模板会随界面语言正确显示；阶段预览不再裁掉字形下伸部分，重命名输入框不再被挤窄，确认和删除按钮保持在行尾。

## [1.2.0] - 2026-09-13

### 新功能

- **会话待发消息队列**：网页聊天里排队发送的消息现在保存到服务端——关闭页面、刷新或换浏览器都能恢复队列并自动继续发送；多个标签页同时打开时通过租约自动防重复派发；排队消息改为输入框上方的气泡卡展示，不再混进对话流。
- **IM「显示中间回复」开关**：个人资料的 IM 面板新增开关（默认开启）。关闭后飞书/QQ 只发送最终回复，不再推送工具调用过程中的中间草稿。
- **Markdown frontmatter 预览**：文件查看器打开 .md 文件时识别并表格化展示 YAML frontmatter 元数据；正文任务勾选按头部偏移映射原始行，不再写错行。
- **新增 list_skills 技能清单工具**：咕咕被问「你有哪些技能」时能按需列出内置与自定义技能的名称、用途与状态，不必加载技能正文。
- **Knowledge 条目触发式描述**：知识条目支持单行 description，进入 RAG 索引文本参与召回，并计入文档版本戳驱动增量重建。

### 改进

- **RAG 检索大幅提速，长对话不再超时**：索引改为增量同步、检索服务抽成常驻进程并加大缓存预算——长对话每轮之后的第一查从 5-10 秒全量重建降到几十毫秒，2 秒以上的检索卡顿不再出现；文件等大来源索引重建持续失败的问题一并修复。
- **界面主题：常驻内容面去磨砂**：常驻内容面板默认去掉背景模糊，只保留半透明底色与描边；模糊效果保留给弹窗、菜单等临时浮层；卡片取消整卡 hover 变色，反馈集中在内部控件。整体更清爽，滚动与渲染开销更低。
- **上传上限统一提到 512MB**：聊天附件与文件库上传上限（原 10MB / 200MB）统一提到 512MB，超限提示按新上限计算；上传改为分块流式处理，大文件不再一次性占用整块内存。超过 64MB 的覆盖上传不再支持网页端撤销（避免为撤销留存双份大文件）；语音录音超过 64MB 会被拒绝，超大音频附件按原格式暂存、不再做进程内转码。
- **画布与定时任务写入接入确认门**：画布批量操作、定时任务创建与修改现在会弹出确认卡，与其它敏感操作同一套确认体验。
- **沙盒出网代理开箱即用**：非 Compose 部署新增 `gugu-sandbox-egress` 引导服务，自动拉起 Squid 代理容器；出网档位按管理后台配置生效。
- **时间提醒不再打断前缀缓存**：当前时间从对话历史移到每请求动态注入，跨天等时间变化不再导致模型前缀缓存整段失效。
- **咕咕删除项目改为软删除**：与网页端同口径，删除的项目不再直接消失。
- **镜像基础包安全补丁完整化**：生产分离部署与一体化镜像改为直接从 Debian security pool 做整段安全升级，消除镜像源同步滞后残留的 gzip/glib/mbedtls/pcre2/sqlite3/libssh2/perl 等 HIGH/CRITICAL 漏洞。

### 修复

- **ask_user 无选项提问网页端无法回答**：不带选项的提问此前在网页端没有任何回答入口，现提供自由输入的兜底回答。
- **压缩后上下文重复拼接**：摘要水位竞态导致「新摘要 + 旧基线」重复拼进上下文；工具调用参数跨轮次字节保真，修复旧会话工具历史导致 DeepSeek 请求失败的问题。
- **QQ 回复顺序错乱**：ask_user 选择卡先于说明文字到达——现在交互卡发出前先冲刷本轮已生成的正文。
- **沙盒 `curl -o /dev/null` 误报越界**：路径守卫放行容器内 `/dev` 标准设备路径。
- **视觉能力判定过严**：自定义模型名或未点「检测」的新预设显式开启视觉后，不再被静态能力表反向否决。
- **杂项**：对象卡链接内加粗文字不跟随 hover 变色、日历侧栏事件描述长文本溢出、项目颜色传语义色名被规范化为 None、Admin「记忆压缩」页签读错模板文件导致恒为空。

## [1.1.4] - 2026-09-11

### 改进

- **Knowledge 检索不再偶发报内部异常**：TS RAG worker 与缓存的索引 revision 不一致时，旧版本会带着内部异常类名回执给模型；现在 RAG 层会先作废缓存并按当前 revision 全量重建一次再重试，模型和用户都看不到这条故障，只在 gugu-diag.log 留诊断。

### 修复

- **工具回执不再透传内部异常类名**：把已知内部异常注册成对人话（索引重建中请稍后重试），工具报错只给一句话；类名与原始消息仍按规范只进受限诊断出口，模型不再据此误判服务挂了。新增同类型异常只需在注册表里登记一行。
- **磁盘旧版本 RAG 索引自动清理**：RAG 用户索引目录多了按制品版本戳清理，不一致且严格更旧的索引不再等 30 天 TTL、升级 / 重建完直接回收；取不到版本或解析不了时一律按未知处理、不删数据。
...
- **Shell 自动模式**：开启管理员和用户两级自动模式后，危险命令及临时 egress 不再重复弹出确认；沙盒、代理、配额、超时和审计边界仍然生效。
