...
- **默认问候接入感知、不再催人**（`agent/greeting.py`）：原问候 prompt 明写「优先挑最近在推进的项目提」→ 一开口就像催活。改为 ① 默认**暖的、开放的招呼**（先当朋友打招呼、把话头交回用户，别一上来提项目/待办/日程）；② **接入感知**——读 per-user `stance`（`.agent/stance.json`，上次的相处状态）+ summary 定口吻（累/情绪不高→温柔别提活、闲聊→也轻松、专注推进→顶多轻轻关心一句并留「先歇会儿」空间）；③ 项目/日程降为**可选背景**（多数不提、真提只挑一个用关心口吻、不催办不报进度）。是「默认=推进」病根在问候面的修复。
