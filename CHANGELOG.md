...
- **MCP 服务接入（PRD-MCP-1）**：「技能」页新增「MCP 服务」子页，可接入 Streamable HTTP 与 stdio 两类外部 MCP 服务，工具按用户动态加载进咕咕的工具集（stdio 经沙盒运行，容器内隔离）；凭据采用槽位制——在服务配置里声明 header/query 槽位，凭据值只在服务端加密保存、按当前用户注入请求，不进日志与模型上下文；支持每服务工具白名单、超时与调用确认门（全部确认/自动调用）；服务数与单服务工具数有上限，超限整服务拒绝加载并给出明确提示。
