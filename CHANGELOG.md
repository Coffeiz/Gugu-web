...
- **文件重命名可同时修改后缀**：文件库、项目文件区和最近文件面板在普通重命名时即可分段编辑名称与后缀；只修改文件名和存储路径，不转换文件内容。
...
- 修复失败或取消后工具事件按落库时间错序、正文 Markdown 分隔线导致表格内容被误删，以及标准 cron 周字段编号偏移。
- 修复工具参数目录未表达可识别的模式互斥条件，并增强文件编辑和思维笔记结构错误时的调用提示。
...
- **文件库大文件下载失败**：下载接口补齐文件存在性/删除态/真实大小校验（历史 size 脏数据不再导致 Content-Length 失配断连），大文件下载改走浏览器直下不占内存，失败给出提示。
...
- **IM 会话工具气泡刷新后错位**：IM/定时任务会话的展示时间线现在包含工具项并按真实执行顺序持久化，刷新后工具气泡不再整体跳到对话前面，与 Web 会话一致。
