...
- **恢复一体化镜像内置 PostgreSQL/Redis（单容器一键部署）**：v1.2.3 移除内置数据库导致 `docker run` / NAS 面板单容器部署在等待 `postgres:5432` 时超时失败。内置路径回归镜像默认开启（supervisord 托管、只监听 127.0.0.1），Compose 部署显式 `GUGU_EMBEDDED_DEPS=0` 不受影响；同时补上持久化守卫：内置数据库要求 `/data` 绑定宿主机目录；匿名卷（未绑目录的兜底）默认拒绝启动并给出绑卷指引，显式 `GUGU_ALLOW_ANONYMOUS_DATA=1` 可临时放行；完全未挂卷（数据在容器临时层）一律拒绝。
