...
- **`http_get` 按 Content-Type 自动提取正文，不再无脑截断原始响应体**（`agent/tools/web.py`，新增依赖 `trafilatura`）：真实验证案例——某文档站首页截断到4000字符给模型的内容 100% 是 `<meta>`/CSS/JS 头部噪音，网页明明抓得到（200），但截断发生在提取正文之前，模型拿到的东西完全没法用。改为 `text/html` 用 `trafilatura` 提取正文转 markdown（去导航/广告噪音，带内联链接——想接着读某条链接直接再调一次 `http_get`，不用重新搜，等于免费拿到「多跳阅读」能力）；提取不出实质内容（可能是纯 JS 渲染页面）直接提示模型改走 `web_search`/`deep_research`，不把空内容硬塞给模型自己猜；`application/pdf` 复用 `app/core/doctext.py` 现成的 pdftotext 提取；其它（JSON/纯文本，含天气用的 wttr.in）原样返回不受影响。
...
- **文件预览窗支持直接编辑文本/代码文件**（`components/common/viewers/TextViewer.vue`，新增依赖 `codemirror`/`vue-codemirror`/`@codemirror/lang-*`）：此前文本类文件（md/txt/代码）只能看不能改，想改字段/配置/脚本得下载下来在本地编辑器改完再传回去。分两条路：txt/md 本来就不大，保留"预览+编辑"切换，编辑是普通 textarea；JSON/YAML/Python/JS/TS/Go/Rust/Java/C/C++/SQL/Shell 等代码类扩展名（跟后端 `TEXT_EXTS` 对齐）改用 CodeMirror 6，直接就是编辑态、没有单独的只读预览（它本身就能当预览用），真实文件改动停顿 800ms 自动保存（不是敲一个字符存一次，也不会因为改到一半切走文件而丢改动或存错文件——待存内容和文件 id 在排队那一刻就闭包捕获住了），聊天附件等非真实文件是只读态。中间还踩了一版弯路：最初用 hljs 高亮结果叠一层透明 textarea 做"编辑时也高亮"，实测 1100 行文件打字会卡到几秒（hljs 每次全文重新高亮 + `v-html` 整体替换 DOM 的开销，不是防抖能解决的架构问题），换成 CodeMirror 后从根上解决——它做虚拟滚动（只渲染可视区域）+ 增量分词，编辑大文件不受行数影响；CodeMirror 全部懒加载，不影响所有用户都要下载的主 chunk。

### 修复

- **docx/xlsx/pptx 预览「转换失败 (500)」——两层部署配置问题叠在一起**（`gugu-backend.service`/`gugu-worker.service`/`gugu-supervisor.service`、`app/api/v1/files.py`、`app/core/doctext.py`）：真实翻车案例，用户传的 docx 打不开。①三个 systemd 单元的 `Environment="PATH=..."` 只填了 venv 的 bin 目录、把系统路径整个顶掉，`libreoffice`/`pdftotext` 这些系统命令再装也找不到（`FileNotFoundError`），改成 venv bin 优先、后面接完整系统路径。②修完①露出第二层：三个单元开了 `ProtectSystem=strict`，`$HOME/.config` 对进程只读，LibreOffice 建不了默认用户 profile 直接失败（`returncode=1`，stderr 只留一条不相关的 `javaldx` 警告，真实原因不会自己冒出来）——给两处 LibreOffice 调用都加 `-env:UserInstallation` 指到本次调用专属的临时目录（`PrivateTmp=true` 保证可写），不用放宽沙箱安全设置。均已在 devserver 实测验证。
- **`http_get` 反复超时后咕咕答非所问**（`agent/prompts/skills.md`）：真实翻车案例（QQ 会话），用户让咕咕查某文档，`http_get` 连续3次网络超时/连不上，工具报错已正确回传给模型，但最终回复是一句毫不相关的填充语——不是「没调用工具」，是调用了、失败了，但不知道怎么如实说「连不上」。补一句要求：连不上/超时试一两次仍失败就直接告诉用户，别用无关话搪塞。
...
- **建项目支持优先级，`set_priority` 独立工具下线**（`agent/tools/projects.py`、`app/core/events.py`）：优先级此前得建完项目后另外调一次 `set_priority` 补，且从没被主动问过/推荐过。现在 `create_project`/`update_project` 直接接受 `priority` 参数，一次建好；`create_project` 说明加三档指引——有明确紧急信号直接给并说理由，项目分量不轻但语气看不出紧急度就顺口问一句带推荐，日常小事不问不设。`update_project` 本就是通用字段更新入口，单独为一个字段开工具意义不大，已删除该工具及其 handler；工具清单 59→58。`set_color` 不受影响。
...
- **Intent Router 空闲时拦截"查到了吗"类追问 + 意图守卫漏判省略主语的承诺句**（`agent/router.py`、`agent/core.py`）：真实案例——咕咕报错后回复「马上重新查一下」，用户追问「查到了吗」，咕咕只回了句寒暄，没真的去查。两处根因：① `router.py` 的关键词短路不看忙闲，状态已回 IDLE 时把追问拦成空话、压根没进主模型，改为只在真「忙」时才短路；② `core.py` 的意图守卫要求"我这就去…"类宣告显式带"我"开头，但"马上重新查一下"是常见的主语省略、且"马上"和"查"间插了"重新"，双重原因测不中，已放开这两类变体。真实对话文本验证两处均正确命中。
...
- **全项目安全审计 + 一批修复**（`app/core/ratelimit.py` 新增 + `agent/tools/{files,base}.py` + `app/api/v1/{auth,admin_auth,config}.py` + `app/main.py`；报告 [docs/security/安全审计报告-2026-07-03.md](docs/security/安全审计报告-2026-07-03.md)）：对认证授权与多租户隔离 / 注入·SSRF·文件与工具 / 密钥·配置·日志·部署面三个攻击面做了一轮完整审计。**核心结论是应用层内核扎实**——`get_owned` 归属校验全覆盖、错误脱敏真挂在 `registry.dispatch` 唯一收口、IM 凭据 AES-GCM 加密且经 env 注入子进程、登录防枚举、JWT 算法固定、SQL 全参数化、无命令注入/反序列化面、13 个 `/admin/*` 全挂 `require_admin`，逐条核实到位；审计中还证伪了两条误报（上传 `ext` "路径穿越"——`rsplit('.',1)` 取的段按定义不含 `..`，实测所有 PoC 都逃不出用户目录；admin 决策轨迹"IDOR 泄露对话"——正文其实已 `_redact_text` 脱敏）。默认凭据（默认 admin 口令 / 默认 `secret_key`）经确认生产端已设强值、不成立，仅建议将来加启动 fail-closed 守卫。**本次修复的真实问题**：① **H3 SSRF 重定向绕过**——`send_file` 下载网络图片时 `follow_redirects=True`，而私网校验只查初始 URL，公网页可 `302` 跳 `169.254.169.254`/内网绕过（对比 `http_get` 早已 `follow_redirects=False`，两处不一致即遗漏）；改为手动跟随、每一跳都重新过 `_url_is_safe`（最多 3 跳）。② **H2 全站无限流**——新增轻量 Redis 固定窗口限流件（fail-open，Redis 挂了不锁死登录），挂到用户登录（10 次/5min·按 IP+用户名）、注册（20/h）、找回密码（5/h）、admin 登录（10/5min）。③ **H4 邀请码注册竞态**——查 `used_at is None` 与置位之间无行锁，并发同码可注册多号；改为原子 `UPDATE … WHERE used_at IS NULL`、`rowcount≠1` 即拒（实测两并发抢同码 `rowcounts=[1,0]`）。④ **上传无大小上限**——加单文件 200MB 硬闸（整个请求体一次性进内存、配额可能为 None）。⑤ **安全响应头缺失**——中间件补 `X-Content-Type-Options`/`X-Frame-Options: SAMEORIGIN`/`Referrer-Policy`/`HSTS`（CSP 因 Vue 内联风险暂缓）。⑥ **config 端点回传 traceback**——PATCH/init-db 的 500 不再把 `format_exc()` 塞进 detail，改服务端日志 + 前端通用消息。⑦ **嵌套 error 漏脱敏**——`_redact_result` 改递归，批量工具 `failed`/`saved` 列表里 `{"error": str(e)}` 的原始路径/连接串也过 `sanitize_error`（实测嵌套已抹、正常内容不动）。⑧ **重置链接信任 `Origin` 头**——改用服务端 `request.base_url`（经 nginx 固定为真实域名），防伪造 Origin 投毒钓鱼。⑨ **审计日志恒记 "admin"**——`require_admin` 把 `payload["sub"]` 落到 `request.state.admin_username`，多管理员可归因。全部改动经 `py_compile` + 模块导入 + devserver 上 10 项回归测试（真实 DB/Redis）验证通过。暂缓项（需更大改动，报告 §9 列明）：H3 的 DNS-rebinding 残留（pin IP 会破 HTTPS）、token 撤销、admin 日志 SSE token 走 URL（需前端换 fetch 流式）、加密密钥与 JWT 密钥解耦（需迁移重加密）、XFF 可信代理白名单。
...
- **全站 TypeScript 转换收尾（51 个 `.vue` 文件补 `lang="ts"`）+ 顺带修复三处真实 bug**：此前 67 个 `.vue` 文件里只有 14 个接了 `lang="ts"`（`tsconfig.json` 走渐进式迁移：`checkJs:false` 让存量 JS 不参与检查，只查已标 ts 的文件），本次把剩余 51 个逐一补上并清空随之暴露的类型错误（`App.vue`/`AdminApp.vue` 无 `<script>` 标签，天然跳过；初始 196 条错误 → 0）。方法论是优先定位"欠类型的根声明"而非逐条打补丁——例如 `GuguChat.vue` 的 `messages = ref([])` 补一个 `ChatMessage` interface，单个改动就消掉 27 条错误；`composables/useBoxSelection.ts` 的 `ref(new Set())` 补成泛型 `Set<F>`（文件库用字符串文件夹 key、项目编辑卡的 `parseFolderId: Number` 用真数字，两边类型天然不同），消掉 `Files/index.vue`/`ProjectModal.vue` 里一大片级联错误，中途踩到 Vue `ref<T>()` 遇未约束泛型参数时 `UnwrapRef` 解析失真的已知限制，改用 `shallowRef`（Set 本来就不做深层响应式展开，行为不变）绕开。类型检查顺手抓出三处一直存在的真 bug：① `ProjectModal.vue` 下载命名逻辑引用了一个从未声明过的 `selectedProjectId`（死代码，真触发就是 `ReferenceError`），改用同文件里另一处已验证的 `props.project?.name` 写法；② `Privacy.vue` 隐私政策正文里 `` `/memory` ``/`` `/forget <内容>` `` 两处反引号忘了转义，把外层 JS 模板字符串从这里截断（对比同文件 30 行外正确转义的 `` \`localStorage\` `` 才发现前后风格不一致），编译产物核实过修复后完整文本正常还原；③ `Files/index.vue`/`ProjectModal.vue` 里多处 `filesApi.update`/`filesApi.copy` 调用传 `folder_id`/`display_name`/`project_id`（下划线），实际 schema（`CamelModel`）要求 `folderId`/`displayName`/`projectId`——查了后端确认 `populate_by_name=True` 让这些调用运行时歪打正着能用，仍改成规范驼峰写法而非绕过类型错误了事。验证：`npm run typecheck` 0 错误、`npm run build` 构建通过、Vite 开发服务器对所有改动文件实时编译无报错，且在编译产物里逐一核对过上述三处修复确实生效。
...
- **记忆向量语义检索（可选自托管 embedding，默认关闭·零副作用）**（新 `agent/memory/embedding.py` + `store.py` + `app/core/config.py` 独立 `EmbeddingSettings` 段 + `app/api/v1/config.py` 测试/重建端点 + `Admin/Agent/index.vue` 配置 UI）：记忆检索从词法（字 bigram Jaccard）升级到向量语义——facts 超注入上限时按语义相关性挑，而非字面重叠（"我压力好大"能匹配到"是设计师、赶稿易焦虑"）。定位为一层**共享 embedding 基建**：记忆检索是第一个消费者，未来相处镜片的记忆引用/检索排序都接同一层。**默认关闭，全链路退回词法，对现有行为零副作用。**
  - **模型独立 pin、与聊天彻底解耦**：新增 `EmbeddingSettings` 独立配置段，**不进 `pick_model` 路由**——聊天模型天天轮换也碰不到向量；只有换 embedding 模型才需重建。走 OpenAI 兼容 `/embeddings`；key 为空不发 `Authorization` 头（自托管 Ollama 无需鉴权，实测 `qwen3-embedding:0.6b` / 1024 维连通）。
  - **写侧**：反思写完 facts 增量 embed（只算新 fact / 换过模型的，删的 GC），跑在异步反思里、off 热路径。**读侧**：`read_memory` 在启用且 facts 超上限时才 embed query（轻用户零成本），`render_facts` 有向量走 cosine 语义、无则退回 bigram；只认当前模型 tag 的向量（换模型后旧向量忽略 → 自动退回词法直到重建，无空窗）。
  - **规模**：per-user 几十~几百条 facts，Python 暴力 cosine 即可，**不需要向量数据库**。向量与 facts 分开存（`.agent/facts_vec.json`）——文本是主数据、向量是可重建缓存。
  - **admin（Agent 配置·行为 tab）**：配置表单 + 启用开关 + 连通测试（返回维度）+ **重建向量**按钮（换模型后批量重算所有用户、后台跑 + 进度轮询、期间检索自动退回词法）。设计见 `docs/agent/参考/咕咕改进方案-MaiBot借鉴.md` 改进一。
- **飞书 / 微信支持消息引用识别，QQ 协议层做不到**（`agent/adapters/{feishu,wechat}.py` 新增 `_fetch_quoted_text`/`_extract_quoted_text`）：用户在 IM 里"引用/回复"某条历史消息发送时，之前咕咕只看到新发的这条文本，感知不到"这句话是针对哪条历史消息说的"——三个平台的接入代码原本都没读取引用字段。用真实引用消息在三个平台上实测，能力差异很大：**QQ** 官方机器人 C2C 单聊协议（`botpy` 的 `message_reference`）验证是空的、引用内容也没拼进文本，协议层硬限制，判定不可行、跳过。**飞书** SDK 收消息事件自带 `parent_id`（被引用消息 id），但只给 id，需要多调一次 `GetMessage` API 反查内容——中间踩了个坑：咕咕自己的回复走 `interactive` 卡片（`_do_send` 优先发卡片，不是纯 `text`），反查回来的卡片内容被飞书归一化成 `{"tag":"text","text":...}`，跟发送时原始的 `{"tag":"markdown","content":...}` 结构不一样，且 `elements` 是数组的数组（按分组/分段组织），第一版按扁平列表解析直接崩 `AttributeError`，改成递归拍平 + 两种字段名兼容后才拿到真实内容。**微信** iLink 最省事，引用消息的 `item.ref_msg.message_item` 直接内嵌了被引用消息的完整内容（含 `text_item`），不用额外查询。两边实现都只包装送进 LLM 的文本（新增 `llm_text`，不改原始 `text`），router/秒回表情逻辑继续用原始文本判关键词，不受影响。
...
- **感知诊断面板：反馈信号分布 + 数据导出**（`api/v1/agent_perception.py`、`Admin/Perception/index.vue`）：配合上面的反馈信号采集，面板加一条**反馈信号分布**条（正向〔确认夸赞/顺着聊/主动分享〕绿、负向〔改写重问/无视跳开〕红，`perc:events` 的 `t=fb` 按枚举计数、活跃用户窗口内，空态提示采集器上线时间）——攒数据期可直接看燃料积累量；再加**数据导出**按钮（`GET /admin/perception/export?hours=` 返回窗口内全量 `perc/misperc/fb` 原始事件 JSON 附件，按 ts 排序、全部脱敏结构化字段无用户原文），供离线分析。
...
- **咕咕悬浮球会挡住聊天窗**（`components/common/GuguChat.vue`）：`.ai-fab` 固定 `z-index:99999`，恒高于聊天窗所在的窗口带（20000+）——展开大窗口（右下角延伸到 `bottom:12px`）和小窗（离球顶仅 10px 空隙）都可能被固定在右下角的球盖住一角。改为动态 `fabZ`：聊天窗打开时（不论小窗/大窗口）球压到 `chatZ-1`、永远不高于自己的窗口，完全关闭时才回 99999（常驻可点的唤起入口）。
- **拖拽克隆缩略图看不出模糊 + 聊天窗大小切换偶发闪烁**（`assets/styles/global.css`、`components/common/GuguChat.vue`）：① 文件卡拖拽克隆里的图片缩略图不透明、`backdrop-filter` 穿不透，卡片模糊效果完全看不出——给缩略图区单独挂一份 `backdrop-filter` + 降图片透明度，让"背后模糊"透出来而不糊图片像素本身。② GuguChat 大小窗切换的 `resizing` 状态原靠硬编码 420ms 定时器结束、跟窗口 0.42s 位移过渡零缓冲对齐，性能不足掉帧时过渡还没走完定时器却准点触发、`backdrop-filter` 被提前打开而闪一下——改成监听真实 `transitionend`，定时器降级为兜底。
- **pointer 模式下松手指针原地不动、卡片悬浮态卡在"未悬停"**（`composables/usePhysicsDrag.ts`）：`_revealWithoutStaleHover`（揭示卡片时无条件摘 160ms 命中测试）本是给原生拖拽（`dragstart` 起暂停 `mouseover`/`mouseout` 派发）设计的治标手法；文件卡/项目卡早已全转 pointer 模式、全程不暂停 hover 派发，这手法对它们不必要反而帮倒忙——摘命中测试的窗口内浏览器正确判"没悬停"，窗口结束后指针没挪动、无新 `mousemove` 重判，`:hover` 就永远卡住。pointer 模式下不再套用这套治标手法。
- **文件复制/剪切跨空间粘贴静默失败**（`app/api/v1/files.py`、`schemas/__init__.py`、`Files/index.vue`、`ProjectModal.vue`）：`copy_file`/`update_file` 此前无条件从源文件继承 `space`/`project_id`，「项目文件复制/剪切到个人文件库」这类跨空间粘贴表面成功、文件实际还留在原项目。`FileUpdate` 补 `project_id` 字段，两接口改按调用方显式传入的 `project_id` 决定目标空间；前端 cut 粘贴此前算出了 `projectId` 却没实际发送，一并补上。
...
- **咕咕相处方式重构：persona 纯人格 + 反思驱动 stance 选行为模块**（`prompts/persona.md` + `prompts/behaviors/*.md` + `agent/behaviors.py` + `memory/{store,reflection}.py` + `context/builder.py`）：解决「总爱把对话往推进项目上带、想闲聊时没闲聊感」。① persona **瘦身为纯人格**（删四态打法 + 主动思考），相处行为全部抽成独立模块；② 新增常驻 `baseline`（四态地图 + 中性默认「先理解、别急着推进」）+ `companion`（陪聊）/`execution`/`record`/`query`/`reflect`，与既有 `emotion-first`/`stuck-first`/`decision-explore` 一起按 **1:1 stance** 点亮；③ 模块选择从「正则猜本句」改为**反思（异步 LLM）产出的 stance 驱动**（= `perception.intent`，落 per-user `.agent/stance.json`、带新鲜度衰减 30min 闸；热路径零 LLM、滞后一轮可接受）。效果：推进从「无脑默认」降为「判成执行/推进 stance 才点亮」，闲聊轮纯陪聊、无压力。详见 `docs/agent/10-感知系统.md` §2.6。
- **错读需求案例收集器 + companion 松绑（感知系统续）**（`memory/reflection.py` + `prompts/reflection.md` + `behaviors/companion.md`）：① 反思检测到「感知误读」纠正时，顺带吐**脱敏**的结构化诊断 `miss{read_as, actual, pattern}`（三字段只认固定枚举（read_as·actual=需求类型、pattern=错读模式），枚举外落「其他」、结构上零泄漏；此为 5 轮真实反思验证逼出——free-text 会行内夹带用户内容），单独收进 Redis `perc:misread_cases`、并**持久化进全局 md `_analytics/misread.md`（跨 Redis 不丢）+ admin 下载端点 `/admin/perception/misread/export`** + **后台 Perception 面板「错读案例」预览栏（最近 30 条 read_as→actual + 模式，`/misread/recent`）+ 「下载完整记录」按钮**——把感知误判从「只有数字、没有原因」补成「有具体（脱敏）、可预览可下载的原因」，给后续「需求发现闭环」攒燃料（见 `docs/agent/10-感知系统.md` §11）。② `companion` 松绑：区分「真上心地帮（查一查 / 给角度）」与「生产力式硬推」，分享时先共情、grounding 配着帮，仍保留「别任务化 / 别拐回推进」。
...
- **意图守卫:治「说了要做却没动手」（咕咕"我去查一下"然后停住）**（`agent/core.py`）：回复循环里「自由文字+无工具=终止态」，模型随口宣告意图就触发结束。新增第三类确定性守卫 `_announces_intent`——检测「**我去/我来/这就/稍等我/让我/接下来 + 查/搜/建/改/记/整理…**」这类**将来式宣告**且本轮零工具 → 逼一轮当场调工具（与现有 narration「假装已做完」、decision「擅自不做」守卫并排，anthropic/openai 两路都接）。要求明确的"将要"引导词（避免裸"我+动词"误伤如「我改天再看」）；**问句/征询硬排除**（「要我去查吗?」是在等用户拍板，命中即放过、绝不误逼）。只追一次、不死循环。属「A-lite+B」方案的 phase 1（B），finish 工具 + 翻转终止规则（A-lite 结构件）留待观察后再上。详见 `docs/agent/31-多步执行与防停顿.md`。
...
### 感知系统 P0–P2（新子系统，详见 `docs/agent/10-感知系统.md`）
...
精力（Token 配额）从「软降级」改为「硬拦」，新增满额冻结与全局逐字流式。详见 `docs/agent/12-精力系统.md`。
...
- **可靠性架构文档**：新增 `docs/agent/03-可靠性.md`（Execution Verifier 执行验证层：信 Tool 不信 Assistant）+ `docs/agent/01-架构图.md`（可靠性执行链路 + 系统模块全景图）。
...
- **文档收口**：`开发链路-roadmap.md` → `并发优化ROADMAP.md`（单一权威，P0–P4 + ①–⑨ + 压测），新增 `并发压测结果.md`，删旧 `并发与性能优化.md`；`00-总览.md` 重整为纯架构参考（1059→418）、`02-决策环.md` 同步并发现状
...
- 新增 **`21-飞书接入指南.md`**（从零到跑通 + 频道面板原理 + 排错表）；`00-总览.md` Phase 4 补频道架构
