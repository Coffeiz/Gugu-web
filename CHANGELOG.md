...
- **登录支持用户名或邮箱**（`app/api/v1/auth.py`、`views/Login.vue`）：注册时已经收了邮箱（唯一约束），但登录一直只能用用户名，邮箱地址存了却没地方用。登录查询改成 `username`/`email` 任一字段匹配（两个字段都有唯一约束，不会互相撞车匹配到别人），前端登录页标签/提示语同步改成"用户名 / 邮箱"。密码找回、注册流程本身不受影响。
...
- **Intent Router 空闲时拦截"查到了吗"类追问 + 意图守卫漏判省略主语的承诺句**（`agent/router.py`、`agent/core.py`）：真实案例复盘——咕咕报错 F1 比赛数据后回复「马上重新查一下」，用户之后问「查到了吗」，咕咕只回了句「在的，你说～」，没有真的去查。两处根因分别在网关和主循环：① `router.py` 的 `_PROGRESS` 关键词集合字面量收了「查到了吗」，但短路逻辑不看忙闲、只要文字命中就回固定状态话术——`state` 已回 IDLE（Gugu 的"马上查一下"只是嘴上说说，从未触发真实工具调用、状态机不知道有个悬而未决的承诺）时，这条追问被拦成空话，压根没进主模型。改为只在真「忙」时才短路，空闲一律交主 Agent 看历史自己判断。② `core.py` 的意图守卫（B，防「说了要做却没动手」）用正则识别"我这就去查/建/改…"这类将来式宣告，命中却本轮零工具调用就强制模型当场真执行——但正则要求显式"我"开头，"马上重新查一下"是中文常见的主语省略（"我"隐含在上一句），且"马上"和"查"之间插了"重新"两个字，双重原因导致这句真实文本测不中，守卫没触发。放开"这就/马上/现在/稍后"可以不带"我"（前面不是"你"，避免误伤"你现在查一下"这种建议句），动词前也放开"重新/再"。用真实对话文本验证过两处都能正确命中/触发。
