...
- **一键部署**：默认 Compose 改为拉取包含前端、Nginx、Uvicorn、worker 和 IM gateway 的 standalone 镜像，PostgreSQL、Redis 与搜索服务仍独立运行。
- **部署配置**：清理不再使用的 Preview Compose 入口，保留 standalone、Dev 和 Prod 三条清晰路径；standalone 支持沙盒 egress 网络自定义。
- **运行时依赖**：刷新自有镜像基础依赖，并将 LoopScope 的 `better-sqlite3` 更新到兼容 Node 22+ 的 13.0.3。

### 修复

- **静态资源访问**：修复 standalone 镜像中 logo、字体等构建产物权限过严导致 Nginx 返回 403 的问题。

## [1.0.5] - 2026-09-04

### 修复

- **工具确认门**：重构为服务端授权——用户确认一次后，同能力范围内的后续操作在有效期内直接放行，不再出现「每条命令都重新确认」的循环；确认凭证不再经由模型中转，安全性更高。
...
- **控制台横幅**：DevTools 控制台的版本横幅改为单次打印，并跟随主题配色变化。
- **接口文档**：`/docs` 的 Swagger UI 资源版本固定，不再因上游更新出现图标错乱。
