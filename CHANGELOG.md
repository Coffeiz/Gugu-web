...
- **Shell 支持复合命令**：沙盒里的普通 Shell 现在可以跑 `&&`、`||`、`;`、管道与重定向，整条命令交给容器内 `sh -c` 解释，语义与终端一致；开启「直跑运行时」后不再误拦引号里的元字符。风险分类、确认门与路径校验边界保持不变。
- **知识库工具补全**：新增 `update_knowledge` 显式修正/更新已有条目（改名、纠错不再误存新条，也不会复活已删除条目）；`save_knowledge` 支持 keywords 检索别名，并在保存前引导先查重合并。

### 改进

- **文件同步更快更省**：同步改为按精确事件单点投影，内容指纹复用 stat 缓存（大小和修改时间没变就不重复哈希），全量补偿扫描从每分钟一次降为每天一次，只给最近 7 天活跃用户的绑定挂监听；列表刷新与空闲时的资源占用明显下降。
- **文件同步不再误报冲突**：咕咕重新生成同名文件、旧整根绑定与工作区分绑定重叠等场景不再被报成待处理冲突，路径也不会被冻结在对账之外；「取消冲突」点击后冲突会真正从列表消失。
- **工作区目录进入文件库**：修复工作区下由 Shell 写入的文件夹整树被拒的问题，`_tools/` 之类的产物现在会正常出现在文件库里，同名目录按工作区隔离。
- **沙盒读写权限自动补齐**：按需为工作区与个人/项目文件根补齐 rootless 身份 ACL（含目录默认 ACL 继承），修复沙盒读不到文件库文件、`edit_file` 改完文件库侧后沙盒侧写入失败的问题；环境不支持 setfacl 时仍退回权限兜底。
- **卡死会话可回收**：重启或崩溃遗留的生成状态会按心跳判活并回收，终止按钮改为直接取消本进程的生成任务（跨进程回退取消标记），会话长期停在「生成中」或基线整理中的情况明显减少，重启打断的整理也不会再永久挡住后续消息。
- **长对话上下文整理**：run 内溢出压缩统一走追加式路径，复用主会话的消息序列与缓存锚点，整理更快、缓存命中更稳。
- **矢量图预览**：SVG 打开即按视口适配（不再以 100% 小图出现），浮窗开窗比例按内容纵横比计算，宽幅 SVG 不再开成竖长窗口、也不再只占屏幕中间一小块。
- **确认门只确认一次**：确认破坏性操作后由服务端按原参数直接完成，不再让模型重新调用一次工具——回复里不会再出现「用户已确认，重新调用…」，工具卡片从「等待回复」直接转为完成，也不会重复弹出确认。
- **上下文整理可见**：run 结束后的基线整理阶段会显示「正在整理上下文」，期间发送的消息明确提示排队；手动执行 `/compact` 时同样显示整理状态而不是无意义的转圈；刷新页面或切回会话也能看到该状态，不再像卡死。
- **暗色模式玻璃 hover 恢复**：普通玻璃面板在暗色下恢复 hover 高亮（幅度按暗色标定），顶栏、日历、文件工具栏的仿玻璃 hover 带淡入淡出；终端页与技能页统一为同一套标准玻璃面板样式（面板本身不带 hover）。
- **工作区文件重命名**：`rename_file` 支持 Workspace 空间的目标文件夹（此前 workspace 目录下改名会误报「目标文件夹不存在」）。
- **Admin 面板**：用量场景统计支持「今日 / 最近 7 天」时段切换；文件同步绑定列表默认只显示有异常或冲突的绑定并显示计数，可一键切回全量列表。
- **技能列表改为卡片**：技能页换成与定时任务同款的悬浮卡片（投影、悬浮高亮、右上角启停开关），停用的技能整卡变淡；原有双列瀑布排布保留。
- **卡片页脚按钮 hover 统一**：定时任务的「试运行/编辑/删除」和技能卡的「编辑/删除」现在共用同一套 hover 表现——文字变亮并浮出一层药丸底色，删除键取红色底；此前两边各写一套，悬停效果不一致。
- **无语言代码块自动高亮**：代码围栏没写语言名时自动探测语言并高亮，不再一律按纯文本显示。

### 修复

- **取消确认后没有下文**：在确认卡上点「取消」，工具气泡会立即显示「已取消」而不是永远停在「等待回复」（刷新后同样正确），收尾回复也作为底部的新消息出现，不再并进工具调用前那条气泡。
- **取消后多余的自我解释**：修复取消收尾被误判为「模型续轮中断」而额外发起一次模型请求的问题，回复不再多出一条解释性内容。
- **确认结果的落库**：确认门重投的真实执行结果现在会写入会话历史，此前刷新或重放可能读到「等待确认」占位。
- **模型转述内部机制**：工具回执不再携带「授权 N 分钟内有效」这类服务端机制说明，咕咕不会再把「系统判定」「没走确认门」这类话讲给用户听。
- **对话里 Ctrl+Z 误伤其它输入框**：聊天输入框历史栈为空时不再把按键放给浏览器文档级撤销，项目、文件库等其他输入框的编辑不会被一并回退；同时修复在文件库点空白处后聊天框「假失焦」导致页面撤销无响应。
- **edit_file 两处故障**：模型把行号传成 JSON 数字时不再被拒绝（string 字段数字归一化），沙盒创建的文件回写时属主变更失败不再中断写入。
- **BYOK 停用后配额接口报 500**：停用 BYOK 凭据后 `/auth/quota` 因缺少 `cache_write` 键直接 500，已修复。
- **图片改后缀成文本**：把图片等非文本文件改成文本后缀（如 png 改成 .py）不再放行，避免产出坏文件；二进制拦截从 docx/pdf/xlsx 扩展到所有非文本源后缀。
- **技能深链刷新重弹**：从聊天里的技能链接打开编辑窗后，地址栏的 `skill=` 参数会被清理，刷新页面不再重新弹出该窗口。
- **Admin 部分文案显示成键名**：模型列表无匹配、感知异常项和配额编辑标题等处不再把 `llmExtraUi.noModelsMatch` 这类翻译键直接显示给用户（语言包分块被重复赋值导致三个语种同时丢键）。
- **聊天气泡引用卡片换行**：气泡里 @ 引用的文件卡片不再被挤成两行，图标固定尺寸、标签单行省略。
- **定时任务清除时间范围**：清除键的「×」不再偏高，悬停时也不再直接跳变；符号换成与其它关闭键同一套的居中图标，底与字色的变化带淡入。
- **技能卡悬停没有高亮**：技能卡的悬浮提亮此前只写在组件内，暗色主题下没有任何可感知变化；现在与定时任务卡共用主题层的同一条 hover 规则，两种主题下提亮表现完全一致。
- **点「停止」不再变成报错**：终止正在进行的生成时正确显示已取消状态，不再弹出「咕咕开小差了」的通用错误。
- **SVG 预览窗口跟随缩放**：拖动浏览器窗口大小后矢量图预览抽屉宽度自动重算；方形图标等无尺寸 SVG 开窗不再被拉成宽条。
- **聊天代码块贴错主题**：消息里的代码块此前只有一套浅色配色，暗色下注释发灰、部分内容甚至隐形；现在语法配色跟随主题切换，用户气泡里的代码块与咕咕气泡使用同一套面板样式，两种主题下都清晰可读。
- **工具参数畸形包装**：模型把数组或整次调用参数包成单键对象时不再执行失败，归一化兜底扩展到两种新的畸形形态。
- **终端残留与输出膨胀**：删除会话后遗留的咕咕终端会被保留策略自动清理（删除会话也会同步关闭其终端），终端输出超过 50 万字符后自动滚动丢弃最旧内容，不再无限占用内存。

## [1.1.2] - 2026-09-08

### 新功能

- **邮件附件**：交互式邮件和定时邮件支持附加文件库文件，也能附带本轮生成的文件；服务端校验文件归属、数量和大小，确认内容包含附件摘要。
- **文件工具整理**：文件工具按领域拆分，`read_file` 支持按行读取，新增跨个人、项目和工作区文件的 `grep` 搜索，文件发送与编辑职责更加明确。
- **用户级无限工具模式**：支持通过 `/unlimited` 解除当前运行的普通工具调用轮次限制，同时保留停止、上下文预算和服务超时保护。

### 改进

- **Docker 一键部署**：未设置 `SECRET_KEY` 时首次启动自动生成并持久化，`/data` 与 `/config` 自动创建或使用 Docker 卷；发布稳定版本时自动同步快速部署文档到 Docker Hub Overview。
- **IM 慢工具进度声明**：保留为可选能力，默认关闭，避免与每轮 draft 重复；需要时可在 Admin 中手动开启。
- **系统能力与用户技能**：系统工具能力按当前运行条件统一注入，用户技能继续按需检索和管理，不再依赖额外 Profile 门控；行为规则不向模型暴露模式标签。
- **确认门交互**：Web、QQ 和其他 IM 在等待工具确认时明确显示“任务已暂停”，确认成功后由服务端恢复原任务并返回真实结果，避免前端提前显示已完成或重复确认。
- **上下文与 IM 稳定性**：收敛压缩、会话基线和批处理时序，改善 IM 多消息、过期交互和文件投递的状态一致性。
- **Workspace 文件落点**：未显式绑定工作区的文件操作统一落到默认 Workspace，路径解析、文件夹移动和 Agent 可见声明保持一致。
- **RAG 与 Knowledge 索引**：来源文档改由索引更新事件构建，召回阶段复用持久化索引；文件读取增加并发上限、Conversation 改为批量查询，并补充文档数量与缓存未命中原因诊断。
- **RAG 召回质量**：召回评分升级 confidence-v4（非线性 IDF 词法归一 × 来源质量 × query match，弱相关召回收紧），Knowledge 语义向量接入混合融合，知识库里语义相关的内容更容易被召回入选。
- **QQ/IM 出站稳定性**：优化多消息发送、群文件过期降级和过期交互提示投递位置，减少消息顺序错乱和提示丢失。
- **文件同步**：存储对账清理已失效的历史冲突，保留仍需要人工决策的单边变化，避免状态页被旧冲突污染。
- **Shell 直跑运行时开关**：Admin 新增 `shell_direct_runtime_enabled`，开启后沙盒内普通 Shell 可直接执行 python/node 解释器命令（默认关闭）；危险命令确认门与 run_script 授权边界不变。

### 修复

- **邮件附件校验**：附件大小校验改为使用实际文件内容大小，避免元数据与实际大小不一致时错误放行或拒绝。
- **反思与会话基线**：修复会话基线竞态和反思批处理边界，避免连续消息期间重复压缩或遗漏待处理内容。
- **运行状态回收**：worker 退出后回收遗留的会话执行状态，避免旧 run 持续显示为生成中并阻塞后续消息。
- **撤销/重做并发**：撤销与重做应用操作改用行级锁认领，串行化状态转换，并发场景同一操作不再重复执行。
- **RAG 权限复核**：统一查询二次权限复核的可见范围过滤改为整条候选一起判断，拒绝候选后不再错位取行。
- **文件工具工作区权限**：文件夹删除/重命名等操作的空间判断漏掉 workspace 导致误判无权限；`move_items` / `copy_file` 显式支持以当前绑定工作区为目标，全家工具补齐 workspace 空间校验。
- **create_file 落点与 SVG**：显式 `space=workspace` 与绑定语义对齐（此前 schema 枚举拒收业务面已支持的工作区）；咕咕创建的 svg 正确标记 `image/svg+xml` 并回填存量，缩略图与预览恢复正常。
- **工具名容错全局化**：模型把 XML 片段拼进工具名时，截断前缀匹配兜底从单点提升到主循环全局生效，事件记录、熔断计数与历史回放一并救活。
- **图片预览平移**：放大后的平移钳制改为随缩放的溢出量并放宽余量，修复放大后图片右侧内容拖不到的问题。
- **界面细节**：主色按钮 hover 恢复可感知的明暗变化并补齐玻璃系顶部 1px 高光（原 hover 透明度恰好抵消渐变变化，肉眼不可见）；统一玻璃主题卡片/按钮悬浮动画与过渡；文件重命名输入框泛光不再被裁切；亮色主题与文件夹图标视觉优化。
...
- **BYOK 用户 Embedding 凭据（PRD-SEC-2）**：用户可以在 BYOK 里配置自己的 Embedding 凭据，知识库向量能力与官方预设同构接入；凭据解析、run 级绑定与 Admin 侧逐用户重建向量全链路打通，Embedding 卡片支持测试连接与用户侧重建向量，provider 列表收窄到有 embedding 端点的厂商。
- **核实轮上限弹窗**：核实轮数触顶时弹出与目标模式同款的询问弹窗，用户点解除后原 run 直接续跑，不再静默卡住。
- **顶层 Workspace 文件库空间**：文件库新增与个人文件、项目文件平级的 Workspace 目录，可创建、重命名、删除并从网页直接进入浏览；目录同步生成 Agent 可见的工作区声明，Shell 会话与定时任务可绑定 Workspace。物理目录采用不可变的 `workspace-<id>` 命名，重命名不影响已存文件；并修复与系统目录重名可导致误删、覆盖校验漏比较 Workspace 等数据安全问题。
- **Shell 沙盒工具链**：Shell 沙盒镜像内置 git、Node.js、Python3、ffmpeg 与中文字体，支持在工作区内直接 clone 与版本管理；镜像配方（`backend/Dockerfile.sandbox`）入库，配套构建、trivy 预扫与固定 digest 的更新流程。沙盒支持 workspace 路径与宿主机绝对路径的受限放行，并收紧了脚本执行、备份与权限边界。
- **Skill 全局搜索**：新增 Skill 全局搜索与按需管理工具，Agent 可以按需检索并调用技能。
- **长期记忆整理**：新增长期记忆整理调度，自动整理与压缩历史记忆；修复记忆提取、维护预算与并发初始化问题。
- **RAG 主动召回增强**：完善 Knowledge 向量缓存与主动召回，新增基于真实数据的 RAG 诊断脚本；新增搜索事实时效核验规则。
- **定时任务增强**：定时任务支持执行时间窗口与 Workspace 绑定，修复网页通知投递、咕咕编辑任务后的实时刷新等问题。
- **跨 Provider 推理状态**：推理状态跨 Provider 持久化，切换模型不再丢失运行状态。
- **IM 引用文件回读**：历史附件引用扩展到非图片文件，引用条以文件 chip 渲染，咕咕可回读被引用的文件内容。

### 改进

- **待办工具收敛**：add_todo / remove_todo / update_todo 收敛为 `update_stage` 统一入口——只传 stage 即切换当前阶段，传 add/todos 即批量增、删、改、移、勾待办（一次可整阶段勾完）。
- **用户消息 Markdown**：用户发出的消息气泡支持 Markdown 渲染（粗体、行内代码、列表、代码块等），输入框保持字面文本不自动转换；粘贴富文本嵌套内容不再丢失。
- **输入草稿**：聊天输入框草稿按会话保留并本地持久化，切换会话、刷新或重开浏览器后都能接续。
- **工具参数自纠**：模型按旧参数形状多传的字段不再被静默忽略，成功回执会带上被忽略字段清单并提示查询最新 schema，一轮自纠。
- **长内容排版规范**：人格提示词补充条件式排版规范，长回复自动分节并用分割线隔开，短对话不上格式。
- **BYOK 草稿直测**：编辑配置时可直接用草稿内容「测试」与获取模型列表，不必先保存；切换 Provider 或 Endpoint 后会提示重新填写 Key。
- **Admin 模型下拉**：模型选择弹层对齐 BYOK 体验——支持预填过滤、弹出动效、点外立即关闭。
- **Agent 生命周期**：重构 Agent 生命周期与工具确认边界，统一能力目录、交互通知链路与 Shell 权限策略；授权状态与命令交互补齐多语言。
- **Agent 执行稳定性**：主循环新增连续相同工具调用熔断（按 repeat_safe 白名单累积），防止核实阶段同参重读死循环；Session Snapshot 命中改为滑动 TTL，长时间连续对话不再每 30 分钟强制重建上下文。
- **工具调用容错**：自动归一化模型侧「数组拍平」参数形态，脚本工具补齐超时与输出上限的单位说明，减少参数校验往返失败。
- **文件同步与存储对账**：完善文件同步边界、快照与存储对账流程，统一对账操作按钮组件；修复同步竞态与权限边界。
- **IM 流式交互**：IM 会话运行镜像接入 Web genstream，改善流式回复体验。
- **用量统计**：Admin 用量统计口径统一并排除 BYOK 流量，避免混算。
- **LoopScope**：统一指标格式、增强系统提示词展示，修复换行内容与追踪输出。
- **部署**：更新 Docker 构建与发布流水线，Compose 补充数据目录自动创建与迁移说明；修复数据库会话清理 idle in transaction 连接。

### 修复

- **分割线可见度**：Markdown 分割线与标题下划线在亮/暗色下重新定档，从正文色派生，清晰可辨且不喧宾夺主。
- **BYOK Embedding**：修复凭据解析漏 await 导致无凭据用户每轮误报解密/绑定失败的问题。
- **项目卡 hover 高光**：恢复亮色渐变与淡入淡出过渡，暗色模式同样有克制的微光。
- **弹层滚动**：修复弹出菜单内容超长时撑出屏幕且无法滚动的问题。
- **BYOK 界面**：模型名下降字母（g/y/p）不再被卡片裁掉；编辑 Embedding 配置不再把已存向量维度悄悄清零。
- **Admin 预设卡**：补 glm 厂商圆点颜色。
- **登录输入框**：修复认证页输入框聚焦泛光瞬间出现、没有淡入过渡的问题。
- **图片预览**：修复图片预览首帧缩放动画跳变的问题。
- **视觉文件判定**：`vision_ready` 按本轮实际运行模型判定，不再误判。
- **会话与历史**：修复历史对话会话 ID 映射、并行工具确认的历史边界、Anthropic 历史归一化边界与回复出口误触发身份兜底。
- **文件粘贴**：修复工作区内粘贴文件掉回个人根目录的问题。
- **界面细节**：清空回收站按钮 hover 保持危险色、暗色模式 IM 分组悬停反馈、新建文件夹输入交互、聊天侧栏高度回归等修复。

### 安全

- **BYOK 凭据防跨服务商泄漏**：测试连接、获取模型列表与能力检测不再把已存 Key 带往其他 Provider 或 Endpoint——切换 Provider 或 Base URL（含清空后落到官方默认端点）都必须重新填写 Key；保存已有凭据同样强制目的地绑定，目的地变更而未重输 Key 一律拒绝且不留任何改动；运行时解析（Embedding / 语音识别 / 主模型）也不再从平台配置继承 Base URL 拼用户 Key，目的地不明的凭据宁可回落平台配置；无鉴权的自托管 Embedding（本地 Ollama / llama.cpp 等）允许空 Key 保存与测试。
...
- **用量统计**：统一 Web、IM、定时任务和反思流程的 LLM 用量记账口径，缓存命中与 BYOK 用量沿用同一套统计边界。
- **交互一致性**：统一弹层拖选关闭保护、画布项目分组箭头和项目模板多语言文案，改善暗色模式下 IM 分组悬停反馈。
...
- **提醒与 Agent 边界**：修复定时提醒查询越过用户服务边界及重复提醒问题，补齐 Agent、IM 流式交互和反思流程的运行边界。
- **RAG 与镜像构建**：修复 RAG 运行时依赖、索引 timing 覆盖和默认镜像扫描误报，确保发布镜像可正常运行。

## [1.0.5] - 2026-09-04

### 修复

- **工具确认门**：重构为服务端授权——用户确认一次后，同能力范围内的后续操作在有效期内直接放行，不再出现「每条命令都重新确认」的循环；确认凭证不再经由模型中转，且阻断模型伪造确认、修复终端确认后的执行死路，并将授权绑定到具体资源，安全性更高。
- **活动表单提醒下拉**：点击编辑弹窗卡片内的其他区域时，提前量下拉列表能正常关闭了。
- **BYOK 主密钥提示**：保存主密钥后提示立即消失，不再需要手动刷新页面。

### 改进

- **BYOK 部署**：主密钥文件写入持久卷，重启容器后不再丢失。
- **文件多选**：勾选图形增加淡入淡出与回弹动画，勾选/取消方向不同时过渡更自然。

## [1.0.4] - 2026-09-03

### 修复

- **日历提醒表单**：提前量选择改用统一的列表弹窗与开合箭头，文字居中显示；备注输入框随内容自动增高（有上限）；点击弹窗内的选项不再连带关闭整个浮层。
- **对象卡跳转**：在定时任务/便签/画布页点击聊天里的对象卡片，已在当前页时也能正确打开对应内容；关闭弹窗后刷新页面不再重复弹出。
- **IM 与问候语**：修复飞书语音入口调用错误导致无法处理语音的问题，并在问候语配置或生成失败时正确返回前端兜底内容。
- **暗色模式 hover**：日历项目条/日程块在暗色模式下悬停有反馈了。
- **按钮 hover 动画**：主色按钮悬停颜色过渡恢复平滑淡入淡出，不再瞬间跳变。

### 改进

- **工具调用兼容**：模型把单值参数包成 `{"item": …}` 时（如发送渠道只选一个）也能正确解析，不再调用失败。
- **箭头方向统一**：全站收起/展开箭头统一为「收起朝右、展开朝下」的默认方向，由公共组件保证，不会再出现个别位置方向相反。
- **控制台横幅**：DevTools 控制台的版本横幅改为单次打印、跟随主题配色变化，并与实际运行的 Runtime 版本保持一致。
- **接口文档**：`/docs` 的 Swagger UI 资源版本固定，不再因上游更新出现图标错乱；反代到子路径时 Authorize 流程也能正常工作。
- **定时任务与模型路由**：定时任务及后台 LLM 调用支持用户 BYOK 配置，任务完成汇报会根据任务复杂度说明实际完成事项。

## [1.0.3] - 2026-09-03

### 修复

- **BYOK 用量统计**：修复用户自带 API Key（BYOK）聊天后「今日/本月 token」恒为 0 的问题——BYOK 标记在落库环节丢失，导致用量被误记成平台用量；同时 BYOK 流量不再错误消耗平台 6h/周配额。
- **BYOK 缓存命中率**：修复命中率可能超过 100% 的问题——分母漏加了缓存命中的输入 token，现在按「命中 /（新增输入 + 命中）」正确计算。
- **BYOK 用量口径**：用户面板「今日/本月 Token」改为展示总消耗量（新增输入 + 缓存命中 + 输出），不再只显示计费部分；缓存命中率标签明确为本月统计。
- **BYOK 用量趋势图**：个人面板 Token 区块改为近 30 天趋势曲线，悬停显示当日总共/输入（含命中缓存）/输出，右上角汇总近 30 天总量（K/M 缩略）、日均、今日与缓存命中率。
- **长期记忆压缩**：压缩生成不再限制输出 token 预算，长对话压缩结果更完整，不再因预算截断导致记忆条目缺失。
...
- **提示词缓存**：修复多轮对话缓存前缀被历史回放形状变化打断的问题（上一轮注入的检索知识/上下文在下一轮被合并进用户消息），缓存命中从几乎全失恢复到正常，响应更快、token 消耗更低；Anthropic 缓存写入 token 现已计入总用量与趋势图，建缓存当天不再少算。
- **模型工具调用**：修复部分模型（如 MiniMax）把数组参数包成 `{"item":[…]}` 导致笔记更新等工具连续调用失败的问题，入参归一化自动解包。
- **项目弹窗实时刷新**：项目编辑弹窗打开时，咕咕在聊天/IM 里修改的阶段、待办、名称等会实时同步进弹窗（自己正在编辑的字段不会被覆盖）；修复点击右侧文件区空白退不出阶段/待办重命名的问题。
- **文件库**：修复咕咕操作回收站后网页不实时刷新的问题；修复文件重命名时点击空白处不退出的问题。
- **文本/代码编辑器**：txt 文件并入 CodeMirror 路径，获得行号、撤销栈与自动保存；修复暗色模式下选中文字不可见、光标隐形、行号槽白底的问题；语法配色、折叠箭头与占位符、光标/选区样式全部改为随主题令牌适配；Markdown 编辑器与代码编辑器共用同一套样式。
- **多选交互**：修复退出多选时勾选框结尾闪烁的问题；退出多选的勾选框与选中遮罩过渡收紧为 0.15s，与进入多选保持一致。
- **界面细节**：修复上传文件弹窗暗色模式失去渐变背景的问题；项目弹窗阶段区收展按钮改为实体按钮；画布卡片悬浮操作按钮与文件库统一为同一套样式。

## [1.0.2] - 2026-09-03

### 修复

- **弹窗动画**：弹窗打开不再先闪出现实心底色块，改为整个弹窗（底色与内容）一体淡入；关闭淡出动画保持不变。
- **模型请求**：彻底下线 temperature 参数，修复 Anthropic SDK 升级后每次对话请求报错、用户收到「咕咕开小差」的问题；后台 LLM 预设不再提供 temperature 配置项。
- **文档预览**：生产镜像默认安装 LibreOffice，修复 Docker 部署下 Office 文档预览失败的问题。
- **终端沙盒**：修复 Docker 部署下终端请求被 sandboxd 拒绝（「请求进程无效」）的问题。
- **全局搜索**：修复搜索面板滚动条超出容器、遮挡毛玻璃圆角边框的问题。
- **Admin 界面**：修复 LLM 预设编辑器界面文案缺失（直接显示翻译键名）的问题，Token 上限与上下文预算输入框改为并排布局。

## [1.0.1] - 2026-09-02

### 改进

- **镜像发布**：正式版本同时发布到 GHCR 和 Docker Hub，并补充双仓库部署说明。
- **生产部署**：补充 Rootless Docker 沙盒运行配置、前后端反代与独立 404 页面；Docker 部署的 Web、Worker 和网关日志可在服务器侧持久化查看。
- **Admin 维护工具**：记忆维护支持分批执行和按作用域预览，降低大批量刷新对服务和用户数据的影响。
- **配置模板**：拆分 Compose 编排配置与后端应用配置模板，管理员、模型和存储配置的位置更加明确。

### 修复

- **画布交互**：修复连接点被便签边框缩放热区遮挡，无法开始关系拖拽的问题。
- **邮件编辑**：修复邮件发布页输入框的焦点和调整尺寸样式不一致问题。
- **模型兼容**：修复 Anthropic 兼容接口与 MiniMax 参数传递不匹配导致的请求失败。
- **IM 记忆预览**：修复记忆预览对任务类型作用域识别不完整的问题。
- **生产镜像安全**：升级生产镜像中的 Python 依赖和 Debian 安全包，修复发布扫描发现的已知漏洞。

## [1.0.0] - 2026-09-02

### 新增

- **追加式事件记忆**：长期记忆支持一次记录多个独立事件，按章节追加并保留时间、经过、感受、结果和未解决事项；RAG 可按事件章节检索，避免持续重写整份记忆。
- **邮件订阅与受控 HTML**：新增用户邮件订阅状态和后台管理能力；邮件工具支持受严格白名单约束的 HTML 内容，并保留纯文本降级。
...
- **聊天窗口布局**：统一聊天大窗、小窗、顶部栏和底部输入区的尺寸与间距，减少不同窗口模式下的高度跳变。
- **前端交互与国际化**：统一刷新、反馈、确认和上传控件的公共样式；补齐管理页及开发工具页面的多语言文案，改善日文按钮布局和缺失翻译提示。
- **画布与文件交互**：统一 Interaction Runtime 的拖拽、同步和跨标签页刷新行为，优化便签编辑、文件卡片和画布关系操作的反馈。
...
- **交互同步与 Runtime**：收敛项目、文件、画布等页面的乐观更新与服务端最终收敛逻辑，由 InteractionSync 统一协调本地 mutation、服务端响应和跨标签页事件；补充 client-id 回声抑制，减少发起页重复刷新。
...
- **画布同步与拖拽**：修复画布来源切换、乐观认领、连接布局和拖拽取消过程中的状态竞态；修复 Safari 拖拽和框选时误选文字的问题。
- **邮件与认证页面**：修复邮件模板预览、发件订阅管理和密码找回页面的多语言缺失、通用按钮高度不一致及输入文字裁切问题。
- **主题样式**：恢复主题按钮渐变，修复统一控件收口后部分顶栏、上传入口和反馈控件的颜色与交互状态异常。
