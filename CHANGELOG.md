...
- **网关正则把「嗯/好」等确认吞掉 + 误判带话题的句子**（`agent/router.py`）：IM 网关原先用关键词把「嗯/好/谢谢」当闲聊短路回「嗯嗋～」、把「法拉利怎么这么慢」当成催咕咕，吃掉了用户的真实意图（如咕咕说「我去查」后用户回「好」被吞、搜索没接上）。**精简为只保留两件确定性短路**：斜杠命令（/stop /status /help）+ 咕咕**正忙时**的「是否在思考」进度追问（还在吗/查了吗/好了吗/进度 → 回状态、不打断）；其余一律交主模型。取消改走显式 `/stop`。
