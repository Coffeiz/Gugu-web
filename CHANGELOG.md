...
- **部署入口端口统一 8000**：一键部署/生产 Compose 的宿主侧默认端口从 9595 改为 8000，与镜像 EXPOSE、面板默认映射、`PUBLIC_APP_URL` 默认值对齐——部署后只有一个入口端口，不再出现「文档 9595、面板 8000」的分叉；已部署用户显式设置过 `GUGU_HTTP_PORT`/`GUGU_PUBLIC_APP_URL` 的不受影响。
- **一键部署镜像**：镜像声明数据库/Redis 等业务环境变量默认值，fnOS 等面板部署时可直接看到并填写，不再只露出 Python 自带变量；数据库等待超时改为明确报错并给出排查提示，不再静默继续后抛一长串连接栈。
