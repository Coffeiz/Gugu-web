...
- **新建项目 modal 日期预填**：日历页多选日期范围后点击顶栏「新建项目」，开始/截止日期自动填入选区；选区通过 `uiStore.calendarActiveRange` 实时同步，非日历页无影响
- **全局标题编辑框样式**：新增 `.title-edit-input` 全局 CSS 类（`global.css`），统一弹窗/卡片标题内联编辑框的底色（`rgba(255,255,255,0.72)`）、边框、圆角、阴影，与其他填写框一致；新建项目与项目编辑卡名称输入框均使用此类

### 调整

- **新建项目 modal 顶部**：名称输入区高度固定 52px（与项目编辑卡一致），切换显示/编辑态不再引起高度跳变；输入框字体与显示态统一（17px 700）
- **项目编辑卡填写框底色统一**：阶段重命名框、待办输入框聚焦态底色统一为 `rgba(255,255,255,0.72)`
- **`saveTodos` 走 `_patchProject`**：修复 `saveTodos` 直接调 `projectsApi.update` 不携带 `version`，导致后续操作触发 409 的问题；现在通过 `projectStore.updateProject` 走版本管理链路

### 修复

- **`ProjectModal.vue` 缺少 `projectsApi` 导入**：`saveTodos` 调用 `projectsApi.update` 但未导入，运行时报 `ReferenceError`；已补入 import
- **阶段切换待办不实时更新**：改为直接在 `setStage` 同步计算并替换整个 `localStages.value`，不再依赖 store 异步回写，UI 立即响应
- **退回阶段目标阶段本身待办未还原**：还原循环从 `newIdx+1` 修正为 `newIdx`，目标阶段自身的自动打勾待办也会被正确还原
...
- **日历 / 看板排序规则统一**：格子内项目优先级降序 → 开始日期升序 → 截止日期升序 → 创建时间兜底；已完成项目优先收入「更多」溢出
- **总览项目列表排序**：进行中 > 待开始 > 已完成，组内按优先级→开始日→截止日→创建时间排序
- **看板列排序加入优先级**：进行中/待开始列组内先按优先级降序，再按原有截止日期/开始日期规则

### 修复

- **`api.js` 变量名冲突**：`err` 重复声明导致构建失败，改为 `apiErr`
- **后端 `_to_resp` 缺失字段**：项目响应补入 `priority`、`version`；活动响应补入 `version`，前端乐观锁版本号现可正确同步
