...
- **思维面板「记录」上线**（`app/api/v1/mind.py`、`app/core/mind.py`、`views/Mind/`）：新增思维模块的记录页——按「发生时间」排的时间流便签，支持补录到过去的日期（补录后提示「已记到 X 月 X 日」）。页面是全站首个 full-bleed「工作台」视图：无顶栏，胶囊条切换记录/画布并自带便签筛选；便签流按日期分组、双列纸片卡（时间戳常驻、卡上直接勾待办、长内容折叠展开、可选低饱和底色），右侧窄日期索引随滚动联动、点击跳转；底部常驻「记点什么…」捕捉条，展开出格式工具栏。便签用窄口径所见即所得编辑器（正文/单级标题/待办/列表，无 Markdown 语法触发、格式只走工具栏），输入 `@` 可引用项目/文件/日历活动，存的是 `type:id` 稳定锚点，业务对象改名也不会指错；点已有便签就地展开编辑。便签正文进了全局搜索（顶栏下拉与咕咕 `global_search` 都能按正文召回，其余类型仍只搜名称/标题）。底层是新的三层数据模型（全局节点 / 画布视图 / 关系边），画布与咕咕协作在后续阶段。
