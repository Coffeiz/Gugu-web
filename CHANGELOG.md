...
- **默认 Compose 自动启用沙盒**：`docker compose up -d` 会启动独立 sandboxd，拉取沙盒执行镜像并校验其 OCI 签名与官方发布身份后固定 digest；一体化应用镜像不再内嵌沙盒镜像，单容器模式默认关闭沙盒。
- **反思触发阈值统一**：网页与所有私聊共用可配置阈值；群级、群友及群内 Owner 反思统一按 50 条群消息触发，未满阈值时 3 分钟空闲收束。
- **反思闲置收束加快**：owner、群主和群/群友反思在最后活动后 3 分钟触发，减少等待超过上游缓存窗口后才执行的情况。
- **弹窗关闭按钮统一**：统一不同弹窗和卡片的关闭按钮外观与圆角定位，项目待办列表使用更紧凑的按钮尺寸。
- **文件大小显示更简洁**：文件卡片和列表按大小自动显示 B、KB、MB 等单位，避免裸字节数过长。
- **Responses 分支任务兼容**：上下文压缩等分支请求使用所选模型的 Responses 协议，并保留独立指令和工具配置。

### 修复

- 修复 Admin 将 backend 容器的 Docker 状态误当作 Shell Rootless 执行器状态的问题。
- 修复停止 run 后界面仍显示进行中的流状态竞态。
- 修复中止 run 后执行历史未完整持久化的问题。
- 修复文件预览中任务列表的行内代码被挤窄换行，正文排版恢复连续。
- 修复 Responses 模型执行上下文压缩分支时协议和 system instructions 不匹配的问题。
