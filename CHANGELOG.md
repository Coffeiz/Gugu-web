...
- **项目支持查看/取消归档**（`app/api/v1/projects.py`、`views/Projects/index.vue` + 新增 `ArchivedProjectsModal.vue`）：`archive_project` 工具此前只有 agent 侧能力，网页端毫无入口——归档后项目从看板消失即无处可寻，也无法撤销。`GET /projects` 补 `archived` 查询参数（默认只看未归档，看板不再混入已归档项目）；项目页新增「已归档」入口，弹窗列出已归档项目并可一键「取消归档」。
