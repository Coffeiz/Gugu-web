...
- **思维面板「记录」上线**（`app/api/v1/mind.py`、`app/core/mind.py`、`views/Mind/`）：新增思维模块的记录页——按「发生时间」排的时间流便签，支持补录到过去的日期；便签用窄口径所见即所得编辑器（只有正文/标题/待办/对象引用四种块），输入 `[[` 可引用项目/文件/日历活动，存的是 `type:id` 稳定锚点，业务对象改名也不会指错。便签正文进了全局搜索（顶栏下拉与咕咕 `global_search` 都能按正文召回，其余类型仍只搜名称/标题）。底层是新的三层数据模型（全局节点 / 画布视图 / 关系边），画布与咕咕协作在后续阶段。
