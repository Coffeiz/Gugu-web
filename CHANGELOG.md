...
- **一次性定时任务过期不自动清理**（`app/scheduled_tasks.py`）：正常触发的 `@once` 任务由 `execute_task` 即时删除，但**停用 / misfire 没触发 / 残留**的过期一次性任务无人回收，会一直僵在面板里（`execute_task` 对停用任务直接 early-return，走不到删除）。`reconcile`（每 ~30s）新增 GC：**过点的一次性任务自动清理**——正常触发的由 `execute_task` 即时删，没触发到 / 残留的过点 **1 小时**后由 GC 扫掉（过期任务多留一会儿可见再清），周期 cron 与未到点的不受影响。
