...
- 卡片内联重命名沿用共享 ghost sizer：单输入模式由隐藏 ghost 的自然行盒决定高度，input 绝对定位覆盖其上；不要给共享单输入 sizer 强制 `1lh` 高度或 `appearance: none`。文件夹、文件名与后缀输入统一在 `.rename-sizer .rename-input-inline` 使用适配紧凑内容盒的行高；分段布局只增加容器规则，禁止只给文件输入另设行高。窄控件需实测 `g`、`p`、`q` 等下伸字形及基线，避免公共正文行高超出控件内容盒而裁切。
