...
扫码创建），gateway 从 DB 读启用列表，每个起一条网关子进程，凭据走**环境变量注入**
...
运行（从 backend/ 跑加载 .env）：
    .venv/bin/python -m agent.gateway.gateway
"""
...
def _desired() -> dict[str, dict]:
    ...
    try:
        ...
        print(f"[gateway] 读取 user_bots 失败（保持现状）: {type(e).__name__}: {e}", flush=True)
...
def _spawn(key: str, spec: dict) -> subprocess.Popen:
    print(f"[gateway] ▶ 启动 {key}", flush=True)
...
def _kill(key: str, proc: subprocess.Popen) -> None:
    print(f"[gateway] ■ 停止 {key}", flush=True)
    proc.terminate()
    try:
        proc.wait(timeout=5)
    ...
    for key, spec in desired.items():
        ...
        if key in _next_retry_at:
            ...
            if lived < FAST_CRASH_SEC:
                ...
                print(f"[gateway] {key} 启动后 {lived:.1f}s 内退出（第 {_fail_count[key]} 次连续秒崩，"
...
def main() -> None:
    ...
    print(f"[gateway] 频道管家启动（每 {POLL_SEC}s 同步一次配置）", flush=True)
    while not _stop:
        try:
            reconcile()
        except Exception as e:
            print(f"[gateway] reconcile 出错: {type(e).__name__}: {e}", flush=True)
        ...
        health.beat_sync("gateway", {"gateways": gateways, "count": len(gateways)})
