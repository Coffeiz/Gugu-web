...
- 两个卡片的矩形边缘之间必须至少保持 `100px` 安全距离。
- 如果采用中心点排布，则两个卡片中心必须至少相隔 `600px`；不要把中心点距离规则误当成卡片左上角距离。
- 两个规则满足其一即可，但优先使用边缘 `100px` 规则；除非用户明确要求紧凑排列，不得重叠或贴边。
...
- “靠近某节点”使用 `near_node`，在目标节点右侧留出至少 100px。
...
`x = 文件 right + 100`，并保持合适的 `y`；如果用户没有指定高度方向，不要覆盖文件。
