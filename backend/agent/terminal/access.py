...
async def pty_access(db: AsyncSession, user_id) -> TerminalAccessDecision:
    ...
    if not getattr(get_settings().sandbox, "full_user_sandbox_authorization_enabled", True):
        return TerminalAccessDecision(False, "完整用户沙箱授权已关闭，交互式 PTY 不可用", TerminalOperation.INPUT)
