...
def _out(b: UserBot) -> dict:
    ...
    return {
        ...
        "group_allowed_tools": b.group_allowed_tools or ["web_search", "image_search"],
...
async def update_my_bot(
    ...
    if body.group_allowed_tools is not None:
        unsupported = set(body.group_allowed_tools) - {"web_search", "image_search", "group_context_search"}
        if unsupported:
            raise HTTPException(400, "当前群成员只支持网页搜索、图片搜索和当前群上下文搜索")
