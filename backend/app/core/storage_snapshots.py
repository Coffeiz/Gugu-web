...
async def _sql_snapshots_locked() -> None:
    """能直接用 DB 汇总列算的几类——不碰存储层，一条 SQL 秒出结果：
    聊天附件按 `draft`/`attached` 分开统计（能直接反映草稿清理任务是否在
    正常工作：如果草稿占用只涨不跌，大概率是清理任务挂了），加上用户文件库
    总量。"""
    ...
    async with db_session._SessionLocal() as db:
        for state, category in (("draft", CATEGORY_CHAT_DRAFT), ("attached", CATEGORY_CHAT_ATTACHED)):
            ...
            await record_snapshot(category, int(row[0] or 0), int(row[1] or 0))

        row = (await db.execute(
            select(func.count(File.id), func.coalesce(func.sum(File.size_bytes), 0))
        )).one()
        await record_snapshot(CATEGORY_USER_FILES, int(row[0] or 0), int(row[1] or 0))


async def record_sql_snapshots() -> None:
    """定时任务入口：Redis 锁防止 backend/worker 并发触发同一次统计。"""
    lock = R.get_redis().lock(_LOCK_KEY, timeout=_LOCK_TIMEOUT, blocking=False)
    if not await lock.acquire(blocking=False):
        return
    try:
        await _sql_snapshots_locked()
    finally:
        try:
            await lock.release()
        except Exception:
