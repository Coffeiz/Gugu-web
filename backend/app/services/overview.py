...
async def upcoming_rows(db, user_id, today, until):
    projects = (await db.execute(select(Project).where(
        Project.user_id == user_id, Project.deleted_at.is_(None), Project.archived.is_(False), Project.status != "done",
    ...
    events = (await db.execute(select(CalendarEvent).where(
        CalendarEvent.user_id == user_id, CalendarEvent.deleted_at.is_(None), CalendarEvent.date >= today, CalendarEvent.date <= until,
    ))).scalars().all()
...
async def dashboard_counts(db, user_id, today):
    ...
    base = select(func.count(Project.id)).where(Project.user_id == user_id, Project.deleted_at.is_(None), Project.archived.is_(False))
    return {
        ...
        "upcoming_events": await count(select(func.count(CalendarEvent.id)).where(CalendarEvent.user_id == user_id, CalendarEvent.deleted_at.is_(None), CalendarEvent.date >= today)),
