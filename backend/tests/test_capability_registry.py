...
def test_builtin_phase1_metadata_is_complete_and_relations_are_registered():
    ...
    assert len(snapshot.tools) == 111  # 含 grep、工作区 CRUD、邮件、技能/知识管理、文件库压缩解压、链接按钮、当前时间及 MCP 管理工具
    assert len(snapshot.skills) == 12
    assert not snapshot.diagnostics
    assert "create_file" in snapshot.tools
    assert "grep" in snapshot.tools
...
async def test_admin_capability_catalog_exposes_metadata_without_schema_or_body():
    ...
    assert len(payload["tools"]) == 111  # inspect_images 已并入 read_file，另含 get_current_time
