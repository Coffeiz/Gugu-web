...
覆盖：
- read_pattern_list 的迁移链（pattern.json → 旧 facts.json → 更旧的 facts.md → 空）
...
# ── read_pattern_list 迁移链 ──────────────────────────────────────────────

async def test_read_pattern_list_prefers_pattern_json(storage):
    ...
    assert patterns[0]["text"] == "新文件里的"


async def test_read_pattern_list_migrates_legacy_facts_json(storage):
    from agent.memory import store
    await storage.put(f"{UID}/.agent/facts.json",
                       json.dumps([{"id": "b1", "text": "旧 facts.json 内容", "kind": "observed", "conf": 0.9, "imp": 3, "ts": 1.0}]).encode())
    patterns = await store.read_pattern_list(UID)
    assert len(patterns) == 1 and patterns[0]["text"] == "旧 facts.json 内容"
...
async def test_read_pattern_list_migrates_ancient_facts_md(storage):
    from agent.memory import store
    await storage.put(f"{UID}/.agent/facts.md", "- 用户喜欢猫\n- 用户住南京\n".encode())
    patterns = await store.read_pattern_list(UID)
    texts = {pattern["text"] for pattern in patterns}
    assert texts == {"用户喜欢猫", "用户住南京"}
    assert all(pattern["kind"] == "observed" for pattern in patterns)   # _migrate_md 判不了 kind，一律 observed


async def test_read_pattern_list_empty_when_nothing_exists(storage):
    from agent.memory import store
...
async def test_apply_profile_ops_add_and_dedupe(storage):
    from agent.memory import store
    # 相似判定阈值刻意保守（_pattern_similar：较短≥6字且是较长的子串，或 bigram Jaccard≥0.7），
...
async def test_apply_pattern_ops_add_observed_and_inferred(storage):
    from agent.memory import store
    out = store.apply_pattern_ops([], [
        {"text": "用户偏好先讨论再要清单", "kind": "observed", "importance": 4},
    ], [])
    assert len(out) == 1
    assert out[0]["kind"] == "observed"
    assert out[0]["conf"] == store._PATTERN_DEFAULT_CONF["observed"]


async def test_apply_pattern_ops_reinforce_upgrades_kind(storage):
    from agent.memory import store
    out = store.apply_pattern_ops([], [{"text": "用户可能喜欢简洁回复", "kind": "inferred", "importance": 3}], [])
    assert out[0]["kind"] == "inferred"
    conf_before = out[0]["conf"]
    # 用户亲述同一条 → 应该把 inferred 升级成 observed，且置信度上升
    out2 = store.apply_pattern_ops(out, [{"text": "用户可能喜欢简洁回复", "kind": "observed", "importance": 3}], [])
    assert len(out2) == 1
    assert out2[0]["kind"] == "observed"
    assert out2[0]["conf"] > conf_before


async def test_apply_pattern_ops_remove(storage):
    from agent.memory import store
    out = store.apply_pattern_ops([], [
        {"text": "用户决定项目推迟到 8/31", "kind": "observed", "importance": 3},
        {"text": "用户喜欢简洁回复", "kind": "observed", "importance": 4},
    ], [])
    out = store.apply_pattern_ops(out, [], ["用户决定项目推迟到 8/31"])
    assert len(out) == 1
...
async def test_review_patterns_majority_vote_keeps_only_consensus(storage, monkeypatch):
    ...
    await mem_store.write_pattern_list(UID, patterns)
    ...
    remaining = await mem_store.read_pattern_list(UID)
...
async def test_memory_compaction_uses_daily_batch_and_rag_only(storage, monkeypatch):
    ...
    await store.write_pattern_list(UID, [
        {"id": "f1", "text": "用户做决定前会先核实事实", "kind": "observed", "conf": 0.9, "imp": 4, "ts": 1.0},
    ])

    captured = {}

    async def fake_complete_json(sys_prompt, user, settings, max_tokens=1500, **kwargs):
        captured["user"] = user
