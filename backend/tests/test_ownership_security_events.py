...
from app.security.events import reset_request_context, security_fingerprint, set_request_context
...
async def test_ownership_event_fingerprints_request_source(db, user_a, user_b):
    file = File(user_id=user_b.id, display_name="private-note", ext="md", storage_key="private")
    db.add(file)
    await db.commit()
    await db.refresh(file)
    ...
    context_token = set_request_context({
        ...
        "metadata": {"source": "http", "client_type": "browser"},
    })
    try:
        assert await get_owned(db, File, file.id, user_a.id) is None
    finally:
    ...
    row = (await db.execute(select(SecurityEvent).where(SecurityEvent.user_id == user_a.id))).scalar_one()
    assert row.client_fingerprint == security_fingerprint("client-a")
    assert row.ip_fingerprint == security_fingerprint("192.0.2.10")
    assert row.user_agent_fingerprint == security_fingerprint("test-browser")
    assert row.metadata_json == {"source": "http", "client_type": "browser"}
