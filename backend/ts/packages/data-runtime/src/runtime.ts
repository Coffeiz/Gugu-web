...
function ragDocumentFromRow(row: Record<string, unknown>): RagDocument {
...
export class DataRuntime {
  ...
  async loadRagIndex(context: DataAccessContext): Promise<{
    ...
    for (const row of rows) {
      ...
      documents.push(ragDocumentFromRow(row));
    }
    ...
    const watermark = revisionBySource.size
      ? { ts: [...revisionBySource.values()].sort().at(-1) as string, id: 0 }
      : null;
    return { revision, snapshot: { documents }, watermark, probe };
  }

  /** 增量读：按 (indexed_at, id) 游标读取水位之后的变更行（含软删墓碑）。
   * 活行 → 文档（upsert）；墓碑行 → 文档键（delete）。同一游标可重复消费，
   * 应用侧幂等；游标推进到本批最后一行，无更多行时 cursor=null。 */
  async loadRagIndexDelta(
    context: DataAccessContext,
    cursor: RagCursor | null,
    limit = 2_000,
  ): Promise<RagIndexDelta> {
    const ownerId = assertOwnerScope(context);
    const operationStarted = performance.now();
    const probe = { stage_ms: {} as Record<string, number>, counts: {} as Record<string, number> };
    const cursorDate = cursor ? new Date(cursor.ts) : new Date(0);
    const cursorId = cursor ? cursor.id : 0;
    let stageStarted = performance.now();
    const rows = await this.query(() => this.sql`
      SELECT id, indexed_at, deleted_at, source_type, source_id, scope_type, scope_id,
             platform, bot_id, group_id, document_id, parent_document_id,
    ...
    `);
    probe.stage_ms.database_query = Math.max(0, Math.round(performance.now() - stageStarted));
    probe.counts.scanned_rows = rows.length;

    stageStarted = performance.now();
    const documents: RagDocument[] = [];
    const deleteIds: string[] = [];
    let last = null as { ts: string; id: number } | null;
    for (const row of rows) {
      const indexedAt = row.indexed_at instanceof Date
        ? row.indexed_at.toISOString()
        : String(row.indexed_at || "");
      last = { ts: indexedAt, id: Number(row.id || 0) };
      if (row.deleted_at !== null && row.deleted_at !== undefined) {
        const parentId = String(row.parent_document_id || row.document_id || "");
        deleteIds.push(`${String(row.source_type)}:${parentId}:${Number(row.chunk_index || 0)}`);
        continue;
      }
      documents.push(ragDocumentFromRow(row));
    }
    probe.stage_ms.delta_projection = Math.max(0, Math.round(performance.now() - stageStarted));
    probe.counts.documents = documents.length;
    probe.counts.deleted_keys = deleteIds.length;
    const hasMore = rows.length === limit;
    return { documents, deleteIds, cursor: last, hasMore, probe };
  }
