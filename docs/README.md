...
| [`架构图.md`](agent/架构图.md) | 两张架构全景图：可靠性执行链路、系统模块全景 |
| [`决策环.md`](agent/决策环.md) | 一轮「用户说话→咕咕回应」内部到底走了哪些步 |
| [`可靠性.md`](agent/可靠性.md) | 可靠性工程：怎么保证「说做了就真做了」 |
| [`多步执行与防停顿.md`](agent/多步执行与防停顿.md) | 回复循环终止策略，防止「说了要做却没动手」 |
| [`IM接入架构.md`](agent/IM接入架构.md) | 飞书 / QQ / 微信接入架构 |
| [`记忆系统.md`](agent/记忆系统.md) | 私有 `.agent/` 五层记忆档案 + 反思机制 |
| [`感知系统.md`](agent/感知系统.md)（+ 两张 svg） | 相处方式系统：感知 / 反思 / stance 行为模块 / 错读案例收集，体量最大的一篇 |
| [`精力系统.md`](agent/精力系统.md) | Token 配额（精力）系统：窗口计算、硬拦、封顶记账 |
| [`LLM支持与适配.md`](agent/LLM支持与适配.md) | 支持哪些模型厂商、怎么适配 |
| [`提示词优化指南.md`](agent/提示词优化指南.md) | prompt 怎么组织、渐进式披露（`use_skill`）机制 |
| [`飞书接入指南.md`](agent/飞书接入指南.md) | 飞书接入操作指南（BYO 扫码自连） |
