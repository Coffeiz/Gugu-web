...
> **部署形态：默认单机**（2026-06-24 决策）——三个服务 + 网关同机，一套配置管全部、Admin 配置/重启全生效。跨主机为可选路径（§10.3 / [`deploy.md`](deploy.md) §4.3）、Admin 推不到远端；扩量靠单机内手段。详见 [`开发链路-roadmap.md`](开发链路-roadmap.md) 部署形态决策。
