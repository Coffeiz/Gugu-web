...
- **群聊**（2026-08）：`user_bots` 有 `group_chat_enabled`（是否处理群消息）、`group_requires_at`（是否需要 @）和 `group_read_enabled`（是否只记录未 @ 普通消息）三列，**目前只有 QQ 网关真正接了**（每条群消息现查一次 DB，切了立即生效、不用重启进程）。开启普通消息读取后，未 @ 消息只写入按群共享的会话，后续 @ 咕咕时进入上下文，不调用模型、不回复；详细链路见 [`21-群聊消息架构.md`](21-群聊消息架构.md)。QQ 群消息用 `chat_id`(=group_openid) 与 `platform_user_id`(=群内发言人 member_openid) 区分「回复发去哪」和「这是谁」，回复走 `qq.send_group`（区别于私聊 `send_c2c`）；QQ 群聊暂不支持发图片/文件（官方 API 限制，兜底文字提示）。三平台总体接入见 [`20-IM接入架构.md`](20-IM接入架构.md) §3.5。
