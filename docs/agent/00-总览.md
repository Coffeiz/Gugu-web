...
- `behaviors/`（9 个行为模块）：**这轮该怎么相处**——常驻 `baseline`（姿态地图+中性默认）+ 8 个 stance 模块（execution/stuck-first/record/query/decision-explore/reflect/emotion-first/companion），由上轮反思判的 per-user stance 1:1 点亮（`behaviors.py`，30min 新鲜度闸）。详见 [`10-感知系统.md`](10-感知系统.md) §2.6。
