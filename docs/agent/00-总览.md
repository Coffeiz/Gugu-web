...
- **`embedding.py`（向量语义检索 · 可选加固，默认关）**：记忆检索的相关性打分从字 bigram 词法**升级到向量语义**（cosine，抓「会议」↔「周会」这类同义）。**独立 pin 的 embedding 模型**（与聊天解耦、走 OpenAI 兼容 `/embeddings`，支持自托管 Ollama）；facts 超注入上限（`FACTS_INJECT_MAX`=100）走向量挑、`memory.md` 超 1500 字切块向量挑、无向量则退回词法/整篇（**零回归**）。反思写 facts / compress 写 memory 时**增量自动嵌**（只嵌没向量的），换模型有重建按钮。per-user 几百条用暴力 cosine、不要向量库；向量与文本分开存、是可重建缓存。是 Person Model 检索基建的地基（第一个消费者），收益要真超阈值才兑现。**详见 [`11-记忆系统.md`](11-记忆系统.md) §11。**
