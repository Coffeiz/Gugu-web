...
- **自我核实闭环（`MAX_VERIFY = 5`）**：成功调用已识别的增删改工具后，工具结果一回灌就直接注入一轮「系统自检」——不再先等待模型生成一段没有新信息的“已完成”收尾。工具返回 `error` 不触发；自检轮若只查证没改动 → 结束，若补做了 → 再进入自检，直到"只查不改"或封顶 5 轮。`mind_get` / `mind_search` 也被认作有效读回。**不是固定跑 5 轮**：通过即停，只读任务零额外开销。两路（Anthropic/OpenAI）同构，轮预算 `MAX_ROUNDS + MAX_VERIFY*2` 不挤占任务轮。当前查询由模型选择，尚未做到按 mutation receipt 确定性核对同一对象与字段，详见 [`03-可靠性.md`](03-可靠性.md)。
