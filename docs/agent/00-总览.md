...
└── agent/
    ...
    ├── gateway/{base,web,qq,feishu,supervisor}.py
    ├── events/{bus,types}.py
...
#### `gateway/qq.py` / `feishu.py` / `supervisor.py`

QQ / 飞书 BYO 网关（botpy / lark-oapi WebSocket 长连）+ supervisor 网关管家。见「IM 接入」+ [`20-IM接入架构.md`](20-IM接入架构.md)。
...
- **动态网关 · 进程级**（`gateway/supervisor.py`）：lark/botpy 连接只有 `start()`、**无 `stop()`** → 一个 bot 一个子进程，kill 子进程=断开。supervisor 每 5s 查 `user_bots` reconcile（增/删/崩溃自愈）；**凭据走环境变量注入**（不走 argv，避免 `ps` 泄漏 secret）。
