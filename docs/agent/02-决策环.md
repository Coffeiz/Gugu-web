...
| 文件 | 负责 |
|------|------|
| `app/core/events.py` + `backend/ts/api/live.ts` + 前端 `stores/live.ts` | 改动型工具成功 → `publish(user_id, 资源)`；IM 一轮后 → canonical `sessions` payload → Redis pub/sub → TypeScript Live SSE → 前端消费统一事件 |
