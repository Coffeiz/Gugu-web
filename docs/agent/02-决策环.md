...
- ✅ **自我核实闭环（`MAX_VERIFY=5`）**：成功调用已识别的写工具后，工具结果一回灌就立刻注入「系统自检」，让模型用查询工具查证真生效/完整；工具返回 `error` 不触发。**通过即停（自检轮只查没改 → 结束），不是固定 5 轮**；补做了就再自检一轮，封顶 5；只读任务不触发。`mind_get` / `mind_search` 也算有效读回，避免笔记已读回仍重复复查。复查过程文字不显示，读回后的最终确认正常展示。当前仍是模型选择查询工具，尚未做到按 mutation receipt 确定性核对同一对象与字段；详见 [`03-可靠性.md`](03-可靠性.md)。
