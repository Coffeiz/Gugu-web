...
- **模型的"决定"上面还盖了一层兜底**：光靠提示词让模型"说到做到"并不可靠（尤其换成能力较弱的模型时），所以在模型收尾前加了一层确定性检查——模型说"做完了"却没真调工具、或者用户明确要求改动模型却擅自拒绝，都会被代码检测出来并逼它重新来一轮。这层机制的详细设计在 [`03-可靠性.md`](03-可靠性.md)。
