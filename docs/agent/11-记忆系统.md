...
1. ✅ **检索从「按重要度注入 top-N」→「相关性优先 + 重要度保底」**(已实现,见 §2 `render_facts`):facts 超上限时按当前消息词法相关性挑、重要度前 6 保底 + 补齐。**下一步(已升格为 Person Model 前置基建)**:把词法相关性换成 **embedding 向量召回**,补上「同义/语义」相关(词法只抓共享词,抓不到「会议」↔「周会」这类)。**升格理由**:北极星的 Person Model 会把 observation 攒到几百条,「每轮注入哪些」的检索/预算问题比「怎么写入」更难(写协议已解决写入,读取侧设计还是空白)——`render_facts` 这套「相关性+保底+预算」就是未来检索系统的雏形,向量召回是它的必经升级,**不是可选优化**。
