...
flowchart TB
    ...
    DAILY -- 攒满 75 条 · LLM 融合压缩 --> MEM[(memory.md<br/>长期记忆 · 越压越精)]
    PROFILE -. 周期性复核(§13) .-> PERIODIC[worker._profile_loop<br/>每 7 天一次·尚未实现]
...
**一句话读图**:每轮对话「读」六层记忆注入上下文 → 咕咕回复 → 「写」端后台反思把这轮提炼进 profile/pattern/daily/summary/lens(+感知遥测)→ daily 攒满融合进 memory、衰减件给 summary/inferred-pattern 降权 → 下一轮再「读」。捕捉(写)和注入(读)分离,反思失败不挡回复。profile 只在对话触发式反思里增删,不参与周期性复核(内容几乎不变,没什么可复核的);pattern 两条腿走路——对话触发式反思随手增删 + §13 的周期性复核做体检(常驻自动版尚未实现,手动/管理员触发版已实现,见 §13)。
...
| **daily** | `daily.md` | 本次/近期会话流水(新在上) | 攒够压缩进 memory,留最近 50 条 |
...
**去重**(`_fact_similar`):两条算「同一条」的判据——归一化相等 / 较短(≥6)是较长的子串 / 字符 bigram Jaccard ≥ 0.7。增量写入时合并,避免堆叠重复。阈值取高(0.7)是刻意保守——短中文条目里「喜欢猫」「喜欢狗」bigram≈0.6,低阈值会把不同条目误并成一条丢信息;宁可留近似重复(render 顶多多一行、反思可后续 remove),绝不误并。
...
**周期复核**:pattern 会参与 §13 的周期性复核——它是会演进的东西(过时的习惯该淡出、新习惯该被总结),不像 profile 那样几乎不变。
...
- **压缩触发**:daily 攒到 `DAILY_COMPACT_AT`(75) 条 → 取最老的(留最近 `DAILY_KEEP_RECENT`=50) → 连同已有 `memory.md` 交 LLM **融合**(合并重复、修正矛盾、丢弃琐碎、越压越精、不评判不推测) → 写回 memory.md,daily 只留最近 50。**约每 25 轮压一次**,按条数(不按天)控注入体积。`DAILY_HARD_CAP`(95)是压缩失败时的安全上限,防 daily 无限膨胀。
...
- **memory.md 注入 = 整篇 或 向量挑段**:memory.md 未超 `MEMORY_INJECT_CHARS`(1500 字)→ 整篇注入(现有行为);**超了且 embedding 启用** → 切块(按段/超长按句,`MEMORY_CHUNK_MAX`=400)后按 cosine 挑相关块拼到预算内(**保原文顺序**保叙述连贯);无向量/覆盖不足 → 退回整篇,零回归。见 §12。
...
│   ├── periodic.py     # 周期性反思常驻自动版(§13,设计中):pattern.json 体检 + 提炼,worker._profile_loop 调用
│   #（手动/管理员触发版已实现，不需要 periodic.py：见 scripts/refresh_memory.py + Admin「记忆维护」面板）
...
### 13. 周期性反思——常驻自动版设计中，手动/管理员触发版已实现
...
**方案**:在 §7 的对话触发式反思之外,加一个**周期触发**的入口,只针对 `pattern.json`(profile 不参与,见 §2)——同一套"提炼→写回"骨架,只是触发源和输入不同,不是新系统。**两件事都是 pattern.json 的日常维护,不引入新文件**:① 体检(该淡出的旧模式清掉)② 提炼(近期行为里有没有能总结成新模式的东西),都通过 pattern 已有的 `apply_facts_ops` 增删管线写回。

```mermaid
flowchart TB
    LOOP[worker._profile_loop<br/>常驻循环,仿 _reconcile_loop] --> CHECK{该用户距上次<br/>refresh_ts 超过 7 天?}
    CHECK -- 否 --> SKIP[跳过,下个用户]
    CHECK -- 是 --> GATHER[收集输入:<br/>当前 pattern.json 全量<br/>+ summary.md 当前状态<br/>+ 近期有变动的项目/daily]
...
**现状(2026-07-09):体检 + 一次性迁移债已实现,②提炼 和常驻自动触发还没有**——

| 部分 | 状态 | 在哪 |
|---|---|---|
| ① 体检(删过时 pattern) | ✅ 已实现 | `scripts/refresh_memory.py` 的 `--facts`(即 `_review_facts`,3 次投票) |
...
| ② 提炼(从近期行为总结新模式) | ❌ 未实现 | —— |
| 常驻自动触发(`worker._profile_loop`,每用户 7 天一次,不用人工点按钮) | ❌ 未实现 | —— |

也就是说,现在这套"周期性反思"是**人工/管理员按需触发**的(命令行脚本 或 Admin 按钮),不是**系统自动按 7 天周期跑**的——上面 mermaid 图里的 `worker._profile_loop` 和"距上次超过 7 天"这套调度逻辑还没写,`agent/memory/periodic.py` 这个文件也还不存在(体检+迁移逻辑目前直接写在 `scripts/refresh_memory.py` 里,被 `app/api/v1/config.py` 复用,还没抽成 `agent/memory/` 下的正式模块)。
