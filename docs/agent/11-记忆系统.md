...
**方案**:在 §7 的对话触发式反思之外,加一个**按活动量触发**的维护入口,只针对 `pattern.json`(profile 不参与,见 §2)——同一套"提炼→写回"骨架,只是触发源和输入不同,不是新系统。长期不活跃用户不运行;用户重新活跃并积累足够新信息后再异步维护。**两件事都是 pattern.json 的维护,不引入新文件**:① 体检(该淡出的旧模式清掉、重复模式合并)② 提炼(近期行为里有没有能总结成新模式的东西),都通过 pattern 已有的增删管线写回。

```mermaid
flowchart TB
    EVENT[对话结束后的活动计数] --> CHECK{达到活动量阈值<br/>且通过最小冷却?}
...
| ② 提炼(从近期行为总结新模式) | ❌ 未实现 | —— |
| 按活动量自动触发(活跃后异步维护,不扫描沉默用户) | ❌ 未实现 | —— |

也就是说,现在这套维护仍是**人工/管理员按需触发**的(命令行脚本或 Admin 按钮);按活动量自动触发的队列、冷却和异步调度还没接入,`agent/memory/periodic.py` 也还不存在(维护逻辑目前直接写在 `scripts/refresh_memory.py` 里,被 `app/api/v1/config.py` 复用,还没抽成 `agent/memory/` 下的正式模块)。
