...
flowchart TB
    ...
    PROFILE -. 周期性复核(§12) .-> PERIODIC[worker._profile_loop<br/>每 7 天一次]
    PATTERN -. 周期性复核(§12)<br/>多数票机制 .-> PERIODIC
...
**一句话读图**:每轮对话「读」六层记忆注入上下文 → 咕咕回复 → 「写」端后台反思把这轮提炼进 profile/pattern/daily/summary/lens(+感知遥测)→ daily 攒满融合进 memory、衰减件给 summary/inferred-pattern 降权 → 下一轮再「读」。捕捉(写)和注入(读)分离,反思失败不挡回复。profile 只在对话触发式反思里增删,不参与周期性复核(内容几乎不变,没什么可复核的);pattern 两条腿走路——对话触发式反思随手增删 + §12 的周期性复核做体检。
...
| **pattern** | `pattern.json` | 行为/决策模式(结构化,2b),每条带 kind/conf/imp/ts | 增量增删改、衰减、去重、退休、**周期复核(§12)** |
...
**周期复核**:profile 不参与 §12 的周期性复核——内容本来就不该变,没什么好体检的。只有对话触发式反思(§6)发现明确矛盾时才会增删。
...
- **超上限且有当前消息 query → 相关性优先挑**:① 重要度保底(前 `FACTS_FLOOR_K`=6,核心档案不被挤掉)→ ② 按对 query 的**相关性**填充 → ③ 仍没满用重要度补齐(不浪费预算)。**relevance > importance,importance 兜底**。query 从主路径 `req.message` 经 `read_memory` 穿入。
  - **相关性打分 = 向量语义 或 词法兜底**:embedding 启用且该条有当前模型的向量 → 用 **cosine 语义**(抓「会议」↔「周会」这类同义);否则退回**字 bigram 词法**(共享词才命中)。向量是可选加固,关了/没同步到就是纯词法,零回归。见 §11 向量检索。
...
**周期复核**:pattern 会参与 §12 的周期性复核——它是会演进的东西(过时的习惯该淡出、新习惯该被总结),不像 profile 那样几乎不变。
...
- **memory.md 注入 = 整篇 或 向量挑段**:memory.md 未超 `MEMORY_INJECT_CHARS`(1500 字)→ 整篇注入(现有行为);**超了且 embedding 启用** → 切块(按段/超长按句,`MEMORY_CHUNK_MAX`=400)后按 cosine 挑相关块拼到预算内(**保原文顺序**保叙述连贯);无向量/覆盖不足 → 退回整篇,零回归。见 §11。
...
│   ├── periodic.py     # 周期性反思(§12,设计中):pattern.json 体检 + 提炼,worker._profile_loop 调用
│   ├── compress.py     # daily 攒够 → 融合进 memory.md + 重嵌 memory 块向量
│   ├── embedding.py    # 向量原语:embed()/cosine()/model_tag()/is_enabled()(独立 pin,见 §11)
...
### 11. 向量语义检索(embedding,`memory/embedding.py` + `store.py`)
...
**要解决的问题**:现有反思(§6)只在**对话触发时**运行,一次只看得到"这一轮聊了什么",天然有两个盲区:① pattern 里混入不该留的内容后,除非某轮对话恰好戳中矛盾,否则永远不会被清理(2026-07-08 发现的教训:一条"项目推迟到 8/31"的过时条目,项目后来被拆分改名,但没有哪轮对话直白地推翻过它,于是一直留着);② 跨越多次对话才能看出来的行为模式(创作方向的偏好、卡点规律),单轮反思看不到全局,只能等某次对话恰好聊到才被动记一笔,不会主动去总结。

**方案**:在 §6 的对话触发式反思之外,加一个**周期触发**的入口,只针对 `pattern.json`(profile 不参与,见 §2)——同一套"提炼→写回"骨架,只是触发源和输入不同,不是新系统。**两件事都是 pattern.json 的日常维护,不引入新文件**:① 体检(该淡出的旧模式清掉)② 提炼(近期行为里有没有能总结成新模式的东西),都通过 pattern 已有的 `apply_facts_ops` 增删管线写回。
...
- **不是 ScheduledTask**:语义是"后台发现该刷新了",不是"用户要求某时刻发生某事",不走投递渠道,静默运行,失败不影响任何对话(与 §6 的反思同一哲学:锦上添花,不能拖累主流程)。
