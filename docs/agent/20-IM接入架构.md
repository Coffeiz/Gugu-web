...
> 群组与成员长期记忆不在本文展开；其独立设计与实施计划见 [`PRD-IM-3-群组与成员记忆.md`](../product/PRD/PRD-IM-3-群组与成员记忆.md)。本文只描述平台连接、消息归一化、队列、worker 和出站边界。
...
1. 读 `user_bots` 表里 `enabled=True` 的 bot（BYO：每用户自带 bot，凭据在 DB，不在 `.env`），平台→模块映射 `feishu`/`qqbot`/`wechat` → `agent.gateway.{feishu,qq,wechat}`
