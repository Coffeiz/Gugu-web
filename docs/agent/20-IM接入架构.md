...
实现是**单聊 C2C + 群聊** + **每用户自带 bot（BYO）**，代码 `agent/adapters/qq.py`。
...
- **收（群聊，2026-07-06 新增）**：同一 intent 下的 `on_group_at_message_create` 事件；`message.group_openid` = 群、`message.author.member_openid` = 群内发言人。**⚠️ botpy SDK 只有这一个群消息事件，且只在群里 @ 了机器人时才会触发**——没有「收全部群消息」的能力，是 QQ 平台层面的硬约束，不是咕咕没做，详见 §3.5。
...
**这对开关目前只有 QQ 真正接了**：QQ 网关（`agent/adapters/qq.py` 的 `on_group_at_message_create`）**每条群消息都现查一次 DB** 的 `group_chat_enabled`（`_group_settings`），不是启动时从环境变量注入一次——这样用户切开关立即生效，不用重启对应网关子进程。这是刻意的设计选择：`supervisor.py` 的 `reconcile()` 只在进程退出/新增删除时才 respawn，**进程存活期间改配置（含 `sandbox`/`app_id`/`app_secret`）它并不感知**，所以任何「开关要即时生效」的字段都不能走 env-注入-一次 的老路子，得在 handler 里现查。飞书本身天然收全部群消息、不分开关地全量处理（chat_id 通用路由，见 §3.1）；微信群聊目前有独立的回复路由 bug（见下），也没接这个开关。**前端「个人设置 → 接入咕咕」只在 QQ 的 bot 卡片下展示这个开关**（PUT `/me/bots/{id}` 落库）——飞书/微信不接就不摆一个切了没反应的假开关。

**QQ 的 `group_requires_at` 是假开关**：botpy SDK 层面群消息事件只有 `group_at_message_create` 一种，官方注释明确是「当收到群 @ 机器人的消息时」触发——没有"收全部群消息"的事件。也就是说 QQ **不管这个开关切成什么样，群里不 @ 机器人它就是收不到**。前端对 QQ 把这个字段固定显示为恒开启（不可点），标注"QQ 平台限制"，不假装能关掉一个关不掉的东西——所以前端目前也没给这个字段做真开关，只留了展示。
...
**微信群聊的已知遗留问题（未在本次一并修）**：`agent/adapters/wechat.py` 的 `_handle_msg` 已经在识别 `group_id` 并置 `chat_type="group"`/`wechat_group_id`，但 `worker.py` 里微信的发送分支目前**没有消费 `wechat_group_id`**，回复始终发去发言人的私聊而不是群——这是本次排查群聊架构时顺带发现的，和 QQ 群聊是两件独立的事，还没有修，之后单独处理。
