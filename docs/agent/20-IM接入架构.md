...
- **收（群聊）**：同一 intent 下支持 `GROUP_AT_MESSAGE_CREATE`（@机器人）和 `GROUP_MESSAGE_CREATE`（群消息）事件；`message.group_openid` = 群、`message.author.member_openid` = 群内发言人。咕咕默认只处理 @ 消息，关闭 `group_requires_at` 后处理全量群消息，具体能力取决于 QQ 机器人后台的群消息权限。
