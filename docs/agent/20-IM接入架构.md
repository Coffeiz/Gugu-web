...
**QQ 群聊暂不支持发图片/文件**：官方 `post_group_file` 接口本身就注明 `file_type=4`（任意文件）"暂不开放"，且现有 `qq.send_file`（走另一条私聊专用的富媒体上传接口）本就没做群聊版本——群里让咕咕发文件时 `worker._send_files` 会兜底回一句文字提示，不会静默丢文件。
