...
> **状态**：🟢 飞书 + QQ + 微信三平台均已上线（飞书/QQ BYO 扫码自连，微信 iLink 长轮询）｜文本/图片/语音三平台打通｜队列+worker+supervisor 三进程已部署，supervisor 带秒崩退避｜**群聊**：飞书天然支持、QQ 已接（2026-07-06，per-bot 开关）、微信部分支持（有已知遗留问题，见 §3.5）
> **分类**：技术架构 / Agent 平台接入
...
| 网关进程管理 | ✅ **已建**：`agent/gateway/supervisor.py` 按 DB `user_bots` 动态 spawn/kill 飞书·QQ·微信网关子进程，带指数退避（详见 §10） |
...
| `gugu-supervisor` | `python -m agent.gateway.supervisor` | **网关管家**：按 DB 拉起/看管飞书·QQ 网关子进程 | always | `logs/gugu-supervisor.log` |
...
- supervisor 仍手动启动：`python -m agent.gateway.supervisor`（后者需 DB 里有启用的 bot）；或临时单起一个网关：`FEISHU_BOT_ID=… FEISHU_APP_ID=… python -m agent.gateway.feishu`
...
| `router.py`、`runtime_state.py`、`gateway/feishu.py`/`qq.py`/`wechat.py`/`supervisor.py` | `gugu-supervisor`（取消标志的*读*在 worker，故路由判定改了也重启 worker 稳妥） |
...
- `agent/gateway/web.py`（SSE 流式）/ `feishu.py` / `qq.py` / `wechat.py`（IM 网关，BYO env 注入）/ `supervisor.py`（网关管家，按 DB spawn，带秒崩退避）
