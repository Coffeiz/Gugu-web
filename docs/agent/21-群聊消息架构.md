...
> 群组与成员长期记忆的独立设计、文件职责和实施计划见 [`【已完成】PRD-IM-3-群组与成员记忆.md`](../product/PRD/【已完成】PRD-IM-3-群组与成员记忆.md)。本文只维护消息接入、短期会话窗口和群聊分流。
...
要接收未 @ 咕咕的普通群消息，除了打开咕咕自己的 `group_read_enabled` / `group_requires_at` 外，还必须在**手机 QQ 的目标群**中开启平台权限：

```text
进入目标群 → 群设置 → 机器人 → 选择咕咕 → 设置
→ 机器人可获取的群聊消息范围 → 获取群内全部消息
```

这个开关不在 QQ 开放平台的 Web 管理后台，而是在手机 QQ 的每个目标群内单独设置。未开启时，QQ 通常只会下发被 @ 的事件，咕咕应用侧即使关闭“只响应 @”或开启“读取普通群消息”，也收不到 `GROUP_MESSAGE_CREATE`，因此无法记录普通消息。

平台侧的“获取全部消息”只决定 QQ 是否把事件推送给咕咕，不代表咕咕一定回复。收到普通消息后，是否静默记录或进入 Agent，仍由本地 `group_read_enabled` 和 `group_requires_at` 决定。
...
50 条上限解决的是短期群上下文的体积问题。群组/member 长期记忆已按 PRD-IM-3 完成作用域、异步反思、daily 压缩和删除屏障的代码实现；真实多平台人工验收仍待完成。本文不重复维护记忆文件和反思职责。
