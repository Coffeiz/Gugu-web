...
> 想看**一轮对话内部走哪些步、每步谁负责**，见 [`02-决策环.md`](02-决策环.md)（运行时决策环专题）。
> 本文是**架构总览 + 完整工具清单 + 现状**；**架构全景图**（可靠性执行 + 系统模块两张图）见 [`01-架构图.md`](01-架构图.md)，可靠性工程见 [`03-可靠性.md`](03-可靠性.md)；并发/扩量分期见 [`../ops/并发优化ROADMAP.md`](../ops/并发优化ROADMAP.md)，变更记录见 [`CHANGELOG.md`](../../CHANGELOG.md)，决策过程见 [`../DEVLOG.md`](../DEVLOG.md)。
...
- **说到要做到**：用户让改一个文件、建一个项目，不能嘴上说"好的已经改好了"实际上什么都没做——这是所有 AI Agent 最容易栽的坑，咕咕专门为此加了好几层代码兜底（不只是靠提示词自觉），详见下文"自我核实"和 [`03-可靠性.md`](03-可靠性.md)。
...
**说到做到，靠代码兜底而不是靠模型"自觉"**：模型每次说"做完了"，代码会强制它再核实一遍——真的去查数据库确认改动生效了吗？如果发现说了没做，会被打回去重做。这套机制经过参考同类开源项目（38 万星标的 OpenClaw）验证过设计思路是对的，细节见下文与 [`03-可靠性.md`](03-可靠性.md)。
...
└── agent/
    ...
    ├── memory/{store,reflection,compress,lens,_llm}.py + decay.py   # 五层记忆：facts(增量delta)/daily/memory/summary(时间衰减)/lens(解读先验)；reflection 提炼 + compress 压缩 + decay 衰减（详见 11-记忆系统.md）
    ├── behaviors.py + prompts/behaviors/   # 行为模块库（反思驱动 stance 互斥选择，9 模块：baseline 兜底 + 8 stance，见感知系统 §2.6）
    ...
    ├── gateway/{base,web,qq,feishu,gateway}.py
    ├── events/{bus,types}.py
...
```
Web 路：浏览器 → gateway/web.stream() → 后台任务 _generate()（脱离 HTTP，刷新不丢）
...
- **自我核实闭环（`MAX_VERIFY = 5`）**：成功调用已识别的增删改工具后，工具结果一回灌就直接注入一轮「系统自检」——不再先等待模型生成一段没有新信息的“已完成”收尾。工具返回 `error` 不触发；自检轮若只查证没改动 → 结束，若补做了 → 再进入自检，直到"只查不改"或封顶 5 轮。`note_get` / `note_search` 也被认作有效读回。**不是固定跑 5 轮**：通过即停，只读任务零额外开销。两路（Anthropic/OpenAI）同构，轮预算 `MAX_ROUNDS + MAX_VERIFY*2` 不挤占任务轮。当前查询由模型选择，尚未做到按 mutation receipt 确定性核对同一对象与字段，详见 [`03-可靠性.md`](03-可靠性.md)。
  - **静默复查过程（`verify_mode`/`verify_fixed`）**：核实阶段的过程文字先缓冲、不实时显示；即时进入复查后，读回完成的最终确认会展示给用户，避免只有工具状态却没有完成反馈。若发现遗漏并补做，补做说明也会正常展示。该逻辑在 core 源头处理，web/IM 两路统一受益。
- **真实性守卫（确定性兜底「说了没做」幻觉，两路同构）**：无工具收尾时检测两类幻觉、各追一轮逼纠偏（封顶 1 次）——① narration 兜底（`_looks_like_narration`：过程叙述/完成断言 + 本轮零工具 → 逼真调）；② 决策守卫（`_is_decision_dodge`：用户明确命令改动 + 回复驳回 + 零工具 → 逼执行或问清）。另有意图守卫 `_announces_intent`（将来式宣告 + 零工具 → 逼一轮，见 [`33-多步执行与防停顿.md`](33-多步执行与防停顿.md)）。配合自我核实闭环覆盖「动嘴不动手 / 改了不核对 / 自作主张不做」三坑。**失败模式→守卫的完整映射与工程哲学见 [`03-可靠性.md`](03-可靠性.md)（本文只列实现锚点，不重复设计论证）**
...
- `behaviors/`（9 个行为模块）：**这轮该怎么相处**——无有效 stance 时使用 `baseline`（姿态地图+中性默认），有有效 stance 时只选择对应的 8 个模块之一（execution/stuck-first/record/query/decision-explore/reflect/emotion-first/companion），由上轮反思判的 per-user stance 1:1 选择（`behaviors.py`，30min 新鲜度闸）。详见 [`10-感知系统.md`](10-感知系统.md) §2.6。
...
```
  ...
  **稳定的在前、易变的在后**：人格/规则/红线稳定 → 记忆 → 实时数据。persona/skills/policy 独立于 profile、所有 profile 共享。
  ...
  > **「今天/现在」按用户时区**（时区迁移 Phase 3）：`{now}/{today}`、深夜 0-4 点判断、`load_events` 的"今天"过滤都走 `user.timezone`（`loaders.load_user_tz`，`build(..., user_tz=)`）；工具 `overview`/`greeting` 的"today"走 contextvar `tz.now_ctx()`（入口 set 一次）。`user.timezone` 为空 → 回退服务器 `LOCAL_TZ`、零行为变化；填了（前端探测回写）异地用户才不再被咕咕报错"今天"。详见 [../backend/时区与时钟迁移方案.md](../backend/时区与时钟迁移方案.md)、[backend.md §2.13](../backend/backend.md)。
- **`context/session_history.py`**：会话历史按数据库消息 id 正序连续追加；未压缩时不再使用会滑动的最近 N 条窗口。压缩后根据 `ConversationSession.baseline_message_id` 只读取 baseline 之后的新消息，并保留 summary 行。
- **`tokens.py`**：负责 token 估算、工具块计数和压缩相关预算；不再负责 runner/web 的跨 run 历史窗口裁剪。
...
- **`embedding.py`（向量语义检索 · 可选加固，默认关）**：记忆检索的相关性打分从字 bigram 词法**升级到向量语义**（cosine，抓「会议」↔「周会」这类同义）。**独立 pin 的 embedding 模型**（与聊天解耦、走 OpenAI 兼容 `/embeddings`，支持自托管 Ollama）；facts 超注入上限（`FACTS_INJECT_MAX`=100）走向量挑、`memory.md` 超 2000 字切块向量挑，无向量或覆盖不足时也严格按 2000 字注入预算截断。反思写 facts / compress 写 memory 时**增量自动嵌**（只嵌没向量的），换模型有重建按钮。per-user 几百条用暴力 cosine、不要向量库；向量与文本分开存、是可重建缓存。是 Person Model 检索基建的地基（第一个消费者），收益要真超阈值才兑现。**详见 [`11-记忆系统.md`](11-记忆系统.md) §11。**
...
> ⚠️ **记忆系统现状**：已落地 **`facts.json`（结构化：kind/conf/imp/ts，反思增量增删改 + importance 过滤 + inferred 时间衰减）+ `daily.md` + `memory.md` + `summary.md`（带时间衰减）+ `lens.json`（解读先验）五层**（daily→memory 压缩在跑；summary 由反思顺带产出；lens 事件驱动自学；记忆变更走 `events/bus.py`；`/memory` `/forget` 控制命令）+ **向量语义检索**（`embedding.py`，可选加固、默认关、超阈值才动，见上）；**仍未实现**：facts 完整来源链（现只标 kind=observed/inferred，不记是哪轮对话）、weekly 层、lens 反驳↓通道、`/newchat`（UI 已有）。详见「用户个性化文件系统」+「现状与演进」。
...
「怎么读懂这个用户」的偏置规则，自成体系、事件驱动自学，属感知系统范畴——完整算法（候选→提拔双闸、触发语为键的复现匹配、confidence 动态）见 [`10-感知系统.md`](10-感知系统.md) §3.5，本文不重复。
...
- **Skills（`agent/skills/`，已落地）**：带触发条件的**「剧本」**——每个 skill 一个 markdown（frontmatter `name` / `description`(=何时用) / `emoji` + 正文），正文是一段可复用的做法说明，可指挥模型调用若干 tool。
  ...
  - **执行原语**：需要联网/取数的 skill 靠 `http_get(url)`（`tools/web.py`）——**带 SSRF 私网拦截**（私网/环回/链路本地/元数据全拦）、不跟随重定向；按 `Content-Type` 分支处理而非无脑截断原始响应体：`text/html` 用 `trafilatura` 提取正文转 markdown（带内联链接，读不出正文则提示可能需 JS 渲染、改走搜索）、`application/pdf` 复用 `doctext.py` 的 pdftotext、其它原样截断。正文里写 `curl <URL>` 时 builder 提示模型用 `http_get` 抓。例：天气=抓 `wttr.in/{城市}?format=3`。
...
- **IM 也真发文件**：`runner._collect` 收 `file` 事件 → `AgentResponse.files` → `worker._send_files` 按平台分发。
  ...
  - **QQ** `qq.send_file`：按会话目标选择 C2C 或群聊富媒体上传，本地附件统一走 base64；`msg_seq` 用 Redis `INCR qqseq:{msg_id}` 跨进程发号。平台不接受的文件类型仍通过明确失败结果降级，不静默丢附件。
...
- **大图自动压缩**（`_fit_image_for_vision`）：>5MB 或长边 >3840px 时喂模型前等比降采样重压 JPEG，目标约 8MB（只压副本、存库原图不动）。
...
- **文件库视频读取**：`read_file` 只抽取代表画面（长边最多 1920px）并提取前 300 秒音频，转成 16kHz 单声道 WAV 后交给语音模型转写，不把完整视频逐帧送入模型。
...
`client.py` MCP 协议客户端（stdio/SSE/HTTP）+ `registry.py` 动态加载外部 tools 注册为 skill，core 视其与 native 完全相同——这是 **client 方向**（咕咕**消费**外部 MCP），**Phase 3、未做**。

> **server 方向**（把**咕咕开放**给别的 agent 用）:方向探讨见 [`proposals/咕咕作为MCP开放-方向.md`](proposals/咕咕作为MCP开放-方向.md)——动作层可做、记忆层慎;未拍板。
...
- `classify(text)` → `progress / cancel / emotion / ack / agent`（纯关键词，整条匹配；取消/情绪只在短消息上判，**宁漏判进主模型、不误判短路**）
  - ⚠️ **「取消」无条件识别**：`取消` 这个词不受 `len(t) <= 12` 限制，只要消息包含「取消」即判 CANCEL（解决「@咕咕 取消」这类带前缀长消息漏判）；其余取消词（算了/不用）仍只在短消息上判。
  - ⚠️ **情绪/催词用「句首锚定」、不是子串匹配**：只在句首、或仅「你/咕咕」指向咕咕的前缀时才判（如「怎么这么慢」「你怎么这么慢」）。否则子串会把带话题主语的「**法拉利**怎么这么慢」「这电脑太慢」误当成催咕咕而短路。
- `decide(text, state, awaiting, *, current_puid, active_puid)` 结合 State Manager 状态出动作：`reply`（短路回话术，不入队）/ `cancel`（置取消标志）/ `no_permission`（无权取消，回一句提示）/ `agent`（入队）
  - ⚠️ **「取消」只在真在忙时取消**：空闲时发「取消」没有任务可取消，交主模型、不触发「好的，那先不继续啦～」的 ACK。
  - ⚠️ **取消权限隔离**：`current_puid`（当前用户）+ `active_puid`（当前会话活跃 loop 的发起者集合）判断——咕咕在跑别人的 loop 时，当前用户发「取消」返回 `no_permission`（「这个不是你的任务哦，咕咕还在忙～」），不真的取消；发起者本人取消则正常中断。
...
- 活跃 loop 集合 `agentactive:{platform}:{bot_id}:{scope_id}`：loop 启动时 `mark_active` 记录发起者 puid、结束时 `unmark_active` 移除（群聊 scope_id=chat_id、私聊 scope_id=sender.id）。网关据此判断「其他用户取消」的权限——当前用户不在集合里（咕咕在跑别人的 loop）→ 无权取消。
...
IM 回复完  → publish('sessions', appended=[助手消息])  # 再推
  → Redis PUBLISH events:{uid}（按用户隔离）→ FastAPI GET /api/v1/live/stream(SSE) 逐条下发
...
> **完整方案见 [`20-IM接入架构.md`](20-IM接入架构.md)；群聊消息链路见 [`21-群聊消息架构.md`](21-群聊消息架构.md)；飞书扫码细节见 [`22-飞书接入指南.md`](22-飞书接入指南.md)；拆解过程见 [`../DEVLOG.md`](../DEVLOG.md)。** 飞书/QQ/微信均**官方直连、不用 OpenClaw**；按「收消息 ↔ 跑大模型」解耦的**队列 + worker 架构**建。
...
- **动态网关 · 进程级**（`gateway/gateway.py`）：lark/botpy 连接只有 `start()`、**无 `stop()`** → 一个 bot 一个子进程，kill 子进程=断开。gateway 每 5s 查 `user_bots` reconcile（增/删/崩溃自愈）；**凭据走环境变量注入**（不走 argv，避免 `ps` 泄漏 secret）。
- **认人 · owner 即归属**：bot 天然属于 owner → 网关入队 payload 带 `owner_user_id`，`worker._resolve_user` 直接用（省掉绑定表）。
- **运行模型 · 三进程**：`web`（api/SSE）+ `worker`（消费队列跑大脑）+ `gateway`（管网关子进程）。
- **各平台**：飞书 ✅（lark-oapi WS，图片/文件/**语音 opus** 收，群聊天然支持，chat_id 通用路由）、QQ ✅（botpy WS，**C2C 私聊 + 群聊**，**语音 SILK** 收，markdown msg_type=2 无权限回退纯文本）、微信 ⬜（iLink 个人号，封号风险高，最后做）。
- **群聊**（2026-08）：`user_bots` 有 `group_chat_enabled`（是否处理群消息）、`group_requires_at`（是否需要 @）和 `group_read_enabled`（是否只记录未 @ 普通消息）三列，**目前只有 QQ 网关真正接了**（每条群消息现查一次 DB，切了立即生效、不用重启进程）。开启普通消息读取后，未 @ 消息只写入按群共享的会话，后续 @ 咕咕时进入上下文，不调用模型、不回复；详细链路见 [`21-群聊消息架构.md`](21-群聊消息架构.md)。QQ 群消息用 `chat_id`(=group_openid) 与 `platform_user_id`(=群内发言人 member_openid) 区分「回复发去哪」和「这是谁」，回复走 `qq.send_group`（区别于私聊 `send_c2c`）；群聊媒体按会话目标走群聊富媒体接口。三平台总体接入见 [`20-IM接入架构.md`](20-IM接入架构.md) §3.5。

### 并发模型

> **完整方案、分期与压测见 [`../ops/并发优化ROADMAP.md`](../ops/并发优化ROADMAP.md) + [`../ops/2026-06-25-TEST-CONCURRENCY-LOAD.md`](../ops/2026-06-25-TEST-CONCURRENCY-LOAD.md)。**
...
## 六、工具清单（共 58，已实现；核实：原文写 55/47 均已过时，见下方逐条更新）
...
>
> **核实说明（总数）**：逐个 `grep -rn 'name="' backend/agent/tools/*.py` 清点，default profile 实际接线的工具总数为 **58**（项目 15 + 日历 7 + 文件 15 + 客户 4 + 回收站 3 + 聚合 2 + 记忆 1 + 搜索 3 + 对话 2 + 定时任务 4 + web 1 + meta 1）。原文「共 55」「47 工具」两处数字均已过时——主要差异来自：日历新增了 3 个活动提醒工具（`add_event_reminder`/`list_event_reminders`/`remove_event_reminder`），文件库新增了 1 个 `list_recent_attachments`，且 `move_file` 已重构为 `move_items`（见下）；`set_priority` 独立工具已下线，优先级改并入 `create_project`/`update_project` 的参数（2026-07-05）。另有 `im.react`（1 个）已注册但不在 default profile 里，不计入此总数（见文末「工具一等公民」小节）。

> 🔒 = 不可逆操作，受删除二次确认保底（显式 `confirm` 参数，`agent/confirm.py`）保护。所有工具带 `user_id` 所有权校验。

### 项目 · `tools/projects.py`（15）

| 工具 | 说明 |
| --- | --- |
| `list_projects` | 项目列表，可按状态筛选 |
| `create_project` | 新建项目，**可带 `stages` 一次建阶段+待办**，可选 `priority` |
| `update_project` | 改状态/起止日期/客户/备注/名称/优先级 |
...
| `set_color` | 颜色（优先级 2026-07-05 起并入 `create_project`/`update_project`，不再单独开工具） |
...
- `deep_research`：**外部资料研究，走 Admin 选择的 Tavily / 百度搜索 / You.com**；Tavily/You.com 返回研究答案，百度返回普通搜索引用，适合读+总结+比较+研究+给引用；受每日次数配额（`SearchUsage`）。
- 路由与成本预算详见按需加载的技能文档 `agent/skills/web-search.md`（`prompts/skills.md` 只留一行指针）：普通查找走 web_search，读总结走 deep_research，找图走 image_search，SearXNG 超时/没结果由模型兜底转 deep_research；图片搜到后配合 `send_file(url=...)` 直接发出。后台 Admin → Agent 可配三家深度研究 Provider 并测试（`/admin/config/test-search`）。
...
- **感知与相处方式系统**：A+B 感知遥测 + 误判捕获 → Admin「感知诊断」面板（活跃用户宏平均 + 错读案例栏）；**persona 纯人格 + 行为模块库 9 个（反思驱动 stance 1:1 点亮、30min 新鲜度闸）**；per-user 解读先验 lens（事件驱动自学、偏置不独裁）；错读案例收集器（脱敏枚举）。详见 [`10-感知系统.md`](10-感知系统.md)。
- **IM 接入**：飞书 + QQ BYO 官方直连，扫码 device-flow 自动连接，三进程（web/worker/gateway）。
...
- **韧性/运维（P1/P2）**：配额硬拦截 + 6h/周双窗口封顶记账（核实：原文"配额能力降级（只读集）"的描述在当前代码中查无实据，已更正为实际的硬拦截机制，见「gateway/web.py」小节），稳定 consumer 名 + 死 consumer 清理，服务状态页（三进程状态/PID/心跳/一键重启 + IM 队列水位）。
- **定时任务**：用户自定义任务 + 提醒工作流（结果走通知/IM 不进对话）+ 日历活动自带的提醒子系统（核实新增，见「工具清单·日历」）。

### 关键架构决策

- **默认单机部署**：web+worker+gateway+网关同机，一套 `.env`/override 管全部。瓶颈是大模型延迟非机器，一台远超 50 人才到头 → `user_gate` 进程内锁即**终态**，多 worker/分片 park。
