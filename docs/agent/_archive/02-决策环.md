...
> 两个独立进程（网关 `gugu-gateway` / 大脑 `gugu-worker`）靠 Redis 通信。状态查询/取消由**网关层**据 Redis 短路——worker 有界并发但**按用户串行（user_gate）**，同用户后续消息排在锁后进不去，这是支点。
...
- IM 接入：飞书/QQ BYO 扫码自动连接、队列+worker+gateway 架构
