...
> 本文描述 owner 的通用六层记忆和底层读写/压缩能力。IM 群组与平台用户使用独立 namespace，不直接套用 owner 的写入和可见范围；详见 [`【已完成】PRD-IM-3-群组与成员记忆.md`](../product/PRD/【已完成】PRD-IM-3-群组与成员记忆.md)。
...
flowchart TB
    ...
    EX --> SUM[(summary.json<br/>当前状态快照)]
    EX --> LENS[(lens<br/>解读先验)]
    EX --> PERC[感知 / 误判遥测]

    DAILY -- 攒满 100 条 · LLM 整理沉淀 --> MEM[(memory.md<br/>长期记忆主档 · 保留历史)]
...
| **daily** | `daily.md` | 本次/近期会话流水(新在上) | 攒到 100 条后沉淀,保留最近 50 条 |
| **memory** | `memory.md` | 长期记忆主档(daily 沉淀整理而来) | 持续保留历史和时间脉络,不因 50 条规则截断 |
| **summary** | `summary.json` | 「用户当下在忙什么/近期重心」快照 | 增量演进 + 时间衰减(半衰期 5 天) |
...
**schema 刻意简单**:`{type, text, ts}`——**不再保存机器 `id`，没有 kind/conf，不衰减**。`type` 只能是 `name`、`address`、`pronoun`、`background`、`preference`、`note`，用于标记画像类别，不代表置信度。owner profile 保留 `ts` 作为审计时间，不参与衰减计算。旧条目没有 `type` 或仍带 `id` 时按 `note` 兼容，下一次写回会规范成新格式。

**注入**:profile 保持现有去重和稳定顺序，但固定上下文最多直注入 **50 条**（`PROFILE_INJECT_MAX`）。超过 50 条的条目不删除，保留在 memory RAG 索引中，按当前问题按需召回；不为此给 profile 引入新的置信度或重要度模型。
...
- pattern 直注入上限为 `PATTERN_INJECT_MAX`（50）；超过上限时只将重要度前 `PATTERN_FLOOR_K` 条和与当前 query 相关的条目直接注入，其余保留在 RAG 中按需召回。
- **超上限且有当前消息 query → 相关性优先挑**:① 重要度保底(前 `PATTERN_FLOOR_K`=6,核心档案不被挤掉)→ ② 按对 query 的**相关性**填充 → ③ 仍没满用重要度补齐(不浪费预算)。**relevance > importance,importance 兜底**。query 从主路径 `req.message` 经 `read_memory` 穿入。
...
**周期复核**:pattern 会参与 §13 的按条目水位触发的维护——它是会演进的东西(过时的习惯该淡出、新习惯该被总结),不像 profile 那样几乎不变。维护输出以 `merge` 优先,只有明确过时/错误/越界的条目才进入 `remove`;高置信度条目只能作为合并保留项。
...
- **压缩触发**:daily 攒到 `DAILY_COMPACT_AT`(100) 条 → 每次只取最老的 `COMPACTION_BATCH_SIZE`(100) 条,连同已有 `memory.md` 交 LLM **整理**(合并重复、修正矛盾、保留有价值的历史和时间脉络,不因篇幅直接删除) → 写回 memory.md,压缩成功后 daily 保留最近 `DAILY_KEEP_RECENT`(50) 条和未处理的旧记录。`DAILY_HARD_CAP`(175) 只用于压缩失败保护,不能静默丢弃历史。
- **daily 注入有独立硬上限**：存储仍按条数保留，但每轮最多注入最新 **2000 字符**（`DAILY_INJECT_CHARS`）；不会因为上下文预算截断或删除磁盘上的历史。超出部分留在 daily，等待后续压缩沉淀。
- 由 `reflection` 写完 daily **顺带触发**,失败不影响主流程。
- **防误删兜底**:原本有长期记忆、模型却返回空或明显丢失历史 → 不覆盖、也不裁 daily(避免丢数据)。
- **memory.md 注入与存储分离**:memory.md 允许持续增长,注入预算为 `MEMORY_INJECT_CHARS`（2000 字符）；超出后,embedding 启用则切块按 cosine 挑相关块并保留原文顺序;无向量或覆盖不足时使用词法/预算兜底,不把长期主档当成只能保留 50 条的近期窗口。见 §12。
- **事件章节格式（MEM-2 Phase 0–4）**:新写入的长期记忆统一使用 `## 记录长期记忆：<事件标题>` 章节；owner 与 IM 群组压缩会在写回前规范化标题、合并同标题章节并按稳定 hash 去重，旧的无标题主档只在重写时包进兼容章节。事件记忆只保留事件背景、时间线、决策和后续约定，不原句复制 profile/pattern；章节解析、去重和稳定 hash 由 `memory/event_memory.py` 提供。
...
### IM 群上下文的 snapshot 边界

IM 群的公开记忆与发言人的个人记忆采用不同生命周期：当前群的 `group` scope（summary/profile）进入共享会话的 snapshot 前缀，只在 snapshot 首次建立、TTL 到期或压缩 checkpoint 时刷新；snapshot 只保存群 scope，不保存任何 `platform-user` 内容。当前 member 的 `platform-user` scope 按本轮发言人动态读取，放在当前用户消息前的独立 reminder 中，因此不会因成员切换把个人记忆写入共享群 session，也不会污染缓存前缀。

两类 scope 分别执行权限判断和 `MEMORY_INJECT_CHARS=2000` 字符预算；每个 profile/pattern 来源还最多直注入 50 条（`IM_MEMORY_ENTRY_INJECT_MAX`），超出部分通过当前 scope 的 RAG 按需召回。group scope 仅允许当前 Bot + 当前群，platform-user scope 仅允许当前 Bot + 当前平台用户；unknown 不读取 platform-user。snapshot hash 覆盖群记忆渲染结果，内容变化先保留 pending revision，直到安全 checkpoint 再重建，不把摘要正文或成员记忆正文写入版本元数据。
...
反思触发按入口区分，不把群聊中“只记录但没有调用 Agent”的消息误当成完整对话轮次：

| 入口 | 默认触发 | 使用工具 | 空闲兜底 |
|---|---|---|---|
| Web / 私聊 | 每个 Agent 回合 | 每个回合 | 不额外等待 |
| 群聊中进入 Agent 的消息 | 累计 5 条未反思消息 | 当前回合立即反思 | 连续 15 分钟无新消息时收束 |
| 群聊中未进入 Agent 的普通消息 | 累计 30 条未反思消息 | 不适用 | 连续 15 分钟无新消息时收束 |

owner 与 member 在群聊中使用相同的触发节奏，但写入的 scope、提示词和可读范围仍然隔离。群组 scope 继续按活跃窗口最长 1 小时、空闲 15 分钟收束。阈值和游标必须持久化，服务重启不能丢掉未反思消息。
...
1. ✅ **检索:相关性优先 + 重要度保底,词法已升级到向量**(均已实现,见 §3 + §12):profile/pattern 每个来源最多直注入 50 条，超出内容通过 RAG 按需召回；pattern 超上限时按相关性挑、重要度前 6 保底 + 补齐。相关性从字 bigram 词法**升级成 embedding 向量语义**(补上「会议」↔「周会」这类同义召回),memory.md 也做了块级向量检索。**为什么是必经而非可选**:北极星的 Person Model 会把 observation 攒到几百条,「每轮注入哪些」的检索/预算问题比「怎么写入」更难——`render_facts` 这套「相关性+保底+预算」就是未来检索系统的雏形,向量召回是它的地基。**剩余**:向量目前默认关、收益要 pattern>50 / memory>1500 字才兑现(小数据期潜伏);相处镜片的记忆引用、检索排序是同一层的后续消费者。
...
5. **用户知识图谱(2026-07-09 讨论、暂缓)**:讨论过要不要给用户建实体/关系图谱。结论是暂缓,先把 §13 的②提炼(从近期行为总结新模式)做出来——按条目水位触发的常驻维护已经落地,图谱要解决的"事实之间有什么关系"这个问题,现在的量级(per-user 几十~百来条 pattern)靠 memory.md 叙事 + 向量检索基本够用,图遍历的优势体现不出来,项目也没有图数据库这类基础设施。真要做的话,"每周批量 + 多数票"这套已验证的稳定性机制可以直接套用(图谱的实体/关系判断比 profile/pattern 二选一多好几个维度,单次调用的判断只会更不稳定,不能省投票这一步);但优先级上,§13②比图谱更值得先做——同样是"理解用户随时间变化",②提炼不需要新增任何存储结构。
...
- **memory.md**:compress 每次重写后调用 `sync_memory_vecs`；块用文本哈希当 key，未变化块复用向量，变化/新增块重嵌，消失块 GC。
  - **query**:`read_memory` 在 pattern>50 或 memory>1500 字时 embed 当前问句**一次**、pattern/memory 共用(热路径省调用)。
...
- **时机**:收益要 pattern>50 / memory>1500 字才兑现(小数据期潜伏),所以现在多为 no-op;是 Person Model 检索基建的地基,先备好。设计见 [`参考/咕咕改进方案-MaiBot借鉴.md`](参考/咕咕改进方案-MaiBot借鉴.md) 改进一。
