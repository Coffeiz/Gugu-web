...
> 群组与成员长期记忆不在本文展开；其独立设计与实施计划见 [`【已完成】PRD-IM-3-群组与成员记忆.md`](../product/PRD/【已完成】PRD-IM-3-群组与成员记忆.md)。本文只描述平台连接、消息归一化、队列、worker 和出站边界。

> **状态**：🟢 飞书 + QQ + 微信三平台均已上线（飞书/QQ BYO 扫码自连，微信 iLink 长轮询）｜文本/图片/语音三平台打通｜队列+worker+gateway 三进程已部署，gateway 带秒崩退避｜**群聊**：飞书天然支持、QQ 已接（2026-07-06，per-bot 开关）、微信部分支持（有已知遗留问题，见 §3.5）
> **分类**：技术架构 / Agent 平台接入
> **创建**：2026-06-23 ｜ **更新**：2026-07-06
> **目标**：把咕咕接入飞书 / QQ / 微信三个 IM 平台，**不依赖 OpenClaw**，且架构**未来可平滑扩到高流量**。
>
> 运行时一轮对话内部决策环（含 IM 前置路由 / 状态机 / 自然语言取消）见 [`02-决策环.md`](02-决策环.md)。
...
实现是 **BYO（每用户自带 app）**，代码 `agent/gateway/feishu.py` + `app/api/v1/feishu_connect.py`。

- **鉴权**：App ID + App Secret（用户扫码自动创建的 PersonalAgent app），SDK 自动管 `tenant_access_token`
- **收**：`im.message.receive_v1`，走 **WebSocket 长连接**（`lark.ws.Client`）→ 不需要公网；收凭据由 gateway 走 env 注入，payload 带 `owner_user_id`
...
### 3.2 QQ — `botpy`（tencent-connect/botpy，官方）· 已落地（C2C 私聊 + 群聊，BYO）

实现是**单聊 C2C + 群聊** + **每用户自带 bot（BYO）**，代码 `agent/gateway/qq.py`。

- **鉴权**：AppID + AppSecret（q.qq.com 开发者后台）
- **收（私聊）**：WebSocket，继承 `botpy.Client`，`botpy.Intents(public_messages=True)` 订阅，事件 `on_c2c_message_create`；`message.author.user_openid` = 用户、`message.id` = 被动回复用的 msg_id
- **收（群聊）**：同一 intent 下支持 `GROUP_AT_MESSAGE_CREATE`（@机器人）和 `GROUP_MESSAGE_CREATE`（群消息）事件；`message.group_openid` = 群、`message.author.member_openid` = 群内发言人。咕咕默认只处理 @ 消息，关闭 `group_requires_at` 后处理全量群消息，具体能力取决于 QQ 平台侧的群级权限。要让 `GROUP_MESSAGE_CREATE` 实际下发，必须在**手机 QQ 的目标群 → 群设置 → 机器人 → 咕咕 → 设置**中，将“机器人可获取的群聊消息范围”设为“获取群内全部消息”；QQ 开放平台 Web 后台没有这个开关，且需要每个目标群单独设置。应用侧的 `group_read_enabled` / `group_requires_at` 只决定收到事件后的本地处理方式。
- **发**：被动回复私聊用 `BotAPI.post_c2c_message(openid, msg_type=0, content, msg_id)`、群聊用 `BotAPI.post_group_message(group_openid, msg_type=0, content, msg_id)`（worker 端独立 `BotHttp(...).login(Token)` 取 token，过期自动重建）
...
- gateway 从 **DB** 读启用的 user_bots，每个起一条网关子进程，凭据走 **环境变量注入**（不走 argv，避免 ps 泄漏）。
...
无 QQ 那种开放 bot 平台；官方新通道是 **iLink**（`ilinkai.weixin.qq.com`）。代码 `agent/gateway/wechat.py` + `agent/gateway/wechat_client.py`，与飞书/QQ 同 BYO 模型（凭据存 `user_bots`，`bot_token` 复用 `app_secret` 字段、`base_url` 复用 `app_id` 字段）。
...
`user_bots` 表每个 bot 一对开关（迁移 `20260706000001`）：
- `group_chat_enabled`：是否处理这个 bot 的群消息，默认关。
- `group_requires_at`：群消息是否要求 @ 机器人才响应，默认开。

**这组三个开关目前只有 QQ 真正接了**：QQ 网关（`agent/gateway/qq.py` 的 `_handle_raw_qq_message`）**每条群消息都现查一次 DB** 的 `group_chat_enabled`、`group_requires_at`、`group_read_enabled`，不是启动时从环境变量注入一次——这样用户切开关立即生效，不用重启对应网关子进程。这是刻意的设计选择：`gateway.py` 的 `reconcile()` 只在进程退出/新增删除时才 respawn，**进程存活期间改配置（含 `sandbox`/`app_id`/`app_secret`）它并不感知**，所以任何「开关要即时生效」的字段都不能走 env-注入-一次的老路子，得在 handler 里现查。飞书本身天然收全部群消息、不分开关地全量处理（chat_id 通用路由，见 §3.1）；微信群聊目前有独立的回复路由 bug（见下），也没接这个开关。**前端「个人设置 → 接入咕咕」只在 QQ 的 bot 卡片下展示这些开关**（PUT `/me/bots/{id}` 落库）——飞书/微信不接就不摆一个切了没反应的假开关。详细的普通群消息落库与 Agent Loop 分流见 [`21-群聊消息架构.md`](21-群聊消息架构.md)。

**QQ 的 `group_requires_at` 现在是真开关**：开启时只处理 `GROUP_AT_MESSAGE_CREATE`；关闭时额外接收 `GROUP_MESSAGE_CREATE`，并进入正常回复链路。另有 `group_read_enabled` 只读取模式：开启后普通群消息会落库供后续 @ 使用，但不调用模型、不回复。若 QQ 机器人后台没有开通全量群消息权限，应用侧开关也不会收到非 @ 消息；前端因此把它标为需要平台权限的可选项。

**回复路由**：群消息和私聊消息「回复发去哪」和「这是谁在说话」是两个不同的 id，QQ 用 `chat_id`(=`group_openid`，回复路由用) 与 `platform_user_id`(=`member_openid`，群内发言人，用于会话状态/State Manager 按用户维度隔离) 区分，`chat_type` 标 `"group"`/`"c2c"`。`worker.py` 的 `_send()` 按 `chat_type` 分流到 `qq.send_group(chat_id, ...)` 或 `qq.send_c2c(platform_user_id, ...)`——这个 `chat_id` 字段名和用法是照抄飞书的既有约定（飞书群/私聊统一用 `chat_id` 路由，见 §3.1，本就不需要区分），QQ 补上群聊后复用同一套字段，没有另起炉灶。

**QQ 群聊媒体发送**：`qq.send_file` 按 `chat_type` 选择 `/v2/groups/{group_openid}/files` 或 `/v2/users/{openid}/files`，本地附件统一以 base64 上传后再发送媒体消息；群聊和私聊共用 `worker._send_files`，失败时保留明确的文本结果，不会静默丢文件。平台能力仍以 QQ API 对当前 `file_type` 的支持为准。

**微信群聊的已知遗留问题（未在本次一并修）**：`agent/gateway/wechat.py` 的 `_handle_msg` 已经在识别 `group_id` 并置 `chat_type="group"`/`wechat_group_id`，但 `worker.py` 里微信的发送分支目前**没有消费 `wechat_group_id`**，回复始终发去发言人的私聊而不是群——这是本次排查群聊架构时顺带发现的，和 QQ 群聊是两件独立的事，还没有修，之后单独处理。
...
代码库已有 `agent/models.py` 的 `AgentRequest` / `AgentResponse` + `agent/gateway/base.py`——缝早留好了。守住两个边界：
...
> Web SSE 适配器（`gateway/web.py`）保持不变（流式、请求驱动）；bot 走新的非流式队列路径；两者共用 `core.LLMRunner`。
...
| 网关进程管理 | ✅ **已建**：`agent/gateway/gateway.py` 按 DB `user_bots` 动态 spawn/kill 飞书·QQ·微信网关子进程，带指数退避（详见 §10） |
| 微信接入 | ✅ **已落地**：`agent/gateway/wechat.py`，BYO + iLink 长轮询，文本/图片/语音全通（图片 CDN+AES-128-ECB 解密，语音走 iLink 自带 ASR 转写，见 §3.3） |
...
| Runtime Router + 状态机（文档 29 / Phase 1.7） | ✅ **已落地**：`agent/router.py`(关键词分类) + `agent/runtime_state.py`(Redis 状态机 IDLE/THINKING/SEARCHING/GENERATING + 取消标志)；网关层短路状态查询/取消，详见 [`02-决策环.md`](02-决策环.md) §⓪。**注**：情绪/催词用「句首锚定」防话题误判（「法拉利怎么这么慢」不算催咕咕）；催促只在咕咕真在忙时才拦、回「还在想/正在弄」，空闲交主模型 |
...
> **重要坑**：后端现为 `uvicorn --workers 1`（生产 backend 单元用 2）。若为扩量把 web 开多 worker，每个进程会各自再拉一遍 bot 长连接 → 重复连接/处理。故 **bot 网关/worker 脱离 web 当独立进程起**（`gateway/` 不依赖 FastAPI request 即为此）——见 §10 三进程拆分。
...
> step 1-5 全部对真实平台跑通：飞书/QQ 走 BYO 扫码自连，`gateway` 按 DB 起网关 → 网关入队 → worker 消费 → `run_collect` → 调平台 API 发回。用户映射用 BYO owner 免了（见 §6）。

---

## 8. 高流量必踩点（第 6 步起逐个加）

- **平台用户 ↔ 咕咿用户映射**：BYO Bot 先按 `user_bots.user_id` 确定咕咿账号；QQ owner 的 `platform_user_id` 不再由首条私聊自动绑定，而由网页生成一次性验证码、QQ 私聊提交后原子绑定。热映射缓存进 Redis（咕咿是多用户，**必须隔离**）。
...
- **进程管理**：worker 是独立进程；开发可用 `make dev-worker` 热重载，生产由 `gugu-worker.service` 管理
...
2. **用户映射策略**：BYO Bot 账号归属由 `user_bots.user_id` 决定；QQ owner 平台身份使用一次性验证码绑定，不做跨 Bot 全局身份合并。
3. **反思等附加 LLM 调用是否计入用户配额**（现不计，高流量下成本需重算）。
4. **IM 慢工具进度声明要不要加开关**：多步工具循环期间 IM 用户完全看不到中间反馈的体验问题，讨论稿见 [`proposals/IM慢工具进度声明-设计.md`](proposals/IM慢工具进度声明-设计.md)；待评审通过、开关位置定下来后，这里要补一节说明。
...
| `gugu-gateway` | `python -m agent.gateway.gateway` | **网关管家**：按 DB 拉起/看管飞书·QQ 网关子进程 | always | `logs/gugu-gateway.log` |

### 10.1 网关是 gateway 动态 spawn 的子进程（无独立单元）

`gateway.py` 不是固定起几个网关，而是**对账循环**（`POLL_SEC=5`，每 5s 一轮）：

1. 读 `user_bots` 表里 `enabled=True` 的 bot（BYO：每用户自带 bot，凭据在 DB，不在 `.env`），平台→模块映射 `feishu`/`qq`/`wechat` → `agent.gateway.{feishu,qq,wechat}`
...
5. 单元设 `KillMode=control-group`：重启 gateway 会**连带杀掉它 spawn 的全部网关子进程**，再由新进程统一拉起

> 含义：**在 Admin 里启用/停用某个 bot 不用碰 systemd**——gateway 5 秒内自动 spawn/kill 对应网关。只有改了代码/凭据机制才需重启服务。
...
systemctl restart gugu-gateway    # 改了网关或路由（router.py / feishu.py / qq.py / wechat.py / gateway.py）→ 重启网关（连带子进程）
...
- `./start.sh {start|stop|restart}` —— **只管 web 后端 uvicorn**，不含 worker/gateway
- Web 开发推荐 `make dev-web`，自动监听 `app/`、`agent/`、`onboarding/` 并重载。
- Worker 开发推荐 `make deps-dev` 后运行 `make dev-worker`，自动监听 Worker 依赖的 Python 目录并重启；不要和同机 systemd worker 并行运行。
- gateway 仍手动启动：`python -m agent.gateway.gateway`（后者需 DB 里有启用的 bot）；或临时单起一个网关：`FEISHU_BOT_ID=… FEISHU_APP_ID=… python -m agent.gateway.feishu`

### 10.3 改代码后该重启谁

进程启动时把对应模块**载进内存**；改磁盘文件不影响已在跑的进程，必须重启对应服务，且**先把新代码同步到该进程所在主机**（worker / gateway 可能与本机不在一台）。

| 改了什么 | 重启 |
|---------|------|
| `agent/core.py`、`skills/*`、`runner.py`、prompts 之外的 agent 逻辑 | `gugu-worker` |
| `router.py`、`runtime_state.py`、`gateway/feishu.py`/`qq.py`/`wechat.py`/`gateway.py` | `gugu-gateway`（取消标志的*读*在 worker，故路由判定改了也重启 worker 稳妥） |
...
- `agent/gateway/web.py`（SSE 流式）/ `feishu.py` / `qq.py` / `wechat.py`（IM 网关，BYO env 注入）/ `gateway.py`（网关管家，按 DB spawn，带秒崩退避）
...
- `backend/{gugu-backend,gugu-worker,gugu-gateway}.service` + `start.sh install` — 三进程部署（见 §10）
