...
- 压缩只处理 baseline 之前的 canonical history，不重新压缩 system、snapshot、工具目录、当前轮或已经独立持久化的 RAG/姿态事件；
...
> 公共分支执行：反思与压缩统一使用 `agent/context/branch.py` 的 `ContextBranch`，provider 调用集中在 `provider_runner.py`。领域模块只负责 Prompt、输入适配、结果校验和持久化；详见 [PRD-AGENT-5](../../prds/PRD-AGENT-5-ContextBranch反思与压缩统一架构.md)。
...
flowchart TD
    ...
    D --> E[builder + assembly]
    E --> F[固定 snapshot + baseline 摘要/增量 history + NewMessageBatch]
...
上下文组装分为“稳定前缀、baseline 历史、canonical history、当前轮 batch”四个边界。当前轮由 `prepare_run()` 通过唯一的 `NewMessageBatch` 入口一次性组装；进入 provider 适配器前只合并一次，不再由各渠道分别拼接动态尾部。工具 schema 按当前声明状态随 provider 请求发送，工具 schema 事件若已经成为 canonical history，则通过统一历史回放恢复，不把 provider wire JSON 写入历史。发送前不使用本地 token 预估触发压缩。
...
flowchart LR
    ...
    D[当前轮 NewMessageBatch\n姿态变化 + 用户消息 + RAG + 临时提醒/时间] --> E
...
组装顺序固定为：`snapshot → baseline summary/history → current turn batch`。`run_context.prepare_run()` 是 Web、IM、定时任务共享的业务组装入口；provider adapter 只负责把 canonical history 转成供应商格式，不得重新排序、补写 summary、重复拼接当前轮或自行计算历史预算。

### 2.1.1 当前轮 batch 的成员与持久化边界

`NewMessageBatch` 只表示本次 provider 请求新增的消息，不等于“全部都会写入数据库”。当前实现按语义拆分如下：

| 当前轮内容 | 进入 batch | 下一轮如何出现 | 说明 |
| --- | --- | --- | --- |
| 当前用户消息 | 是 | 用户消息已由入口持久化，历史读取时恢复 | 不在 run 收尾重复写入 |
| 姿态 reminder | 仅在姿态 digest 变化时 | run 收尾追加 `stance-context` canonical block | 每次变化都保留，不能按正文去重或删除旧姿态 |
| RAG/Knowledge 结果 | 有结果时 | run 收尾追加 `knowledge-context` block | 作为 canonical history 保留，不只是本轮去重 |
| 时间 reminder | 是 | 不持久化，下轮按新消息时间重新生成 | 只描述当前请求的时间语境 |
| 临时 reminder/交互状态 | 视入口传入 | 默认不持久化 | 不得污染稳定 snapshot |
| tool call/result、skill/schema 事件 | provider 续轮产生 | 由统一 canonical history 收尾持久化 | call/result 必须保持原子顺序 |

因此姿态与 RAG 都不是每轮可被“刷新掉”的临时尾部：新的姿态只在 digest 改变时追加，RAG 结果按本轮结果进入历史；已进入历史的内容不能被下一轮重新组装逻辑删除。时间和临时状态则明确保持 request-volatile。

### 2.1.2 工具续轮与缓存锚点

`PromptMessages` 在同一个 run 的工具续轮中只追加新的 batch/工具消息，不重新拼接已经存在的前缀。缓存锚点遵循两项规则：

- 第一个锚点表示当前请求的 baseline，跨工具续轮保留；
- 最新锚点表示当前请求最近追加的尾部，后续续轮只替换这个最新锚点，不覆盖 baseline。

因此工具调用不会因为一次续轮而把整个上下文重新当作新前缀。工具 schema 的变化单独由 schema digest 诊断；不能用改变 history 顺序的方式修复 schema 缓存问题。
...
4. 由 `prepare_run()` 组装当前消息、姿态变化、时间 reminder、RAG 和临时提醒为一个 `NewMessageBatch`；
...
| `backend/agent/context/assembly/` | 定义固定前缀、canonical history 和当前 `NewMessageBatch` 顺序；维护缓存锚点 | 不拼接 provider wire JSON，不维护第二套动态尾部 |
| `backend/agent/context/run_context.py` | Web/IM/定时任务共享的单次 run 组装；读取 snapshot/history 并生成 batch | 不发送 provider、不直接写数据库 |
| `backend/agent/context/run_finalize.py` | 按 canonical 事件类型持久化本轮结果、姿态和 RAG，并调度 baseline | 不重新组装 prompt，不按正文去重历史 |
...
- `turn_batch_tokens`：当前 `NewMessageBatch` 的本轮新增消息，包含时间、姿态、RAG、记忆、项目/文件摘要等，必须计入预算；
...
- summary 不进入当前轮 batch，不作为 system/snapshot 的第二份状态，也不重复复制到固定前缀。
...
- 普通对话轮次不重建 snapshot，也不把当前 batch 写回 snapshot；需要跨轮保留的姿态、RAG、工具和 Skill 事件进入 canonical history；
...
RAG、daily、profile、pattern、群/成员记忆、项目/文件/笔记摘要属于当前轮业务上下文。它们先进入 `NewMessageBatch`；其中 RAG/Knowledge 结果在 run 收尾写入 canonical history，记忆和临时摘要是否持久化由各自模块的事件契约决定，不得把不同生命周期混在一个临时区域：

- 查询前按权限和当前 scope 过滤；
- 返回结构化结果和 token/字符估算，统一计入 `turn_batch_tokens`；
- 已在 snapshot 中覆盖的内容通过 digest 去重；但已经进入 canonical history 的 RAG 事件不能因为相似而删除，避免改变历史前缀；
...
3. 明确属于固定前缀、canonical history 还是当前请求临时 batch；
4. 在 `context/assembly/` 接入，并把估算计入 `ContextBudget`；
...
- RAG/记忆当前轮 batch 计入预算并遵循权限；RAG 结果按契约持久化为 `knowledge-context`，时间等 request-volatile 内容不持久化；
