...
- 请求接近预算时先尝试 LLM 压缩；压缩失败、压缩请求本身超限或最近消息仍超预算时，才执行确定性的分级兜底。
...
请求自救流程不是记忆压缩：先尝试一次 LLM 摘要；若摘要不可用，先保留最近 20 条完整消息，仍超预算时才做本地确定性截断。
...
正常压缩由现有异步记忆维护流程负责，可以在空闲、定时或消息窗口达到阈值时执行。请求前的同步压缩也复用同一摘要契约；同一 session 通过 Redis 锁串行，避免后台维护和手动 `/compact` 覆盖彼此的 baseline。
