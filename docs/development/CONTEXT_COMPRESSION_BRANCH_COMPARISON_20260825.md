...
实现收口后，CAS 写回已由持久 baseline 入口实际执行：摘要请求完成后会重新读取
`baseline_message_id` 与 `baseline_message_hash`，任一变化都会丢弃候选；摘要候选还必须通过
非空、10,000 字符上限和外层包裹标记校验。此前“本次结果不能证明并发提交逻辑已经完成”只适用于
本报告生成时的隔离脚本，不代表当前实现缺少 CAS。
