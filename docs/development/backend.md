...
- 生产环境执行 `make install` 后，`make start`、`make stop`、`make restart`、`make status` 会自动管理 `gugu-backend`、`gugu-worker` 和 `gugu-supervisor` 三个 systemd 服务；需要强制使用本地 PID 模式时设置 `GUGU_SERVICE_MODE=local`。
