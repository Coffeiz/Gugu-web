...
> 相关：[决策环](../agent/02-决策环.md) · [IM 接入架构](../agent/20-IM接入架构.md) · [压测结果](并发压测结果.md)
...
- ✅ 配额能力降级：配额耗尽不再硬拦，降级到只读工具集（12/47）+ 提示婉拒重操作——查询/对话照常（web 路；`gateway/web.py` + `profile.light_tool_names`）
