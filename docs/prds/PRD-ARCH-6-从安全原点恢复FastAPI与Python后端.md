...
- [ ] 按 pnpm 提交清单恢复根目录 `package.json`、`pnpm-workspace.yaml`、`pnpm-lock.yaml`、`.npmrc` 及相关 workspace manifest。
- [ ] 恢复前端、LoopScope 和 TS RAG 的依赖声明、脚本、类型配置与构建入口。
- [ ] 确认依赖安装、前端 typecheck/build、TS RAG build/test 均可独立执行。
- [ ] 扫描并排除 TS API/TS Agent 专属依赖、启动脚本和生产入口，避免依赖恢复顺带改变后端 owner。

### Phase 2：恢复前端修复

只应用前端领域提交，前端 API 目标保持 FastAPI；涉及后端协议的混合提交必须拆分后再应用。

候选 commit（初始状态，均待按前端路径应用）：

```text
[ ] 50d48882  [ ] 81477f84  [ ] 8dc3b228  [ ] 0c1ef88c  [ ] b7d151f1
[ ] 905f0af7  [ ] 6b7fda51  [ ] ef2ea9e6  [ ] 3a504875  [ ] e4841cff
[ ] 823a684a  [ ] 00f41317  [ ] 89862932  [ ] ee57d37c  [ ] 5ac3fc17
[ ] c9b790a9  [ ] 08948adc  [ ] a726761c  [ ] 0be9714f  [ ] ffe5c39a
[ ] 4453969d
```
