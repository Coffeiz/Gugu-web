...
| 单用户身份模式 | 🔲 待评估 | 轻量版只保留一个本地用户身份，复用归属模型保存数据，不维护多租户能力。 |
| Admin 后台 | 🔲 待评估 | 轻量版不提供 Admin 页面、Admin API 或后台用户管理；标准模式继续保留。 |
...
3. 保留单用户 Web、项目、文件、日历、画布、记忆、Agent Loop 和基本定时任务能力。
4. 轻量版不维护多租户和 Admin；标准模式继续保留完整后台、用户管理和多实例能力。
5. 在单进程范围内继续支持 IM；消息必须可恢复、可确认，不能用内存队列替代持久事实。
6. 标准模式继续使用 PostgreSQL + Redis，不改变现有生产拓扑和多实例能力。
7. 安装向导能告诉用户当前模式、资源边界、不可用能力和切换代价。

### 1.3 明确不做

- 不把 Redis 或 PostgreSQL 的异常静默吞掉后继续假装是完整标准模式。
- 不支持轻量模式下的多实例横向扩展、跨主机 worker、跨进程可靠锁和高可用故障转移。
- 不在轻量模式维护多租户、用户邀请、角色权限、用户管理、Admin 配置和后台运维页面。
...
安装或首次启动时支持 `light` 与 `standard` 两种模式。`light` 是单用户单机模式，`standard` 保持当前 PostgreSQL + Redis 模式。模式写入受保护的本地运行配置；轻量模式由本地启动向导展示状态，标准模式由 Admin 页面展示关键依赖状态。

模式切换必须是显式操作：切换前展示数据目录、备份要求、预计不可用能力和是否需要重新导入数据；不能因为连接失败自动切换数据库或队列实现。

### FR-LITE-002：SQLite 数据库

轻量模式使用 `sqlite+aiosqlite`，数据库文件位于用户指定的数据目录。必须启用外键约束、合理的 busy timeout 和单写者事务边界，避免多个请求同时写入时出现难以解释的锁错误。数据库仍保留一个本地用户身份，以复用现有会话、文件和归属边界；该身份不用于多租户管理。
...
IM 入站消息和必须重启后恢复的任务进入本地队列。轻量版只允许由同一 FastAPI 进程消费，避免引入独立 worker 和跨进程协调；标准模式继续由独立 worker 消费 Redis Streams。
...
轻量模式通过启动输出、个人设置中的本地状态区域或诊断命令展示：部署模式、数据库类型、Redis 是否需要、队列积压、过期租约、后台任务失败数和存储路径摘要。轻量模式不提供 Admin 状态页和后台用户管理。日志只记录脱敏的 id、计数、状态和 fingerprint，不记录聊天正文、附件名、Token 或密钥。

轻量模式的基础设施降级必须是显式状态，例如 `redis_required=false`、`queue_backend=sqlite`，不能只靠一条“Redis 连接失败”日志让用户猜测当前实际模式。

### FR-LITE-010：单用户身份边界

轻量模式首次启动创建一个本地用户身份，后续只允许该身份访问本地数据。不得出现用户列表、邀请、角色编辑、跨用户查询、租户切换或 Admin 登录入口。需要升级到标准模式时，必须通过显式导出/导入流程创建标准模式中的用户归属，不得把轻量模式直接伪装成多租户实例。
...
| 身份与配置 | 多用户 + Admin | 单一本地用户 + 启动向导/个人设置 | 本地用户与运行配置 |
...
├── app/api/v1/runtime.py           # standard 模式状态接口；light 使用本地诊断
├── app/local/                      # light 单用户初始化和本地状态
├── agent/queue/                    # 队列 facade 与消费生命周期
├── worker.py                       # standard Redis worker；light 不启动独立 worker
...
- Web/IM/定时任务进入同一 Agent Loop，单会话串行且不会重复回复；轻量模式不启动独立 worker。
...
- 轻量模式不存在 Admin 页面、Admin API、用户列表、邀请和跨用户数据读取。
...
先以显式 `light` 配置启动独立临时实例，完成 Web、文件、项目、日历、记忆、Agent、定时任务和至少一个 IM 平台的真实回归，再考虑安装向导默认推荐。观测队列积压、SQLite busy/lock 次数、单进程事件订阅数、任务重试数、数据库文件大小和备份耗时。Admin、多租户和多实例回归只在 `standard` 模式执行。
...
| 轻量模式被误用于公开多用户服务 | 身份、性能和可靠性边界不足 | 启动状态和文档明确单用户限制，禁用 Admin/多租户入口并拒绝第二进程。 |
...
2. 是否接受轻量模式单进程，从而牺牲独立 worker 的隔离和高并发？
3. 是否需要提供 PostgreSQL 导出到 SQLite 的迁移，还是只支持新建轻量实例？
4. 是否把 Redis 彻底从轻量版依赖中移除，还是保留一个可选 Redis 用于标准版的高质量实时广播？
...
- [ ] `ARCH6-008` 增加轻量模式安装/启动检查、单用户初始化、数据目录初始化、单实例保护和本地状态诊断；责任边界：`backend/start.sh`、`backend/docker-entrypoint.sh`、`app/local/`；验收：无 PostgreSQL/Redis 的干净环境可完成启动，不暴露 Admin/多租户入口，错误提示包含修复动作和能力边界。
- [ ] `ARCH6-009` 提供显式数据备份、导出/导入和恢复校验工具；责任边界：`backend/scripts/`、Storage 服务；验收：测试数据恢复后用户归属、文件元数据、会话、任务和索引校验通过，敏感数据不进入日志。

### Phase 4：完整验证与发布

- [ ] `ARCH6-010` 完成轻量模式 Web、文件、项目、日历、画布、记忆、Agent、定时任务和 IM 真实回归，并完成标准模式 Admin/多租户回归；责任边界：测试与 devserver；验收：两套模式的专用测试集均通过，记录资源占用、队列延迟和已知限制。
- [ ] `ARCH6-011` 更新安装文档、轻量本地状态提示、标准 Admin 提示和部署矩阵，确认轻量模式单用户单实例边界；责任边界：`README`、`docs/`、前端个人设置与标准 Admin；验收：新用户无需手动配置 PostgreSQL/Redis 即可完成轻量启动，切换标准模式前能看到备份和迁移说明。
