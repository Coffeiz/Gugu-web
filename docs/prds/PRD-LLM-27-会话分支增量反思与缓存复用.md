...
状态：实施中（2026-09-20 增加 owner 闲置反思与共享 TTL；2026-09-21 缩短为 3 分钟）
...
- **闲置冲刷**：owner worker 扫描最后活动满 3 分钟的 session；快照不可用时从已持久化历史重建最小 append 输入。群/群友游标扫描沿用同一 TTL 策略，既有 scope batch 仍按各自消息范围构建历史。
