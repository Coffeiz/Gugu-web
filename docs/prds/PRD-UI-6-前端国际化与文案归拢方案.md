...
| 能力/结果 | 状态 | 说明 |
|---|---|---|
| i18n runtime 与语言切换 | 🔲 待评估 | 当前没有统一的 i18n 实例、locale 注册表或语言设置；第一版目标为中文、日文、英文 |
| 前端界面文案归拢 | 🟡 部分完成 | 大量中文仍直接写在 Vue 模板、脚本、路由 meta 和工具函数中 |
...
| 翻译回归验证 | 🔲 待评估 | 当前只有业务功能和视觉测试，没有 key 完整性、fallback 或语言切换测试 |
...
- [ ] `UI6-001` 建立前端固定文案清单并区分 UI message、用户 content、协议值和第三方原文；验收：每条候选文字都有归属和迁移结论。
- [ ] `UI6-002` 确定 `zh-CN`、`ja-JP`、`en-US`、机器语言映射、runtime 依赖和用户偏好字段；验收：形成唯一 key 命名、自动适配优先级与 fallback 规则。
