...
> 当前反思触发阈值以 [PRD-IM-11](./【已完成】PRD-IM-11-群成员长期记忆.md) 为准。本 PRD 的 Phase 2 冻结参数记录首版实现，后续群聊 1 小时活跃窗口、15 分钟空闲收束和私聊逐回合触发均已被新策略替代。
...
#### 2.3.3 首版触发参数（历史决策，已被 PRD-IM-11 替代）
...
| @ 咕咕并进入 Agent | 立即写入 | 群消息累计 50 条触发群级反思 | 群友记忆由同一批 50 条消息的 member-batch 维护 |
| 进入 Agent 且使用工具 | 立即写入 | 群消息累计 50 条触发群级反思 | 群友记忆由同一批 50 条消息的 member-batch 维护 |
| owner 在群中发言 | 立即写入 | 群消息累计 50 条触发 | 群内 Owner 缓冲跟随同群阈值冲刷；未满阈值时空闲 3 分钟收束 |
| 私聊 / Web | 累计到网页/私聊反思触发阈值；未满阈值时空闲 4 分 30 秒收束 | 不适用 | 私聊对象仍写入隔离的 platform-user scope；群聊成员只由完整群上下文的 member-batch 维护 |
| 反思失败 | 不影响回复 | 保留数据库消息，下次重试 | 保留数据库消息，下次重试 |

网页和所有私聊对象共用可配置的反思触发阈值（默认 10）；群级、群友批量和群内 Owner 反思共用 50 条群消息阈值。两类 scope 均在最后一轮/条消息后空闲 4 分 30 秒时收束未达阈值的内容。群级任务和成员批任务使用独立游标，任务失败按退避时间补偿重投。反思和周期整理都不能阻塞当前回复。
