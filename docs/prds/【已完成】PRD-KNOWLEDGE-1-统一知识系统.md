...
> 状态：✅ 已完成（核心范围）；来源定期核验和时间降权列为后续质量优化
> 创建：2026-08-04
> 最近更新：2026-08-25
...
| Phase 2：知识捕捉与更新 | ✅ 已完成 | 新增 `save_knowledge` 明确保存入口，并接入自动反思候选、同主题更新和来源可信度约束。 |
| Phase 3：知识检索工具 | ✅ 已完成 | `search_memory(source=knowledge)` 已接入现有 UnifiedRecallService、Rust/Python lexical cache、scope 过滤、置信度和统一预算。 |
| Phase 4：反思、去重与来源维护 | ✅ 已完成 | 同主题历史、跨来源冲突、软删除、自动反思和索引失效已落地 |
| Phase 5：权限、向量和人工验收 | ✅ 已完成 | Knowledge 复用现有 scope、向量缓存和混合召回，并完成回归测试与 mock 场景验证 |
...
- [x] 增加 mock 样本：重复知识、用户纠正、网页冲突、流程知识、无命中和群聊越权。
- [ ] 来源失效、定期核验和基于时间的降权策略，列入后续质量增强，不阻塞本 PRD 完成。

> 核心范围已完成。来源核验周期、失效状态和时间降权属于独立质量增强，不纳入本版完成条件。
