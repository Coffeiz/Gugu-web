...
状态：Phase 0～4 已完成（Phase 4 安全验收：2026-08-29）
...
- [x] 验证数据库、备份、日志、URL、SSE 和工具结果均无明文凭据。
- [x] 验证跨用户无法读取、测试或删除其他用户凭据。
- [x] 验证解密失败不会静默回退平台 Key。
- [x] 验证主密钥轮换和凭据撤销。
- [x] 验证总开关关闭时任何入口均无法使用 BYOK，开启后可恢复且密文不丢失。

Phase 4 验收记录（2026-08-29）：新增 `backend/tests/test_byok_security_phase4.py`，6 项专项测试全部通过（`6 passed`）。测试使用临时密钥与内存数据库，不读取真实配置、不调用外部 Provider；覆盖信封密文边界、密钥旧版本解密、用户隔离、解密失败阻断平台配置回退、总开关门禁和元数据响应不含密文字段。
