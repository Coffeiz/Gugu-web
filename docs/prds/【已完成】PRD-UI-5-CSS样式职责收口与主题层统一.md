...
### Phase 1：入口与高风险主题域（✅ 已完成，2026-08-30）

- [x] `UI5-001` 盘点并标注现有 refinement、bridge、adoption 文件的唯一职责；已新增 `frontend/src/assets/styles/STYLE-OWNERS.md`，记录入口加载图、域归属和迁移期保留原因。
- [x] `UI5-002` 统一主题组件覆盖入口，并固定日历工具栏、GuguChat 高光/边框规则的加载顺序；已新增 `components/index.css` 与 `bridges/index.css`，`variables.css` 不再直接串联重复入口，现有视觉规则保持原 owner。

#### Phase 1 实施结果

- 主题语义映射先于组件覆盖，组件覆盖先于 bridge/adoption，加载顺序由 `variables.css` 单点维护。
- 日历工具栏、GuguChat 和文件工具栏的现有主题规则暂不搬迁，仅通过组件统一入口编排，避免本阶段引入视觉变化。
- 新增样式入口结构回归测试，锁定组件覆盖与跨 DOM bridge 不得被页面直接重复引入。

### Phase 2：全局与跨 DOM 树样式（✅ 已完成，2026-08-30）

- [x] `UI5-003` 将 `global.css` 中的第一批日历业务规则迁移至 `components/calendar.css`；验收：全局文件移除 `.cap-*` 与 `.cal-chip` 业务规则，结构/主题回归通过。
- [x] `UI5-004` 建立并标注统一 bridge 入口，明确日历、浮层和文件拖拽的跨 DOM 边界；验收：`bridges/index.css` 统一加载，保留的 bridge 规则有职责说明。

#### Phase 2 实施结果

- `global.css` 继续保留 reset、基础排版、通用交互 utility 和全局 glass 基础契约；日历共享业务规则已迁出。
- `components/calendar.css` 负责非 Teleport 的日历共享组件 paint；`bridges/calendar.css` 只负责跨 DOM 的日历菜单/浮层规则。
- 文件卡、通用弹层和基础输入控件的共享结构样式已继续按 owner 迁出 `global.css`；Runtime 规则仍保留在 adoption 域，不作为组件 paint 迁移。
- 本阶段不改变选择器内容，只改变统一入口下的文件归属；主题 refinement 与 Teleport bridge 的边界保持不变。

### Phase 3：组件样式与优先级治理（✅ 已完成，2026-08-30）

- [x] `UI5-005` 完成组件域治理：日历、文件卡、通用弹层和基础表单的颜色/阴影迁移至语义或组件 token，非 Runtime 主题层禁止强制覆盖 Runtime 的 `transform`、`transition`、`opacity`。
- [x] `UI5-006` 补齐 CSS ownership/主题入口静态回归测试；已通过 `typecheck`、`test:run`、`build` 和 devserver 路由冒烟。

#### Phase 3 实施结果

- 日历、文件卡、通用弹层和基础表单 CSS 不再直接持有原始颜色值，最终颜色由语义/组件 token 提供。
- 新增测试会阻止组件 CSS 重新引入颜色字面量，并阻止非 Runtime 主题层通过 `[data-runtime-*]` 强制改写 Runtime 管理属性。
- Runtime adoption 中有明确代理职责的 `background`、`border`、`opacity` 和阶段规则保留，不将运行时状态错误迁入主题层。
