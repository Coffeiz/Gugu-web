...
### IM 接入时（飞书 / QQ / 微信，可选）
- **Bot 凭据**：你填写的 App ID 和 App Secret（微信为 iLink bot token），用 AES-256-GCM 加密后存储，仅用于维持机器人连接
- **消息内容**：你通过飞书 / QQ / 微信发给咕咕 Bot 的消息和文件（含语音 / 音视频），经服务器中转后处理，**处理完成后不会额外留存副本**（对话记录与网页端共用同一份存储；语音另存独立 `.voice/`、30 天自动清理）
...
这些文件存储在你账户下的私有目录中，**其他用户无法访问**。记忆内容仅用于让咕咕更了解你，不会用于训练 AI 模型，也不会对外共享。你可在聊天里随时用 `/memory` 查看咕咕记得你哪些事、`/forget <内容>` 让它忘掉某条。
