...
- 画布元数据；
- 节点和画布项；
- 节点布局的有效尺寸、默认尺寸和推荐间距，供 Agent 排布时避让卡片；坐标按左上角解释，放置/移动不得造成节点矩形重叠；默认边缘安全距离为 150px，采用中心点排布时至少保持 750px 中心距；
- 连接关系；
- 最后一次画布视口；
- 截断和权限提示。
...
- [ ] “列出我的画布”能返回摘要；
- [ ] “搜索画布里的发布便签”不会返回普通时间流笔记；
- [ ] “找一个可以放入画布的发布项目”只返回项目/文件/活动；
- [ ] “放到当前视野中央”位置正确；
- [ ] “放到某节点右边”不会覆盖已有节点；
- [ ] “把文件和项目连起来”会先解析对象再创建连接；
- [ ] 模糊画布名称时会询问，不会写错画布；
- [ ] 删除连接和删除便签会请求确认；
- [ ] 群聊无授权时不能读取私人画布。
...
- [ ] 咕咕创建的画布便签在网页即时出现；
- [ ] 咕咕放入的项目、文件、活动卡样式与手动放入一致；
- [ ] 节点位置与 camera 缩放无关，缩放后仍在正确世界位置；
- [ ] 移动、折叠、置顶后刷新页面状态保持；
- [ ] 咕咕创建的连接与手动连接显示一致；
- [ ] 从画布移除节点不会删除原项目或文件；
- [ ] 删除画布便签后不会残留幽灵卡片或连接线；
