...
- ✅ 微信 iLink 引用咕咕自己发过的历史消息时，`message_item`（type=0）实测只有 `msg_id`/`create_time_ms`/`update_time_ms`/`is_completed`/空的 `button_item_list`，不带原文，iLink 客户端（gugu 和 QwenPaw 均如此）也没有按 `msg_id` 反查内容的接口——平台限制，不是解析代码漏字段。占位文案已从容易误导的「[非文字消息]」改成「引用了机器人自己的历史消息，微信没有提供可取回的原文」。引用图片单独确认过是下载地址问题（`encrypt_query_param` vs `full_url`），已修复；引用用户自己发的纯文字消息未见异常。
