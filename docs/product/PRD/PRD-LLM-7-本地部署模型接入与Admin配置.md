...
| 自动化测试与 devserver 验证 | 🟡 部分完成 | Provider 基础测试已覆盖；原生 Ollama 流式、工具调用和 Admin API 仍需补充专门测试。 |
...
- llama.cpp、vLLM、其他 OpenAI 兼容服务的本地部署入口；
- 统一的 `deployment_mode` / `local_runtime` 配置模型；
- 流式、工具、结构化输出、思考和视觉能力的逐项自动检测；
- 检测结果的模型/地址绑定、时间记录和能力人工覆盖；
- 本地部署场景的完整 Admin/API/原生驱动回归测试。
...
当前已验证：Provider、驱动和本地部署 Admin 回归测试合计 37 项通过，覆盖 Ollama 地址解析、原生请求构造、思考参数、结构化输出声明、本地运行时默认地址、能力覆盖和能力检测结果持久化。
原生 Ollama 流式和工具调用仍需在 devserver 连接真实服务完成手测。
...
- [ ] 补充原生 Ollama 流式调用、工具调用和工具结果回传的真实协议模拟测试。
- [ ] 补充 Admin 模型列表、连通性测试和密钥脱敏接口的完整 HTTP 层测试。
- [ ] 补充本地运行时错误分类测试：连接失败、模型不存在、参数不兼容、能力未启用。
- [ ] 补充多模型切换后能力检测结果不复用的端到端回归测试。
