...
> 状态：简介目录与两阶段工具声明已接入；RAG 仅作为后续工具候选推荐优化，暂不参与声明、Schema 生成或执行。
> 创建：2026-08-22
> 最近更新：2026-08-23
...
| Phase 2：能力目录基础设施 | ✅ 非 RAG 部分完成 | 已提供能力快照、授权交集、可替换 selector 和兼容模式；Capability RAG 索引、候选召回、排序和每轮推荐暂不实施，统一后置到 Phase 5。 |
...
| Phase 4：迁移、观测与回归 | 🟡 基础稳定化完成 | 已完成旧字段清理、Admin 能力目录、Provider Schema parity、脱敏诊断和关键行为回归；独立基线报告仍待后续性能任务。 |
| Phase 5：Capability RAG 与推荐 | 🔲 后置 | 与 `PRD-RAG-1` 联动，实现工具推荐排序、推荐原因、命中率观测和灰度；不缩减授权工具目录，不负责 Schema 硬过滤。 |
| Phase 6：插件/社区扩展 | 🔲 后续 | 在注册制与 RAG 稳定后，再开放外部包、签名和隔离加载。 |
...
| RAG 依赖项 | Capability RAG 索引、BM25/Embedding、每轮软推荐、推荐命中率评估 | Phase 5，依赖 `PRD-RAG-1` | 当前不做每轮推荐，不改变现有工具集合和调用路径。 |
...
| 测试 | 已覆盖 Tool JSON Schema 注册/dispatch、Provider Schema 保持不变、Capability Registry、selector、按需注入和 Skill 使用标记回归。 | Phase 4 继续补齐 Provider parity、权限和 emergency switch；RAG 召回质量归 Phase 5。 |
...
现有 Tool/Skill → Capability Adapter → 统一快照
    ...
    → [Phase 5] Capability RAG 工具软推荐 → Agent 声明后按需注入 Schema
```
...
- [Phase 5] Capability RAG 结合用户输入、当前会话、平台和工作区，为已授权工具生成推荐顺序；权限仍由 Runtime 独立执行硬校验。
...
完整工具 Schema 逻辑上属于动态候选，技术上必须继续放在 provider 原生 `tools` 参数中，不拼成普通 Prompt 文本。Skill 正文不属于候选集合，只能通过 `use_skill` 等明确加载路径进入动态上下文。
...
tail（后置的本轮推荐提示）
  └─ [Phase 5] 可放本轮 Capability RAG 推荐的工具名、短描述和推荐理由
...
- tail 是 [Phase 5] 本轮 UI/Prompt 中的筛选结果；当前不执行每轮推荐。
...
### FR-REG-6：按需注入完整工具 Schema（✅ 已完成）

首轮只注入工具简介目录和固定的 `declare_tools` Schema。模型声明本轮需要的工具后，Runtime
执行权限/注册校验，并在下一轮 provider 请求中注入声明工具的完整 Schema。`declare_tools`
在后续轮次继续保留，允许同一个 Run 重新声明工具集合。

RAG 不属于这条链路的前置条件；未来只需把 RAG 推荐结果作为声明前的优先提示，不改变工具
可见目录、工具 handler、确认门或 history 协议。

正式 Agent Loop 只接收动态候选工具的完整 provider Schema：
...
- 如果模型请求了尚未声明的已注册工具，仍由 `declare_tools` 在下一轮注入其 Schema；这不是 RAG 漏选重试，也不依赖推荐结果。
- 如果模型传入了不存在的工具名，继续返回现有未知工具错误。
- Schema 缓存按 `tool name + api_format` 计算 digest，避免每轮重复序列化。

工具声明的生命周期以“单次模型请求”为单位，而不是以整个 Run 为单位：

- 每次调用模型前都可重新计算当前请求的推荐顺序和已声明工具。
- Agent 进入新的任务阶段或工具结果暴露出新需求时，可以重新声明并刷新 provider 的 `tools` 参数。
- 刷新时以 Runtime 当前声明和授权视图为准，不把上一轮推荐当作不可变全局状态。
- Run 内可以保留任务状态和工具调用历史，但不能把上一轮候选自动带入下一轮。
- 如果当前请求仍然需要同一个工具，selector 可以重新选中它并重新声明 Schema；这属于新一轮匹配，不是候选继承。
...
| `backend/agent/capabilities/models.py` | Phase 1 | 定义 `CapabilityMeta`、`CapabilityRef`、`CapabilitySnapshot`、`SelectedCapabilities`；首版不包含版本字段。 |
...
| `backend/agent/capabilities/selector.py` | Phase 2 / Phase 5 | Phase 2 提供可替换 selector 和兼容模式；Phase 5 消费 Capability RAG 推荐结果，不实现 BM25/Embedding，也不裁剪授权工具。 |
| `backend/agent/capabilities/injector.py` | Phase 2～3 / Phase 5 | 当前生成完整短描述目录和 Provider Schema 接口；Phase 5 接入推荐提示，不把推荐结果当作候选硬过滤。Skill 正文只服务显式 `use_skill`。 |
| `backend/agent/capabilities/diagnostics.py` | Phase 4～5 | Phase 4 计算目录/Schema 字符数、digest 和 token 估算；Phase 5 增加推荐、召回和命中指标。 |
...
| `backend/tests/test_capability_selector.py` | Phase 2 / Phase 5 | Phase 2 测试授权全集保留；Phase 5 增加推荐顺序、推荐失败和权限边界测试。 |
| `backend/tests/test_capability_injection.py` | Phase 2～3 | 目录、完整 Schema、显式 Skill 正文加载和未加载工具扩展测试。 |
...
| `backend/agent/im/permissions.py` | Phase 4 / Phase 5 | Phase 4 收敛统一授权视图；Phase 5 为 RAG 提供授权视图，不在 RAG 或 selector 中复制权限判断。 |
| `backend/agent/runner.py` | Phase 2～3 / Phase 5 | 当前完成注入上下文传递；Phase 5 再传递推荐顺序和推荐诊断，不改变声明与执行链路。 |
...
| `backend/agent/runtime/loopscope_trace/context.py` | Phase 4～5 | Phase 4 记录能力目录和 Skill 正文来源；Phase 5 增加推荐来源，不重复记录完整 Schema。 |
| `backend/agent/runtime/loopscope_trace/hooks.py` | Phase 4～5 | Phase 4 已记录目录/选中工具的脱敏数量、字符数和 digest；Phase 5 增加推荐耗时、推荐命中和声明结果。 |
...
静态 Prompt 末尾注入短描述目录
        ↓
[Phase 5] Capability RAG 根据当前请求生成本轮 tail 工具推荐
        ↓
声明工具后从本轮 tail 展示中去重（Phase 5）
`use_skill` 按需追加 Skill 正文
Provider Adapter 通过原生 tools 参数注入当前工具集合的完整 Schema
        ↓
Agent Loop
        ├─ tool call → registry.dispatch()
        ├─ use_skill → 加载 Skill 正文
        └─ 未声明工具 → 由 declare_tools 补充 Schema
```

当前阶段不执行每轮候选召回，不生成 tail，也不额外调用 LLM 做工具推荐。Phase 5 接入后，候选才必须在每轮请求前重新召回；候选 Schema 不能写入静态 Snapshot。即使未来增加关闭深度思考、低 token 的 capability routing 请求，也属于 Phase 5 的可选优化，不是注册制的前置依赖。
...
| `schema_chars` | 动态候选中实际注入完整工具 Schema 字符数 |
...
- 推荐命中率、推荐为空率和安全回归可解释；推荐失败不影响授权工具声明。
...
| 模型需要未声明工具 | 通过 `declare_tools` 在下一轮补充已授权工具 Schema |
...
- ✅ Snapshot 常驻短简介目录；首轮只提供固定 `declare_tools` Schema。
- ✅ Runtime 校验模型声明的工具名，并在下一轮只注入声明工具 Schema；声明入口在后续轮次保留以支持换工具。
- ✅ 工具候选 tail 的 RAG 推荐、排序和去重仍后置到 Phase 5，不影响当前声明协议。
...
完成标准：首轮不注入业务工具完整 Schema；声明后只注入已授权且已注册的工具 Schema；工具调用和结果仍按原协议进入 history；未声明工具不能被 provider 调用。
...
### Phase 5：Capability RAG 与每轮软推荐（后置）
...
- 🔲 将推荐结果按相关性排在完整授权工具目录前，并记录推荐理由；不得设置会丢失工具的候选上限。
- 🔲 启用每轮推荐 tail；声明后的完整 Schema 仍只由 `declare_tools` 决定。
- 🔲 评估推荐命中率、推荐为空率、Schema 节省、P95 延迟和安全回归，完成灰度后再打开默认开关。
...
完成标准：能力注册表成为工具/Skill 元数据唯一来源；旧字段、重复目录和临时路由代码均有删除记录；声明链路可通过 emergency switch 回滚到全量 Schema。
...
- Phase 5 使用 shadow mode，不改变实际工具注入。
- Phase 5 完成后增加配置开关，支持按用户、平台或 provider 灰度。
...
| 插件带来不可信代码 | 数据、文件和系统安全风险 | Phase 6 前不开放远程代码加载；插件 manifest、签名和沙盒另立安全评审 |
