...
- 状态：P0 已完成代码收口；P1 已完成执行链路与容器临时空间代码，生产部署验收仍进行中
...
- 后续开放 `egress` 时优先使用受控 egress proxy，而不是直接打开完整 Docker 网络。
...
- [x] 将旧 `personal/workspace` scope 的运行时分叉收敛为 `sandbox`；工作区只保留默认目录职责，旧字段仅等待数据库清理窗口移除。
...
- [x] 统一 Shell 配额计量入口，覆盖 Shell 根目录创建、启动前检查、运行中 sandboxd 监测和清空后的空间回收；文件库上传/下载继续使用文件服务配额，跨存储域账本移入 Phase 6。
...
- [x] 增加 `none/egress/internal` 网络 profile 契约；当前实现和生产门禁只允许 `none`，后续 egress 通过受控代理。
- [x] 多租户部署禁用 system executor，仅本地自托管且 `deployment_mode=local` 时允许显式开启。
...
- [x] 将网络策略提升为沙盒配置契约；当前只允许 `none`，未来 `egress` / `internal` 必须经过独立的受控代理与内网拒绝策略，不能直接打开 Docker 网络。
...
- [x] 容器执行器已成为生产 sandbox 默认后端；运行时已停止消费旧 `personal/workspace` scope 和 `shell_scope` 授权语义，数据库历史字段正式删列移入 Phase 6 迁移窗口。
...
- [x] 完成绝对路径、软链接、挂载边界、用户切换、容器重建、取消和异常恢复的核心回归测试；旧 scope 不再参与运行时授权，数据库历史字段正式删列留到 Phase 6 迁移窗口。
