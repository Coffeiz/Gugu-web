...
- [x] 发一张图片给咕咕（正常发送，非草稿），刷新页面，确认气泡里的图片正常显示（走的是 DB 查询路径，不是靠 objectURL）。**已在 devserver 验证**（2026-08-09）。
...
- [x] 删除这条消息所在的整个会话，检查存储里对应的 storage_key 确实被删除了（不再残留孤儿字节），`chat_attachments` 表里对应行也应该消失。（`storage_key` 共享场景——两条消息共用同一份字节、删一条不影响另一条——目前只有自动化测试覆盖；PRD-IM-9 落地前没有可以手动触发共享场景的入口，等 IM-9 上线后再补一条手动验证。）**已在 devserver 验证**（2026-08-09）：删除会话后 `chat_attachments` 表对应行、物理文件均已消失，backend 日志无异常。
- [x] 上传一张图片但**不发送**（只调用暂存接口），等草稿 TTL 过期后跑一次清理任务，确认这个从未发送的草稿被清理掉了；再上传一张、正常发送、发送成功后手动改数据库把这张图的 mtime/created_at 改到 TTL 之外，确认它**不会**被草稿清理误删（因为已经是 `attached` 状态）。**已转成自动化集成测试**：`tests/test_chat_attachments_ownership.py::test_e2e_draft_upload_expires_but_attached_survives`——走真实 `upload_attachment` 接口 + 真实 `attachment_gc.sweep_draft_attachments()` 任务函数（不是直接调内部函数/手动改库），核心逻辑不必每次手动跑，不再需要手测。
