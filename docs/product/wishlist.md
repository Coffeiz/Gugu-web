...
> 核实说明（2026-07-02）：对照 `CHANGELOG.md` 核实后，"规划中"里的天气与语音回复两项**代码里已有部分基础**但尚未做成清单描述的形态，予以细化标注，未直接标"已完成"；其余条目状态准确，未改动。
...
- ~~**反馈信号采集器**~~ ✅（2026-07-02 已落地+验证+部署:①feedback 枚举采集 ②关系温度 ④时长锚点+禁自估红线;③lens 写回等 feedback 攒 1–2 周）
...
- **客户管理页面** — 后端 `clients` API 已完成，前端无页面。**核实：截至 0.15.1 仍未上线**，与 `mvp.md`/`overview.md` 的待办一致
- **思维画布** — 创意节点图，可挂文件附件，侧边栏入口已预留。**核实：仍未开发**，`frontend/src` 无对应视图
...
- **咕咕回语音（双向语音条）** — 用 MiniMax T2A 语音合成（`POST /v1/t2a_v2`）让咕咕也能"说"，渲染成 AI 侧语音条（语音条 UI + `stage_voice` 已就绪）。注意：响应音频是 **hex 编码**（非 base64），部分账号需 GroupId，key 可复用池里的 MiniMax；何时用语音回可加开关或让咕咕自己判断。MiniMax 无 ASR，"听"仍走 mimo。**核实：仍未实现**，代码中未找到 T2A 调用
