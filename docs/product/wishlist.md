...
> 核实说明（2026-09-02）：按仓库当前 `1.0.0`、路由、前后端实现和 `CHANGELOG.md` 重新核对。思维画布已从规划项移入已完成；客户管理仍只有 API/Agent 能力，天气常驻组件、语音回复和团队协作仍未完成。生产部署、真实用户稳定性和运营数据不以代码存在作为完成依据。
...
- **MVP 内测与生产验收**（当前仓库版本为 `1.0.0`；后续功能实现持续进入 Unreleased，生产部署、备份恢复、压测和真实用户稳定性仍需现场验收，详见 [`MVP.md`](MVP.md)）
