...
- [ ] 接力：剩余集中在 5 个巨型视图（Calendar 442/Files 295/GuguChat 227/Admin·Agent 224/ProjectModal 184 ≈ 84%）+ Projects/Dashboard 中型组件，按约定随功能迭代逐个转逐个入档（GuguChat 228 错、巨型视图是大头，随功能推进）；④ Calendar `CalendarEvent`、Mind `NoteEditor` TipTap any（Mind 待并发思维面板重构稳定后再碰）
