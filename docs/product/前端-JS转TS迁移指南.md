...
- [x] 底座补齐 + 组件层起步 ⑤（2026-07-11）：补 auth/useLazyThumb 两个漏网基座；首批 6 个叶子组件入档（NavItem/PdfViewer/FeedbackModal/FileInfoPopup/DatePicker/ImageViewer）。**棘轮累计 38 源文件，底座 100% strict-clean**
- [ ] 接力：继续把 common/viewers 及各视图组件逐个入档（GuguChat 228 错、巨型视图仍是大头，随功能推进）；④ Calendar `CalendarEvent`、Mind `NoteEditor` TipTap any（Mind 待并发思维面板重构稳定后再碰）
