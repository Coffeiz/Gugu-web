...
打开 <http://localhost:9595> 即可使用。**不绑卷也能直接跑**：镜像声明了 `/data`、`/config` 卷，什么都不挂时会自动创建 Docker 匿名卷兜底——重启、崩溃都不会丢数据；入口检测到数据落在容器临时层（未挂任何卷）时会拒绝启动并给出指引。但匿名卷跟容器实例走：通过 NAS 面板「更新镜像」重建容器时会拿到全新的空卷，旧数据滞留在旧卷里无法自动接上（启动日志会在匿名卷模式下显著警告这一点）。**长期使用请绑定宿主机目录**：`/data` 保存数据库、用户文件与记忆，`/config` 保存 Admin 配置，`-v` 绑定后升级镜像、重建容器数据都不丢。
