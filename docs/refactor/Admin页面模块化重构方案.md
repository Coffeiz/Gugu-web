...
当前进度：Phase 4、5 的状态层已收口；LLM 编辑器模板、记忆展示模板和历史迁移注释仍需在本阶段删除/下沉，不能提前标记完成。
...
当前进度：已将 `Quota` 的全局配额、用户列表、编辑保存和格式化逻辑收口到 `Quota/useQuotaAdmin`；`StorageAudit`、`Config` 及其余页面待继续处理。
