...
验证（真实 run，预置 4 条大历史后触发压缩）：重放 LLM 调用 0 次；水位停在保留窗口边界、未越过本轮消息；summary 行正常落库；此前每次压缩后必然出现的「in≈3 万、cr=0/128」重放用量行不再出现。全量后端测试 2470 passed（含新增回归 3 项）。
...
- [x] baseline 压缩的重放调用已去除（复用 run 内摘要 + 只重算水位），`compaction` 场景最大的冷输入来源消除；聚合命中率随新数据积累回升。
