...
**Architecture:** 用 `web_private_reflection_threshold` 替换 Owner 专属配置名，并在配置模型加载时兼容旧 `reflection_threshold`。私聊 IM 成员任务按持久 cursor 的 `pending_agent_count` 聚合，达到阈值后创建持久反思任务；群 scope、member-batch 和群内 Owner 缓冲统一使用 `GROUP_MESSAGE_THRESHOLD=50`，移除群级 1 小时单独触发。闲置扫描继续补齐未反思范围，主会话完整历史前缀和幂等任务保持不变。
...
- 旧配置中若只有 `reflection_threshold`，加载时将其映射为新字段；显式新字段优先。
...
- 老配置只提供 `reflection_threshold`：新字段必须继承旧值且不能触碰运行配置文件；由 Task 1 的配置兼容测试覆盖。
...
- Legacy input `reflection_threshold` populates `web_private_reflection_threshold` only when the new field is absent.
- Frontend reads and saves `agent.web_private_reflection_threshold` and labels it “网页/私聊反思触发阈值”。

- [ ] **Step 1: Add config compatibility tests**
...
assert AgentBehaviorSettings.model_validate({"reflection_threshold": 7}).web_private_reflection_threshold == 7
...
- [ ] **Step 2: Run the focused config tests and confirm the legacy-field case fails**
...
Expected before implementation: failure because `web_private_reflection_threshold` is not yet modeled.

- [ ] **Step 3: Add the new setting with legacy input mapping**

Add a `model_validator(mode="before")` on `AgentBehaviorSettings` that copies legacy `reflection_threshold` into `web_private_reflection_threshold` only if the new field is absent. Validate the new field in the range 1–100. Rename `_owner_reflection_threshold` to `_web_private_reflection_threshold` and update all private Owner call sites to read the new setting. Do not write `config.override.json`, `.env`, or other runtime config files.

- [ ] **Step 4: Rename the admin field and localized copy**
...
Run the command from Step 2. Expected: all selected tests pass, including precedence of the new field over the legacy field.
...
- [ ] **Step 1: Add failing threshold aggregation tests**
...
- [ ] **Step 2: Add failing idle fallback and duplicate-scan tests**
...
- [ ] **Step 3: Run the focused private-reflection tests and verify expected failures**
...
- [ ] **Step 4: Implement persistent pending-turn aggregation**
...
- [ ] **Step 5: Use the renamed setting in the web/Owner buffer**
...
- [ ] **Step 6: Run focused private-reflection tests**
...
- [ ] **Step 1: Add failing group threshold boundary tests**
...
- [ ] **Step 2: Add failing group-owner threshold test**
...
- [ ] **Step 3: Add failing group idle settlement test**
...
- [ ] **Step 4: Run focused group tests and verify expected failures**
...
- [ ] **Step 5: Unify group-trigger behavior on the shared 50-message constant**
...
- [ ] **Step 1: Update PRD trigger table and idle interval**
...
- [ ] **Step 2: Add a concise user-visible changelog entry and detailed Chinese devlog**

Add a short entry to `CHANGELOG.md`; put implementation rationale, configuration compatibility, old/new paths and verification evidence in the new devlog. Do not include real account names or conversation bodies.

- [ ] **Step 3: Review documentation against the approved spec**
...
- [ ] **Step 3: Run source checks and inspect the final diff**
...
- [ ] **Step 4: Report verification and remaining risks**
