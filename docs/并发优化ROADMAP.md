...
| **P1 worker 并发** | 🔜 **进行中** | ①⑦ + 并发热配 ✅ 已上线 + 端到端验证通过；剩 地基 B + 配额降级（地基 A 跟 ④ 后推） |
| P2 用户功能 | ⬜ | 客户页 · 主动触达 · 服务/Bot 管理页 |
| P3 按需扩展 | ⏸️ | 多 key / 多 worker，由埋点数据触发 |
| P4 远期 | ⬜ | 思维画布 · Planner · 多 Agent · 团队版 |

**下一步**：①⑦ + 并发热配已上线并端到端验证（见[压测](并发压测结果.md)）。剩 **地基 B（清 79 个死 consumer）+ 配额能力降级**；地基 A 跟 ④（web 多 worker）一起后推、由埋点数据触发。
...
| B 死 consumer 清理 | 🔜 **近期** | `CONSUMER={host}-{pid}` 重启换 pid，旧的留组里累积（**现已堆 79 个**，不丢消息但越攒越多） | 稳定名 / 启动 `XGROUP DELCONSUMER` 清 idle |
...
- **P2 用户功能**：给用户加看得见的功能——客户管理页、到点主动提醒、后台服务/Bot 管理页。
...
- **地基 B · 清死 consumer**：worker 每次重启换个名字注册到 Redis 消费组，**旧名字永远留着**（现已堆 79 个）。改用稳定名 + 启动清理，**不再堆垃圾**。
