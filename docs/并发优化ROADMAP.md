...
| **P1 worker 并发** | 🔜 **进行中** | ① 代码完成 + 压测通过，待 worker 重启 + 端到端；⑦ 待做 |
| P2 用户功能 | ⬜ | 客户页 · 主动触达 · 服务/Bot 管理页 |
| P3 按需扩展 | ⏸️ | 多 key / 多 worker，由埋点数据触发 |
| P4 远期 | ⬜ | 思维画布 · Planner · 多 Agent · 团队版 |

**下一步**：① worker 重启激活 + 端到端验证 → 加 ⑦（超时 + 429 重试）→ `_MAX_CONCURRENCY` 设 **16**（单 key 实测安全上限，见[压测](并发压测结果.md)）。
...
- 🔜 ⑦ 慢尾兜底：LLM 调用超时 + **429 重试退避**（拐点近、429 全或无，**必须项**）
