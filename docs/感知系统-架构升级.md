...
| **v0.6+** | **错读需求案例收集器**（反思「感知误读」顺带吐脱敏 `miss{read_as/actual/pattern}` → Redis `perc:misread_cases` **+ 持久 md `_analytics/misread.md`（跨 Redis 不丢）+ admin 下载端点 `/admin/perception/misread/export`**，`pattern` 只到模式层、80 字截断兜底） | `prompts/reflection.md` + `memory/{reflection,store}.py` + `api/v1/agent_perception.py`（详见 §11.5 步骤 1）|
...
## 11. 架构演进:从「stance 分类器」到「需求发现闭环」

> 2026-07-01 起草。本节是比 §2.6 更**上层的方向**，不是已落地的设计——记下来防忘，按「先攒数据」的纪律分步走（见 §11.5）。代码除「错读案例收集器」（§11.5 步骤 1，已落）外一律未动。

### 11.1 重心转移

v0.6 搭的是一个**前馈分类器**：`本轮 → 反思判 stance → builder 套模块 → 回复`，信息单向流过。它能干，但骨子里是「**应付场景**」——归类后套剧本。companion 太被动、复合态这些问题，根都在这：**分类压过了发现**。

方向是把重心从「分类器」挪到一个**带反馈、会学的需求发现闭环**：中心不再是「这轮是什么场景」，而是「**对这个人，此刻的需求到底是什么；我有多确定；不确定就去查清；猜错了就改**」。咕咕该有的是一颗**去发现的心**，而不是一张更全的场景对照表。

### 11.2 目标回路

```
                  ┌──────────── 每用户「需求模型」 ────────────┐
                  │ 这个人:开心时要分享 / 卡住时先骂再要方案 …  │  ← lens 重新瞄准「深层需求」
                  └───────────────────┬───────────────────────┘
                                      │ 用它来读
  本轮 → 推断需求 + 置信度 ───────────┤
   (反思,但问的不再是「哪个场景」,      │
    而是「TA 要什么、我多确定」)        │
            ┌─────────────┴─────────────┐
       够确定 → 按需相处                 拿不准 → 轻轻去问一句
       (stance / tone×substance 表达)    (「想说说就好,还是要我出主意?」
                                          ← 这一问 = 发现的心被看见)
            └─────────────┬─────────────┘
                          │ 回应落地了吗?
                  误判捕获(TA 纠正 = 我读错了需求)
...
v0.6 只实现了中间「按需相处」**一臂**，且用的是**通用、无置信度、不回写**的分类器。其余三臂（每用户需求模型、置信度分流、误判回写）= 「发现」，目前是缺的。

### 11.3 几乎全是「重新瞄准现有零件」，不是重写

| 回路环节 | 现状 | 改什么 |
|---|---|---|
| 每用户需求模型 | `lens`（瞄准「解读触发语」） | **重新瞄准「情境→深层需求」**；复用 lens 全套（per-user / 置信 / 衰减 / 纠错学习），只换它学的内容 |
| 推断需求 + 置信度 | 反思吐单值 `intent` | 改问法 → `{需求假设, 置信度, stance}`；**置信度是唯一真·新原语** |
| 按需相处 | stance→模块 | 加复合态 tone×substance 当表达层（待设计：≤1 语气 × ≤2 实质） |
| 拿不准就去问 | 无（只会猜） | 新增**低置信→轻探**行为（`discover.md`）+ 置信度闸 |
| 猜错了就学 | misperc 只打遥测 | **接回 lens 写回**（= 欠的 lens v2 反驳↓）；这条让「发现」真会学、不是口号 |

### 11.4 唯一真正新的原语:对「需求」的不确定性

反应式分类器不需要知道自己有多确定（反正都套剧本）。但一个会发现的系统**必须能表示「我现在不知道 TA 要什么」**——这才有资格去问、而非默默猜。**「知道自己不知道」就是那颗心的内核**，也是整套里唯一非靠重新接线、得新建的东西。

### 11.5 路线（数据优先，否则又变口号）

这套闭环**最吃真实纠错数据**，空转就是学噪声。从最便宜、最复利的那环先动：

0. **（已做）** stance + companion 松绑 → 让它跑。
1. **（已落）错读需求案例收集器**：反思检测到「感知误读」时顺带吐脱敏的 `miss{read_as, actual, pattern}`，单独收进 Redis `perc:misread_cases`（`pattern` 只到「模式层」、不含用户内容、80 字截断兜底），并**持久化进全局 md `_analytics/misread.md`（跨 Redis 不丢）+ 提供 admin 下载端点 `/admin/perception/misread/export`**。**这是攒燃料的第一步**——把「只有数字、没有原因」补成「有具体（脱敏的）、可留存可下载的原因」。
2. **接『误判 → lens 写回』**：把 misperc 信号真喂回 lens（最高杠杆，闭环一旦接上后面每环都有燃料；也是欠的 lens v2）。
3. **lens 瞄准「深层需求」**：有了写回通道再谈学什么。
4. **加置信度 + 低置信探问**：最像「发现的心」，但最易过头变审问，放最后、调最细。

复合态（tone×substance）在「同一句既要陪又要帮」的案例攒够时插进表达层，不抢主线。**prompt 里那句「你最在意搞懂 TA 真正要什么」留到闭环有牙了再加**——否则先立 flag 后空转。

### 11.6 红线

- **口号陷阱（最大风险）**：把漂亮话塞进 prompt 就以为做完了 → 咕咕「演懂你」而非真建了你的模型。判据：goal 必须**① 改变行为 ② 会学**，否则不算。
- **脱敏**：错读案例只留结构（read_as / actual / 抽象 pattern），绝不留用户原话 / 话题（对齐 §3.4 / `privacy.md`）。
- **发现欲要轻**：好奇不盘问；探问过头 = 审问，会毁掉刚拿到的「没压力」。
- **不自动重写控制流**：需求模型（lens）per-user、可逆、偏置不独裁，可自学；全局 Decision Policy 仍只人调（§3.3 红线不变）。

---

## 12. 北极星定稿:Person Model → Relationship（「理解人」而非「完成任务」）

> 2026-07-01，与用户深聊收敛。§11 的「需求发现闭环」是**机制**；本节定它的**终态形状**——学习对象不是需求，是**人**，最终是**关系**。这把咕咕的目标从「完成任务」挪到「理解人」。**这是北极星 / 定形状，不是新工程**——落地纪律见 §12.6，§11.5 数据优先路线一字不改。

### 12.1 一句话产品魂

> 「**我会越来越了解你，所以越来越知道什么时候该陪你、什么时候该帮你、什么时候该什么都不做。**」

留住用户的从来不是「它真聪明」，是「它真的越来越懂我」。这句把目标从「我帮你管项目 / 陪你成长」升级成了「理解人」——也是咕咕和通用助手的分水岭。
...
Goal（永不变）        希望用户过得越来越好 → 为此不断理解这个人
   ↓
Relationship（我们）  共同经历 / 默契 / 已建立的信任 ——「我们是谁」
   ↓
Person Model（你）    对这个人的理解（全是假设）——「他是谁」
   ↓
Current Context（现在）最近项目 / 聊天 / 情绪 / 事件 / 环境
   ↓
Need Hypothesis（此刻）我猜你现在需要 X + Confidence
   ↓
Behavior → Response → Feedback → 回写上面所有层
```

`Need = Person Model × Current Context`：同一句「我今天好累」，对不同人导出 分享 / 休息 / 鼓励 / 重新规划。**整个系统只有一族学习对象（人 / 关系），其余全为更新它服务。**

### 12.3 Person Model:存 Observation，不存 Schema
...
- **落地**：咕咕**已半在做**——`facts`（kind=observed/inferred + conf + ts）、`lens`（规则带 confidence、复现才提拔）就是 observation 雏形。升级 = 全面倒向「**观察为真、特质为派生**」。
- **一把必加的锁（成本）**：全读时实时推导 → 又贵又飘。所以 **observation 是唯一真相源；派生出的 trait 可缓存，但必须带「出处 + 可衰减 + 可推翻」，是缓存不是事实**——别让缓存悄悄变回 schema。

### 12.4 Hypothesis = 整个记忆的「写协议」（不只是读流程）
...
- 参数按证据类型分档：用户**明说**的 = 高置信、慢衰减（黏，但仍可改）；**推断**的 = 低置信、快衰减。**协议统一、参数分层**（= 现有 observed/inferred + 半衰期，只是要做到全层统一、显式）。

### 12.5 Relationship:最深一层、最难复制——也最危险
...
- ⚠️ **但它也是最容易翻车、最该上死锁的一层**：
  - **假亲密是所有失败里最毁的**——脑补一段没发生的共同经历、表演交情，比答错一个事实更伤信任。`persona.md` 的记忆边界（「绝不说你之前聊过 / 喜欢 X，除非记忆区真有……别为亲近脑补共同经历」）**正是为这层准备的**：Relationship 层**绝对继承**它，是全系统**最严的证据闸**。
  - **关系不能设计，只能挣**：最慢、最涌现的一层，从 `daily/memory`（流水→沉淀）里**随真实共同经历长出来**，不能预先建个「关系表」去填。
  - 它还**反向给全栈上色**：五年朋友读「好累」不同，不只因数据多，是因**共享的框架**。所以 Relationship 既是顶层（定相处方式），也是**贯穿解读的镜片**。

### 12.6 纪律不变（这是北极星，不是新工程）

- 全部**自下而上从证据长**：Person Model 从 observation 长、Relationship 从 daily/memory 长。**绝不 top-down 先画 schema / 关系表去填。**
- 这套层级是**给终点起名 + 定形状**，§11.5 数据优先路线**一字不改**——它就是「诚实地养出 Person Model、再养出 Relationship」的唯一长法。
- Relationship 是**最远的星**：N=1、几天历史时根本没关系可建。现在要做的，是让下层（observation 记录、daily/memory）把「关系的原材料」如实存住，关系**日后自然蒸馏**。
