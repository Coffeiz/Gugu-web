...
/**
 * CI 关键路径之三：GuguChat 悬浮窗——发消息收到确定回复、大窗会话列表、
 * 新建会话、收起/关闭。跟另外两条关键路径同一个原则：全部用
 * scripts/mock_llm_server.py 的固定回复（FIXED_REPLY），不接真实模型，
 * 结果确定、不受限流/网络影响。
 *
...
const FIXED_REPLY = '已完成（E2E 假 LLM 固定回复，仅用于验证定时任务执行链路）'

test.describe('GuguChat 悬浮窗', () => {
  test('发消息收到固定回复，刷新页面后会话内容还在', async ({ page }) => {
    await page.goto('/')
    ...
    await expect(chatWindow.locator('.msg.ai .msg-bubble', { hasText: FIXED_REPLY })).toBeVisible({ timeout: 15000 })
    ...
    await expect(page.locator('.chat-window .msg.ai .msg-bubble', { hasText: FIXED_REPLY })).toBeVisible()
  })
