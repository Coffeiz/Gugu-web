...
test('定时任务页面：可以创建自定义单次任务', async ({ page }) => {
  ...
  const onceBoundary = page.locator('[data-testid="schedule-once-boundary"]')
  await onceBoundary.locator('.dp-input').click()
  await expect(page.locator('.dp-popup:visible')).toBeVisible()
  const tomorrowButton = page.locator(`.dp-popup:visible .dp-day[aria-label="${tomorrowIso}"]`)
  if (!await tomorrowButton.count()) {
    await page.locator('.dp-popup:visible .dp-nav').last().click()
  }
