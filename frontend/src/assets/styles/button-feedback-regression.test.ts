...
describe('按钮乐观反馈全局契约', () => {
  it('按钮 token 默认提供乐观按压值，并保留组件级覆盖入口', () => {
    ...
    expect(tokenCss).toContain('--button-feedback-scale: .985')
    expect(tokenCss).toContain('--button-feedback-translate-y: 1px')
    expect(tokenCss).toContain('--button-feedback-opacity: .93')
    expect(tokenCss).toContain('--button-press-scale: var(--button-feedback-scale, .985)')
    expect(tokenCss).toContain('--button-press-translate-y: var(--button-feedback-translate-y, 1px)')
  ...
  it('所有原生 button 获得低特异性 active fallback，关闭时不生效', () => {
    ...
    expect(bridgeCss).toContain("html:not([data-button-feedback='off']) button:not(:disabled):active")
    expect(bridgeCss).toContain('scale: var(--button-press-scale, .985)')
    expect(bridgeCss).toContain('translate: 0 var(--button-press-translate-y, 1px)')
