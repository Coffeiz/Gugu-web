...
describe('主题 CSS 回归契约', () => {
  ...
  it('文件多选工具栏只由共享组件负责 paint，并锚定项目卡非滚动容器', () => {
    ...
    expect(fileSelectionToolbarVue).toContain('--file-selection-divider: color-mix(in srgb, var(--content-primary) 18%, transparent)')
    expect(fileSelectionToolbarVue).toContain('background: var(--file-selection-divider)')
