...
describe('导航 / popup / disclosure 结构回归契约', () => {
  ...
  it('主题组件覆盖和跨 DOM bridge 保持明确的统一入口', () => {
    ...
    expect(componentEntry).toContain("@import './card-actions.css';")
  ...
  it('技能与 MCP 的空状态通过 GuguChat 预填配置请求，页签同行提供新建操作', () => {
    ...
    expect(skillsIndex).toContain('class="skills-nav-action"')
    expect(skillsIndex).toContain('<SegmentedTabs')
    expect(skillsIndex).not.toContain('role="tablist"')
    expect(skillsIndex).toContain('<SkillsHome')
    expect(skillsIndex).toContain('v-show="route.name !== \'SkillsMcp\'"')
    expect(skillsIndex).toContain('<McpServersView')
    expect(skillsIndex).toContain('v-show="route.name === \'SkillsMcp\'"')
    expect(skillsIndex).not.toContain('<router-view')
    expect(skillsIndex).toContain(':create-request="mcpCreateRequest"')
