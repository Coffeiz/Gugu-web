...
<style>
.settings-popup { position:absolute; bottom:calc(100% + 8px); left:0; right:0; z-index:100; overflow:hidden; background:rgba(255,255,255,.44); border:1px solid rgba(255,255,255,.72); border-radius:var(--radius-md); box-shadow:inset 0 1px 0 rgba(255,255,255,.95),0 4px 16px rgba(0,0,0,.08); user-select:none; }
.settings-popup .popup-menu-item { position:relative; z-index:0; padding:9px 12px; border-radius:0; font-family:'PingFang SC','Segoe UI',sans-serif; color:#1e2028; }.settings-popup .popup-menu-item::before { content:''; position:absolute; z-index:-1; inset:-3px 0; background:rgba(255,255,255,.55); opacity:0; pointer-events:none; transition:opacity .15s ease; }.settings-popup .popup-menu-item.danger { color:#c84a4a; }.settings-popup .popup-menu-item:hover:not(:disabled) { background:transparent; }.settings-popup .popup-menu-item:hover:not(:disabled)::before { opacity:1; }.settings-popup .popup-menu-item.danger:hover:not(:disabled)::before { background:rgba(200,90,90,.1); }.settings-popup .popup-menu-sep { background:rgba(0,0,0,.06); margin:3px 0; }
