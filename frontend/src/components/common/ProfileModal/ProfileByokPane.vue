        ...
        <div v-for="group in groups" :key="group.value" class="byok-group">
          ...
          <div v-if="itemsFor(group.value).length" class="byok-card-grid">
            ...
            <div class="byok-form-grid">
              ...
              <div class="model-picker"><div class="model-picker-row"><input v-model="editors[item.id].model" class="form-input" :placeholder="group.value === 'speech_to_text' ? '语音模型名（可选）' : '模型名（可选）'" /><button type="button" class="pm-style-chip" :disabled="modelLoading" @click="fetchModels">{{ modelLoading ? '获取中…' : '获取列表' }}</button></div><Transition name="menu-pop"><div v-if="modelMenuOpen && editor?.id === item.id" class="model-options" @mousedown.stop><div v-if="modelError" class="model-option-hint err">{{ modelError }}</div><div v-else-if="!modelOptions.length" class="model-option-hint">暂无可用模型</div><button v-for="model in modelOptions" :key="model" type="button" class="model-option" @click="selectModel(model)">{{ model }}</button></div></Transition></div>
