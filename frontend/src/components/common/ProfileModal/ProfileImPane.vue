  ...
  <div class="pm-section">
    ...
    <template v-for="platform in platforms" :key="platform.key">
      ...
      <div v-for="bot in botsOf(platform.key)" :key="bot.id" class="pm-bot-item">
        ...
        <div v-if="platform.key === 'qqbot'" class="pm-bot-group-row"><div class="pm-field-desc"><span class="pm-field-name">群聊回应</span><span class="pm-field-hint">开启后，咕咕会回应群里 @机器人的消息</span></div><span class="pm-switch-wrap"><label class="switch sm"><input type="checkbox" :checked="bot.group_chat_enabled" @change="toggleGroupChat(bot)" /><span class="slider"></span></label><span class="pm-switch-label" :class="{ on: bot.group_chat_enabled }">{{ bot.group_chat_enabled ? '已开启' : '已关闭' }}</span></span></div>
        <div v-if="platform.key === 'qqbot' && bot.group_chat_enabled" class="pm-bot-group-row"><div class="pm-field-desc"><span class="pm-field-name">只响应 @</span><span class="pm-field-hint">关闭后会接收群里的普通消息，请确认机器人有对应的群消息权限</span></div><span class="pm-switch-wrap"><label class="switch sm"><input type="checkbox" :checked="bot.group_requires_at !== false" @change="toggleGroupRequiresAt(bot)" /><span class="slider"></span></label><span class="pm-switch-label" :class="{ on: bot.group_requires_at !== false }">{{ bot.group_requires_at !== false ? '已开启' : '已关闭' }}</span></span></div>
...
interface Bot { id: number; platform: string; name?: string; sandbox?: boolean; app_id?: string; enabled?: boolean; group_chat_enabled?: boolean; group_requires_at?: boolean; group_read_enabled?: boolean }
const platforms = [
