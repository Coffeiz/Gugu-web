  ...
  <div class="pm-section">
    ...
    <template v-for="platform in platforms" :key="platform.key">
      ...
      <div v-for="bot in botsOf(platform.key)" :key="bot.id" class="pm-bot-item">
        ...
        <div v-if="platform.key === 'qqbot'" class="pm-bot-group-row"><div class="pm-field-desc"><span class="pm-field-name">QQ 身份绑定</span><span class="pm-field-hint">未绑定时生成验证码，在 QQ 私聊机器人发送“绑定 6 位验证码”</span></div><div class="pm-binding-code"><button v-if="!bot.owner_bound" type="button" class="pm-style-chip" :disabled="bindingBotId === bot.id" @click="createBindingCode(bot)">{{ bindingBotId === bot.id ? '生成中…' : '生成验证码' }}</button><span v-if="bindingCodes[bot.id]" class="pm-binding-code-value">绑定 {{ bindingCodes[bot.id].code }}（{{ bindingCodes[bot.id].expiresIn }} 秒内）</span><span v-else-if="bot.owner_bound" class="pm-field-hint">已绑定</span></div></div>
...
interface Bot { id: number; platform: string; name?: string; sandbox?: boolean; app_id?: string; enabled?: boolean; group_chat_enabled?: boolean; group_requires_at?: boolean; group_read_enabled?: boolean; group_response_mode?: string; group_allowed_tools?: string[]; owner_bound?: boolean }
