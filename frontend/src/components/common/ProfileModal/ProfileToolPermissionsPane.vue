<template>
  <div>
    ...
    <div class="pm-section"><div class="pm-section-label">说明</div><div class="pm-field-row"><div class="pm-field-desc"><span class="pm-field-hint">Shell 后端由当前权限自动决定：默认使用用户沙盒；绑定工作区时工作区只作为默认目录；只有本地自托管同时开启系统权限时才使用系统执行器。管理员或用户未开放对应权限时，Shell 会被禁止。</span></div></div></div>
  </div>
</template>
...
onMounted(async () => { try { const status = await workspacesApi.status(); globalEnabled.value = status.globalEnabled; systemGlobalEnabled.value = status.systemGlobalEnabled; dangerousGlobalEnabled.value = status.dangerousGlobalEnabled } finally { loading.value = false } })
</script>
