<template>
  <div>
    ...
    <div class="pm-section smtp-section">
      <div class="pm-section-label">{{ t('profileToolUi.smtpTitle') }}</div>
      <p class="pm-field-hint">{{ t('profileToolUi.smtpHint') }}</p>
      <div class="smtp-grid">
        <input v-model="smtp.host" class="form-input" :placeholder="t('profileToolUi.smtpHost')" autocomplete="off" />
        <input v-model.number="smtp.port" class="form-input" type="number" min="1" max="65535" :placeholder="t('profileToolUi.smtpPort')" />
        <input v-model="smtp.user" class="form-input" :placeholder="t('profileToolUi.smtpUser')" autocomplete="username" />
        <input v-model="smtp.password" class="form-input" type="password" :placeholder="smtp.configured ? t('profileToolUi.smtpPasswordKeep') : t('profileToolUi.smtpPassword')" autocomplete="new-password" />
        <input v-model="smtp.fromAddr" class="form-input" :placeholder="t('profileToolUi.smtpFrom')" autocomplete="off" />
      ...
      <div v-if="smtpMessage" class="pm-msg" :class="smtpMessageType">{{ smtpMessage }}</div>
    </div>
    <div class="pm-sep"></div>
    <div class="pm-section">
...
const smtp = reactive({ host: '', port: 587, user: '', password: '', fromAddr: '', toAddr: '', useSsl: false, enabled: true, configured: false, saving: false, testing: false })
const smtpMessage = ref('')
const smtpMessageType = ref('ok')
...
onMounted(async () => { try { const row = await preferencesApi.getSmtp(); if (row) Object.assign(smtp, row, { password: '' }) } catch { /* SMTP 未配置 */ } })
...
async function saveSmtp() { smtp.saving = true; smtpMessage.value = ''; try { await preferencesApi.saveSmtp({ host: smtp.host.trim(), port: Number(smtp.port), user: smtp.user.trim(), ...(smtp.password ? { password: smtp.password } : {}), fromAddr: smtp.fromAddr.trim(), useSsl: smtp.useSsl, enabled: smtp.enabled }); smtp.password = ''; smtp.configured = true; smtpMessage.value = t('profileToolUi.smtpSaved'); smtpMessageType.value = 'ok' } catch { smtpMessage.value = t('profileToolUi.smtpSaveFailed'); smtpMessageType.value = 'err' } finally { smtp.saving = false } }
async function testSmtp() { if (!smtp.host || smtp.testing) return; smtp.testing = true; smtpMessage.value = ''; try { const result = await preferencesApi.testSmtp({ host: smtp.host.trim(), port: Number(smtp.port), user: smtp.user.trim(), password: smtp.password, fromAddr: smtp.fromAddr.trim(), toAddr: smtp.toAddr.trim() || undefined, useSsl: smtp.useSsl }); smtpMessage.value = result.ok ? t('profileToolUi.smtpTestPassed') : t('profileToolUi.smtpTestFailed'); smtpMessageType.value = result.ok ? 'ok' : 'err' } catch { smtpMessage.value = t('profileToolUi.smtpTestFailed'); smtpMessageType.value = 'err' } finally { smtp.testing = false } }
</script>
