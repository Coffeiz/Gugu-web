...
type LazyThumbValue = { id?: number | string; size?: string; revision?: number | string }

const ROOT_MARGIN: Record<string, string> = { tiny: '400px', card: '250px' }

function _load(el: LazyThumbEl, id: number | string, size: string = 'card', revision?: number | string, tries = 0) {
  const cached = getCachedThumb(id, size, revision)
  if (cached) {
    el.src = cached
  ...
  const obs = new IntersectionObserver(([entry]) => {
    ...
    getThumb(id, size, revision).then((url: string | null | undefined) => {
      ...
      // 有上限（最多 3 次）+ setTimeout 退避，避免「仍在视口 → 立即再触发 → 失败 → 再试」的紧贴死循环。
        ...
        el._lazyThumbRetry = setTimeout(() => { el._lazyThumbRetry = null; _load(el, id, size, revision, tries + 1) }, 600 * (tries + 1))
      }
    })
...
export const vLazyThumb = {
  mounted(el: LazyThumbEl, { value: { id, size, revision } }: { value: LazyThumbValue }) {
    if (id) _load(el, id, size, revision)
  },
