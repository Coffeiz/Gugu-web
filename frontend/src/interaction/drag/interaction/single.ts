...
export function startPhysicsDrag(event: PointerEvent | DragEvent, sourceEl: HTMLElement, opts: PhysicsDragOpts = {}, deps: SingleDragDeps) {
  ...
  function end() {
    ...
    let drawerScrollTargetTop: number | null = null
        ...
        if (opts.absorbShrink ?? true) {
          ...
          const landOnAbsorbTarget = async () => {
            ...
            const measureTargetLayout = () => {
              const measured = deps.layoutBoxAtTransitionsEnd(sc, targetEl)
              if (sc && drawerScrollTargetTop !== null) {
                return {
                  ...measured,
                  top: measured.top + (sc.scrollTop - drawerScrollTargetTop),
                }
              }
              return measured
            }
