...
const props = defineProps({
  ...
  placeholder: { type: String, default: '选择服务商' },
})
...
const selectedLabel = computed(() => {
  ...
  if (!provider) return props.placeholder
...
.provider-trigger { display:flex; align-items:center; justify-content:space-between; gap:8px; width:100%; height:34px; padding:0 12px; border:1px solid var(--input-border); border-radius:9px; background:var(--input-bg); color:var(--input-fg); font:13px var(--font-sans); cursor:pointer; text-align:left; }
...
:global(.provider-popup .provider-option), :global(.provider-child-popup .provider-child) { display:flex; align-items:center; justify-content:space-between; gap:10px; width:100%; padding:var(--popup-item-padding); border:0; border-radius:var(--popup-item-radius); background:transparent; color:var(--popup-item-fg); font:13px var(--font-sans); text-align:left; cursor:pointer; }
