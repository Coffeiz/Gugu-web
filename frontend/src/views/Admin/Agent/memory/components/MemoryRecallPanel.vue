  ...
  <section class="config-card">
    ...
    <div class="behavior-grid">
      ...
      <div class="behavior-item full-row" :class="{ 'is-disabled': !ragEnabled }">
        ...
        <div class="source-row">
          ...
          <div class="source-bulk">
            <button type="button" class="choice-chip source-bulk-btn" :disabled="!ragEnabled" @click="ragAutoSources = [...RAG_SOURCE_KEYS]">{{ t('adminAgentMemory.sourceSelectAll') }}</button>
            <button type="button" class="choice-chip source-bulk-btn" :disabled="!ragEnabled || !ragAutoSources.length" @click="ragAutoSources = []">{{ t('adminAgentMemory.sourceClearAll') }}</button>
          </div>
        </div>
      </div>
      ...
      <div class="behavior-item full-row">
        ...
        <ToggleSwitch v-model="capabilityRagEnabled" :aria-label="t('adminAgentMemory.toggleCapability')" />
      </div>
      <div class="behavior-item full-row" :class="{ 'is-disabled': !capabilityRagEnabled }">
        ...
        <ToggleSwitch v-model="capabilityRagShadow" :aria-label="t('adminAgentMemory.toggleShadow')" :disabled="!capabilityRagEnabled" />
      </div>
      <div class="behavior-item full-row" :class="{ 'is-disabled': !capabilityRagEnabled }">
      ...
      <div class="behavior-item full-row">
        ...
        <ToggleSwitch v-model="embeddingDraft.enabled" :aria-label="t('adminAgentMemory.toggleEmbedding')" />
      </div>
      <div class="behavior-item full-row">
        ...
        <ToggleSwitch v-model="embeddingDraft.multimodal" :aria-label="t('adminAgentMemory.toggleMultimodal')" />
      </div>
...
.source-row{display:flex;flex-wrap:wrap;align-items:center;gap:6px;justify-content:flex-end;max-width:520px}.source-chip,.source-bulk-btn{cursor:pointer;border:1px solid var(--choice-chip-border);font:500 12px var(--font-sans);user-select:none;transition:color var(--motion-hover-control) var(--motion-ease-standard),background-color var(--motion-hover-control) var(--motion-ease-standard),border-color var(--motion-hover-control) var(--motion-ease-standard)}.source-chip.active{font-weight:600}.source-chip:disabled,.source-bulk-btn:disabled{opacity:.45;cursor:not-allowed;pointer-events:none}.source-bulk{display:flex;gap:6px;margin-left:4px}.source-bulk-btn{white-space:nowrap}
</style>
