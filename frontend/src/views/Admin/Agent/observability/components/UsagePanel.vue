<template>
  ...
  <template v-else-if="usage">
    <SegmentedTabs v-model="period" :tabs="periodTabs" :aria-label="t('adminUsageUi.timezone')" class="period-tabs" />
...
const periodTabs = computed(() => [
  { key: 'today', label: t('adminUsageUi.today') },
  { key: 'recent', label: t('adminUsageUi.recent') },
])
...
// 场景表跟随「今日 / 最近 7 天」时段：后端按同一时区边界返回两套聚合
const scenarioRows = computed(() => period.value === 'today' ? (usage.value?.by_scenario || []) : (usage.value?.by_scenario_recent || []))
...
.period-tabs { margin-bottom: 12px; align-self: flex-start; }
