  ...
  <div class="design-page">
    ...
    <main class="design-content">
      ...
      <section class="design-section">
        ...
        <div class="subsection">
          <div class="subheading"><h3>Scrollbar</h3><p>透明轨道、细滑块、稳定预留空间。</p></div>
          <div class="scrollbar-token-layout">
            <div>
              ...
              <div class="scrollbar-token-label scrollbar-style-heading">密度样式 · 3 档</div>
              <div class="token-grid scrollbar-style-grid">
                <article v-for="token in scrollbarStyles" :key="token.name" class="token-card scrollbar-style-card">
                  <div class="scrollbar-size-swatch"><span :style="{ width: `var(${token.name})`, height: `var(${token.name})` }" /></div>
                  <div class="token-meta"><strong>{{ token.label }}</strong><code>{{ token.name }}</code><span>{{ token.note }}</span></div>
                </article>
              </div>
            </div>
            <div class="scrollbar-preview scroll-surface scroll-surface--compact" aria-label="滚动条预览">
              <p v-for="n in 12" :key="n">滚动表面示例 {{ n }} · Scroll surface contract</p>
            </div>
          </div>
        </div>
...
const scrollbarStyles = [
  { label: 'Default', name: '--scrollbar-size-default', note: '页面 / 普通容器', kind: 'size' },
  { label: 'Compact', name: '--scrollbar-size-compact', note: '紧凑列表', kind: 'size' },
  { label: 'Editor', name: '--scrollbar-size-editor', note: '编辑器', kind: 'size' },
]
const typeScale = [
...
.scrollbar-token-layout{display:grid;grid-template-columns:minmax(0,1fr) 220px;gap:var(--design-grid-gap);align-items:stretch}.scrollbar-token-label{margin-bottom:var(--space-sm);color:var(--content-tertiary);font-size:var(--font-size-xs);font-weight:var(--font-weight-semibold)}.scrollbar-style-heading{margin-top:var(--space-lg)}.scrollbar-style-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.scrollbar-size-swatch{height:56px;display:flex;align-items:center;padding:0 var(--space-lg);border-bottom:1px solid var(--border-hairline)}.scrollbar-size-swatch span{display:block;max-width:100%;border-radius:var(--radius-pill);background:var(--scrollbar-thumb)}.scrollbar-preview{height:184px;overflow-y:auto;padding:var(--space-md);border:1px solid var(--border-subtle);border-radius:var(--radius-md);background:var(--surface-soft)}.scrollbar-preview p{padding:var(--space-sm);border-radius:var(--radius-sm);color:var(--content-secondary);background:var(--surface-raised);font-size:var(--font-size-xs)}.scrollbar-preview p+p{margin-top:var(--space-xs)}
