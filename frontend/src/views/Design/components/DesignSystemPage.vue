...
const contracts = [
  ...
  {name:'Topbar',token:'--topbar-*',note:'静态半透明 tint',icon:PhBrowser},
...
.design-section{position:relative;margin-bottom:var(--design-section-gap);padding:var(--space-xl);background:var(--design-section-bg);border:1px solid var(--design-section-border);border-radius:var(--design-section-radius);box-shadow:var(--design-section-shadow),inset 0 1px 0 var(--design-section-highlight)}
...
.sample-sidebar{height:100%;display:flex;flex-direction:column;padding:var(--space-xl) var(--space-md) var(--space-md);background:var(--sidebar-bg);border-right:1px solid var(--border-subtle);box-shadow:inset -1px 0 0 var(--sidebar-highlight)}
:global(html[data-family='mono']) .sample-sidebar{background:var(--chrome-glass-bg);border-right-color:var(--chrome-glass-border);box-shadow:var(--chrome-glass-shadow)}
...
.sample-main > .sample-topbar { top: 20px; right: 24px; left: 20px; z-index: 40; height: auto; min-height: 52px; gap: 14px; padding: 14px 20px; overflow: visible; }
