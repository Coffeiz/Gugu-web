  ...
  <main class="token-page">
    <div class="token-shell">
      ...
      <section class="hero-note">
        ...
        <div><b>02 · Light / Dark 独立于主题</b><span>Glass 与 V2 各自拥有浅色和深色，组件消费同一套 semantic/component token。</span></div>
      ...
      <TokenSection index="05" title="Elevation" subtitle="Glass 使用环境高光 + 柔阴影建立层级；V2 使用更克制的 surface elevation。" code="elevation">
...
const themeMatrix: Array<{ family: ThemeFamily; mode: ResolvedTheme; label: string; description: string }> = [
  { family: 'glass', mode: 'light', label: 'Glass · Light', description: '当前 dev 主视觉' }, { family: 'glass', mode: 'dark', label: 'Glass · Dark', description: '深色玻璃' }, { family: 'v2', mode: 'light', label: 'V2 · Light', description: 'Pearl / Ink / Iris' }, { family: 'v2', mode: 'dark', label: 'V2 · Dark', description: '低彩深色工作台' },
]
const columns = [
...
const rules = [{ title: 'Family ≠ Mode', description: 'Glass / V2 定义视觉语言；Light / Dark 定义色彩模式，四种状态正交组合。' }, { title: 'One semantic contract', description: '组件只使用 surface、content、border、action、status 和 elevation。' }, { title: 'Glass may stay glass', description: 'Glass 允许普通卡片、侧栏和控制面继续使用半透明、高光与 blur。' }, { title: 'V2 stays quieter', description: 'V2 内容面更实、更中性，Iris 主要承担交互语义。' }, { title: 'Content colors survive', description: '项目色、日历色和画布内容色属于内容层，两套主题都保留。' }, { title: 'Switch at the root', description: '根节点使用 data-family 与 data-theme，新增主题时无需复制组件 CSS。' }]
...
.theme-v2-light .theme-cell-preview { --mini-bg: linear-gradient(#f5f3f6,#eeecf0); --mini-side: rgba(248,246,249,.84); --mini-card: rgba(255,255,255,.74); --mini-border: rgba(42,35,49,.08); }
.theme-v2-dark .theme-cell-preview { --mini-bg: linear-gradient(#1c1921,#17151b); --mini-side: rgba(25,23,30,.86); --mini-card: rgba(38,34,43,.78); --mini-border: rgba(255,255,255,.08); }
