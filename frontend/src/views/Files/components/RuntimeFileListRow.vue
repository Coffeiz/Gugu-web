<template>
  <div ref="elementRef" class="list-row" :class="{ selected: selectedIds.has(item.id), 'pre-selected': previewFileIds.has(item.id), cut: cbStore.type === 'cut' && cbStore.fileIds.includes(item.id) }" :data-file-id="item.id" data-layout-role="card" :data-layout-key="runtimeId" @contextmenu.prevent.stop="openCtx('file', item, $event)" @click.stop="handleFileClick(item, $event)">
    ...
    <span class="lr-proj-cell"><span v-if="item.projectColor" class="lr-dot" :style="{ background: item.projectColor || '' }"></span><span class="lr-projname">{{ item.projectName || item.stageName || '—' }}</span></span><span class="lr-text">{{ fmtBytes(item.sizeBytes) }}</span><span class="lr-text">{{ item.createdAt }}</span>
...
const { selectedIds, previewFileIds, cbStore, handleFileClick, openCtx, fileListIcon, fileIconColor, renamingFileId, renameText, renameExtension, commitRename, cancelRename, startRenameFile, downloadFile, deleteSingleFile, isExtractableArchive, extractFile, inSelectionMode } = props.context
</script>
