...
const props = defineProps({ item: { type: Object as PropType<FileMeta>, required: true }, context: { type: Object as PropType<Record<string, any>>, required: true }, runtimeId: { type: String, required: true }, runtimeSurfaceId: { type: String, required: true }, runtimeAbilities: { type: Array as PropType<readonly string[]>, default: () => ['move'] } })
const { elementRef } = useObject({ id: props.runtimeId, type: 'file-item', surface: () => props.runtimeSurfaceId, abilities: () => props.runtimeAbilities })
const { selectedIds, previewFileIds, draggingFileIds, cbStore, handleFileClick, onFilePointerDown, openCtx, fileListIcon, fileIconColor, renamingFileId, renameText, commitRename, cancelRename, startRenameFile, downloadFile, deleteSingleFile, inSelectionMode } = props.context
</script>
