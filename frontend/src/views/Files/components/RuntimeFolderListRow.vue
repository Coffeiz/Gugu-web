<template>
  <div ref="elementRef" class="list-row folder-row" :class="{ selected: selectedFolderKeys.has(item.id), 'pre-selected': previewFolderKeys.has(item.id), 'drag-over': dragOverFolderId === item.folderId }" :data-folder-key="item.id" :data-folder-id="item.folderId" data-layout-role="card" :data-layout-key="runtimeId" @click.stop="handleFolderClick(item, $event)" @contextmenu.prevent.stop="openCtx('folder', item, $event)">
...
const { selectedFolderKeys, previewFolderKeys, dragOverFolderId, handleFolderClick, openCtx, folderListIcon, folderAccentColor, renamingFolderKey, renameText, commitRename, cancelRename, startRenameFolder, downloadFolder, deleteFolder, inSelectionMode } = props.context
</script>
