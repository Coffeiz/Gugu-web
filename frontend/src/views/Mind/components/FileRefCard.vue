<template>
  ...
  >
    <!-- 直接复用真正的文件卡片模块（components/common/FileCard.vue），不是照抄样式的仿制品；
         走它开放的插槽把 meta 换成「所属项目 · 大小」、悬浮操作换成移除、图片文件补一份跟
         「最近文件」面板一样的缩略图。建立关联改成边缘圆点拖出连线，见 conn-dot。 -->
    <FileCard v-if="file" ref="fileCardRef" :ext="file.ext" :display-name="file.displayName" :has-thumb="isImageExt(file.ext)">
      ...
      <!-- 连接点也走插槽（是 .fc-card 的子节点）——放在 .fr-wrap 上则是拖拽克隆的兄弟节点，
           不会跟着 startPhysicsDrag 的 cloneNode 一起飞，卡片飞走了圆点却留在原地。 -->
...
const { onPointerDown } = useCardDrag({
  ...
  getDragEl: () => (fileCardRef.value?.$el as HTMLElement) ?? wrapRef.value,
...
.fr-wrap { position: absolute; box-sizing: border-box; cursor: pointer; user-select: none; touch-action: none; border-radius: 14px; }
...
.fr-wrap :deep(.fc-card) { width: 100%; overflow: visible; }
.fr-wrap.connecting :deep(.fc-card) { border-style: dashed; }
...
.fr-wrap:hover .fr-actions { opacity: 1; }
...
.fr-wrap:hover .conn-dot { opacity: 1; }
