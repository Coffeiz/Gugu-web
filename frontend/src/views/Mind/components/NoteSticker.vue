<template>
  ...
  <div
    ...
    :class="{ tombstone: !!item.node.deletedAt, 'is-resizing': isResizing }"
    :style="stickerStyle"
...
const stickerStyle = computed(() => {
  const style: Record<string, string> = { left: `${renderedSize.value.x}px`, top: `${renderedSize.value.y}px`, width: `${renderedSize.value.w}px`, height: `${renderedSize.value.h}px`, zIndex: `${props.item.z}` }
  return style
})
...
function emitMeasuredSize() {
  ...
  // wrap 的 left/top/width/height 都是世界坐标（外层 .canvas-world 自带 camera.scale 的
  // transform），所以 ResizeObserver 的屏幕像素需要除以 scale 后再回报。
...
let resizeState: { handle: ResizeHandle; pointerId: number; startX: number; startY: number; start: { x: number; y: number; w: number; h: number } } | null = null
function startResize(event: PointerEvent, handle: ResizeHandle) {
  if (event.button !== 0 || props.item.node.deletedAt) return
  isResizing.value = true
  resizeState = { handle, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, start: { ...renderedSize.value } }
...
function onResizeMove(event: PointerEvent) {
  ...
  const dx = (event.clientX - state.startX) / (props.scale || 1)
  const dy = (event.clientY - state.startY) / (props.scale || 1)
  const { handle, start } = state
  let { x, y, w, h } = start
  if (handle.includes('e')) w = Math.min(CANVAS_NOTE_SIZE.max.w, Math.max(CANVAS_NOTE_SIZE.min.w, start.w + dx))
  if (handle.includes('s')) h = Math.min(CANVAS_NOTE_SIZE.max.h, Math.max(CANVAS_NOTE_SIZE.min.h, start.h + dy))
  if (handle.includes('w')) {
    const nextW = Math.min(CANVAS_NOTE_SIZE.max.w, Math.max(CANVAS_NOTE_SIZE.min.w, start.w - dx))
    x = start.x + start.w - nextW
    w = nextW
  }
  if (handle.includes('n')) {
    const nextH = Math.min(CANVAS_NOTE_SIZE.max.h, Math.max(CANVAS_NOTE_SIZE.min.h, start.h - dy))
    y = start.y + start.h - nextH
    h = nextH
  }
  resizePreview.value = { x, y, w, h }
  emit('measured', props.item, { w, h })
}
async function finishResize(event: PointerEvent) {
  ...
  const next = resizePreview.value
  if (!next) return
  try {
    await store.updateCanvasItem(props.item.id, { x: next.x, y: next.y, w: next.w, h: next.h })
...
.canvas-note-wrap.is-resizing :deep(.note-card) { transition: none; }
.canvas-note-resize-handles { position: absolute; inset: 0; z-index: 4; pointer-events: none; }
.canvas-note-resize-handle { position: absolute; pointer-events: auto; touch-action: none; }
...
.canvas-note-resize-handle.is-sw { left: -6px; bottom: -6px; cursor: nesw-resize; }
.canvas-note-resize-handle.is-nw { left: -6px; top: -6px; cursor: nwse-resize; }
