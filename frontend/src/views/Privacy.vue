...
- **咕咕记忆**：咕咕从对话中提炼的关于你的稳定事实、近期记忆和长期沉淀（存储在你账户下的私有文件中，见第四节）
- **头像**：你上传的个人头像图片
- **偏好与运行设置**：语言、时区、主题、回复偏好、模型和通知设置，用于按你的选择提供服务

### IM 接入时（飞书 / QQ / 微信，可选）
- **Bot 凭据**：你填写的 App Secret（微信为 iLink bot token）用 AES-256-GCM 加密后存储；App ID 是机器人的公开标识符（不是密钥本身），明文存储。仅用于维持机器人连接
...
接入 IM Bot 后，你通过飞书 / QQ / 微信发给 Bot 的消息会经过咕咕服务器，并适用对应平台的隐私政策或隐私保护指引。
...
咕咕会在对话后自动提炼你的信息，存入六层记忆文件：

- **用户画像**（profile）：你的身份、职业、所在地、稳定喜好（几乎不变，不打折扣）
- **行为模式**（pattern）：你做事/做决定的习惯（结构化存储，区分「你亲口说的」与「咕咕推断的」，推断会随时间淡出）
...
| 对话记录 | 账户存续期间 | 账户注销时删除 |
| 咕咕记忆文件 | 账户存续期间 | 可通过对话指令清除，或账户注销时删除 |
...
- **IM 凭据**：App Secret / bot token 用 AES-256-GCM 加密存储；App ID（公开标识符）明文存储
...
咕咕不使用 Cookie 进行广告或跨站追踪。登录状态通过 **JWT Token** 维持：

- 浏览器优先使用 HttpOnly、SameSite 的登录 Cookie；写操作配套 CSRF Cookie 和请求校验
- 前端仍保留 \`localStorage\` 中的 JWT 兼容令牌，用于现有客户端请求和会话恢复
- Token 有效期由服务端配置，登出后会清除 Cookie 和本地令牌
- Token 仅包含认证所需的用户 ID 和角色，不含密码或 API Key
